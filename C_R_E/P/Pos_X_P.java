...
public class Pos_X_P extends Pos_X_P_ZhuCi implements X_P {
    public int P_BaDongCiOfTwo(List<String> outputList, int countLength
        ...
        if (S_Maps.daiCi.containsKey(prefixWord[S_Pos.INT_ZERO].toString())
            || S_Maps.fuCi.containsKey(prefixWord[S_Pos.INT_ZERO].toString())) {
            countLength = charOfTwo(countLength, outputList, strings, prefixWord);
            return countLength;
        }
    ...
    public int P_ShiTaiCiOfThree(List<String> outputList, int countLength
        ...
        if (S_Maps.mingCi.containsKey(strings[S_Pos.INT_TWO].toString())) {
            ...
            if (S_Maps.jieCi.containsKey(prefixWord[S_Pos.INT_ZERO].toString())) {
                countLength = charOfThree(countLength, outputList, strings, prefixWord);
                return countLength;
            }
        }
        if (S_Maps.dongCi.containsKey(strings[S_Pos.INT_TWO].toString())
            || S_Maps.liangCi.containsKey(strings[S_Pos.INT_TWO].toString())) {
            countLength = charOfThree(countLength, outputList, strings, prefixWord);
            return countLength;
        }
        if (S_Maps.CiTwo.containsKey(strings[S_Pos.INT_ONE])) {
    ...
    public int P_FuCiOfThree(List<String> outputList, int countLength, String[] strings
        , StringBuilder[] prefixWord) {
        if (S_Maps.fuCi.containsKey(strings[S_Pos.INT_TWO].toString())) {
            if (S_Maps.fuCi.containsKey(prefixWord[S_Pos.INT_ZERO].toString())) {
                countLength = charOfThree(countLength, outputList, strings, prefixWord);
                return countLength;
            }
        }
