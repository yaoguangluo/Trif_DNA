...
public class Pos_X_P_JieCi extends Pos_X_E implements X_P_JieCi {
    public int P_JieCiOfThree(List<String> outputList, int countLength, String[] strings
        , StringBuilder[] fixWord) {
        if (S_Pos.INT_ZERO == outputList.size() && (wordsForest.get(strings[S_Pos.INT_TWO])
            .toString().contains(S_Pos.NLP_CI_WEI))) {
            ...
            if (S_Maps.qingTaiCi.containsKey(fixWord[S_Pos.INT_ZERO].toString())
                ...
                if (S_Maps.dongCi.containsKey(fixWord[S_Pos.INT_ZERO].toString())) {
                    if (S_Maps.xingWeiCi.containsKey(strings[S_Pos.INT_ONE])
                        || S_Maps.xingRongCi.containsKey(strings[S_Pos.INT_ONE])) {
                        countLength = twoCharOfThree(countLength
    ...
    public int P_JieCiOfThreeForMap(IMV_SIQ_SS outputList, int countLength
        ...
        if (outputList.size() > S_Pos.INT_ZERO
            ...
            if (S_Maps.lianCi.containsKey(fixWord[S_Pos.INT_ZERO].toString())
                || S_Maps.qingTaiCi.containsKey(fixWord[S_Pos.INT_ZERO].toString())
                || S_Maps.weiCi.containsKey(fixWord[S_Pos.INT_ZERO].toString())) {
                countLength = charOfThreeForMap(countLength
