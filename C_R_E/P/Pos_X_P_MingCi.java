...
public class Pos_X_P_MingCi extends Pos_X_P_LiangCi implements X_P_MingCi {
	public int P_MingCiOfTwo(List<String> outputList, int countLength,
	...
	int doP_MingCiOfTwoFinds(StringBuilder[] fixWord, String[] strings,
			List<String> outputList, int countLength) {
		if (S_Maps.CiTwo.containsKey(strings[S_Pos.INT_ONE])) {
			fixWord[S_Pos.INT_ZERO].delete(S_Pos.INT_ZERO,
					fixWord[S_Pos.INT_ZERO].length());
			fixWord[S_Pos.INT_ZERO].append(strings[S_Pos.INT_ONE]);
	...
	public int P_MingCiOfThree(List<String> outputList, int countLength,
		...
		if (S_Maps.xingWeiCi.containsKey(fixWord[S_Pos.INT_ZERO].toString())
			...
			if (S_Maps.dongCi.containsKey(strings[S_Pos.INT_THREE])) {
				if (S_Maps.fuCi.containsKey(S_Pos.EMPTY_STRING
						+ (0 == fixWord[S_Pos.INT_ONE].length() ? "@^_^@"
								: fixWord[S_Pos.INT_ONE]
										.charAt(S_Pos.INT_ZERO)))) {
			...
			if (S_Maps.mingCi.containsKey(strings[S_Pos.INT_TWO])) {
				if (S_Pos.INT_ZERO < fixWord[S_Pos.INT_ONE].length()
						&& S_Maps.zhuCi.containsKey(
								S_Pos.EMPTY_STRING + fixWord[S_Pos.INT_ONE]
										.charAt(S_Pos.INT_ZERO))) {
					if (S_Maps.CiTwo.containsKey(strings[S_Pos.INT_ONE])) {
						if (!S_Maps.dongCi.containsKey(
								fixWord[S_Pos.INT_ZERO].toString())) {
							countLength = charOfThree(countLength, outputList,
									strings, fixWord);
							return countLength;
						}
						countLength = twoCharOfThree(countLength, outputList,
	...
	public int P_MingCiOfTwoForMap(IMV_SIQ_SS outputList, int countLength,
		...
		if (wordsForest.containsKey(fixWord[S_Pos.INT_ZERO].toString())) {
			if (S_Maps.liangCi
					.containsKey(fixWord[S_Pos.INT_ZERO].toString())) {
				countLength = charOfTwoForMap(countLength, outputList, strings,
						fixWord, NE);
				return countLength;
			}
			countLength -= S_Pos.INT_TWO;
...
//} //P_MingCiOfTwo 太长比较条件拆分。
//版权源码中条件太多的都可以这样拆分，通过降低分词计算速度 来换 funtion call 虽然不可取，但是源码好看。
//if (!find) {
//}
//            if (find) {
//                if (S_Maps.CiTwo.containsKey(strings[S_Pos.INT_ONE])) {
