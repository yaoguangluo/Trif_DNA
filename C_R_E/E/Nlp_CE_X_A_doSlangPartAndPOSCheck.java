...
public class Nlp_CE_X_A_doSlangPartAndPOSCheck extends XCE_XAS_U
    ...
    public int forTwoChar(int countLength, List<String> outputList, StringBuilder stringBuilder
        , StringBuilder[] prefixWord, int charPosition, StringBuilder textInputString) {
        //Pos_C_X_P_chuLiMingCi Pos_C_X_P= (Pos_C_X_P_chuLiMingCi)posUtils;
        ...
        if (temp == S_Pos.INT_ZERO) {
            ...
            prefixWord[S_Pos.INT_ZERO].delete(S_Pos.INT_ZERO, temp);
            prefixWord[S_Pos.INT_ZERO].append(stringBuilder.charAt(S_Pos.INT_ZERO));
            outputList.add(S_Pos.EMPTY_STRING + stringBuilder.charAt(S_Pos.INT_ZERO));
            return countLength - S_Pos.INT_ONE;
        }
        String[] strings = new String[S_Pos.INT_TWO];
        strings[S_Pos.INT_ZERO] = String.valueOf(countWordNode.charAt(S_Pos.INT_ZERO));
        strings[S_Pos.INT_ONE] = countWordNode;
        if (S_Maps.mingCi.containsKey(strings[S_Pos.INT_ZERO])) {
            countLength = pos_X_P.P_MingCiOfTwo(outputList, countLength, strings, prefixWord
                , charPosition, textInputString);
            return countLength;
        }
        if (S_Maps.baDongCi.containsKey(strings[S_Pos.INT_ZERO])) {
            countLength = pos_X_P.P_BaDongCiOfTwo(outputList, countLength, strings, prefixWord);
            return countLength;
        }
        if (S_Maps.jieCi.containsKey(strings[S_Pos.INT_ZERO])) {
            if (S_Maps.dongCi.containsKey(prefixWord[S_Pos.INT_ZERO].toString())) {
                if (!S_Maps.jieCi.containsKey(countWordNode)) {
                    countLength = pos_X_P.charOfTwo(countLength, outputList, strings, prefixWord);
                    return countLength;
                }
            }
        }
    ...
    public int forTwoCharForMap(int countLength, IMV_SIQ_SS outputList
        ...
        if (prefixWord[S_Pos.INT_ZERO].length() == S_Pos.INT_ZERO) {
            Util_X_prefixWordEqualZero._E(outputList, countWordNode, prefixWord, NE);
            return countLength;
        }
        String[] strings = new String[S_Pos.INT_TWO];
        strings[S_Pos.INT_ZERO] = String.valueOf(countWordNode.charAt(S_Pos.INT_ZERO));
        ...
        if (wordsForest.containsKey(strings[S_Pos.INT_ZERO])) {
            ...
            if (temp.toString().contains(S_Pos.NLP_CI_MING)) {
                countLength = pos_X_P.P_MingCiOfTwoForMap(outputList, countLength, strings, prefixWord, NE);
                return countLength;
            }
        }
