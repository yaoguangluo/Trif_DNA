...
public class Nlp_CE_X_S extends Nlp_CE_X_A_doPOSAndEMMCheck
    ...
    public int _E(int countLength, List<String> output, StringBuilder stringBuilder
        ...
        String preRegister = S_Pos.EMPTY_STRING + inputString.charAt(S_Pos.INT_ZERO)
            + inputString.charAt(S_Pos.INT_ONE);
        String inRegister = S_Pos.EMPTY_STRING + inputString.charAt(S_Pos.INT_ONE)
            + inputString.charAt(S_Pos.INT_TWO);
        String postRegister = S_Pos.EMPTY_STRING + inputString.charAt(S_Pos.INT_TWO)
            + inputString.charAt(S_Pos.INT_THREE);
        if (S_Maps.dongCi.containsKey(S_Pos.EMPTY_STRING + inputString.charAt(S_Pos.INT_THREE)
            + prefixWord[S_Pos.INT_ONE].charAt(S_Pos.INT_ZERO))) {
            countLength = ofThree(--countLength, output, stringBuilder.delete(S_Pos.INT_THREE
                , S_Pos.INT_FOUR), prefixWord, charPosition, textInputString);
            return countLength;
        }
        boolean A = S_Maps.CiTwo.containsKey(preRegister);
        if (A) {
            if (S_Maps.CiTwo.containsKey(postRegister)) {
                String string = S_Pos.EMPTY_STRING + inputString.charAt(S_Pos.INT_ZERO);
                if (S_Maps.xingWeiCi.containsKey(prefixWord[S_Pos.INT_ZERO].toString())
                ...
                if (S_Maps.zhuCi.containsKey(string)) {
                    ...
                    countLength = pos_X_P.P_ZhuCiOfThree(output, countLength - S_Pos.INT_ONE
                        , strings, prefixWord);
                    return countLength;
                }
        ...
        if (S_Maps.CiThree.containsKey(preRegister + inputString.charAt(S_Pos.INT_TWO))
        ...
        if (A) {
            ...
            countLength = forTwoChar(countLength - S_Pos.INT_TWO, output, stringBuilder
                .delete(S_Pos.INT_TWO, S_Pos.INT_FOUR), prefixWord, charPosition, textInputString);
            return countLength;
        }
    ...
    public int forMap(int countLength, List<String> output, StringBuilder stringBuilder
        , StringBuilder[] prefixWord, int charPosition, StringBuilder textInputString) {
        String inputString = stringBuilder.toString();
    ...
    public int forMap(int countLength, IMV_SIQ_SS output
        ...
        String keyWord = S_Pos.EMPTY_STRING + inputString
            .charAt(S_Pos.INT_ZERO) + inputString.charAt(S_Pos.INT_ONE);
        if (S_Maps.mingCi.containsKey(keyWord)) {
            if (S_Maps.mingCi.containsKey(S_Pos.EMPTY_STRING + inputString.charAt(S_Pos.INT_TWO)
                + inputString.charAt(S_Pos.INT_THREE))) {
                Util_X_ContainsKey._E(output, keyWord, prefixWord, NE);
                return countLength;
            }
        }
        countLength = ofThreeForMap(--countLength, output, stringBuilder.delete(S_Pos.INT_THREE
            , S_Pos.INT_FOUR), prefixWord, NE);
        return countLength;
    }
}
