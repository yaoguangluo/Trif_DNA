...
public class X_Map extends AE implements A_X_Map {
    public IMV_SIQ_SS parserStringByReturnFrequencyMap(String inputString, App NE) {
        ...
        for (int charPosition = S_Pos.INT_ZERO; charPosition < inputLength
            ; charPosition += (countLength == S_Pos.INT_ZERO
            ? S_Pos.INT_ONE : countLength)) {
            if (mixedString.charAt(charPosition) < S_Pos.INT_ONE_TWO_EIGHT) {
                if (fixWords[S_Pos.INT_ZERO].length() > S_Pos.INT_ZERO) {
                    if (fixWords[S_Pos.INT_ZERO].charAt(fixWords[S_Pos.INT_ZERO]
                        .length() - S_Pos.INT_ONE) < S_Pos.INT_ONE_TWO_EIGHT) {
                        fixWords[S_Pos.INT_ZERO].append(mixedString
                            .charAt(charPosition));
            ...
            stringBuilder = nero_C.getBinaryForestRecurWords(stringBuilder
                    .append(mixedString.charAt(charPosition)), mixedString
                , charPosition, inputLength, forestRoots, forestDepth
    ...
    public List<WordFrequency> sortWordFrequencyMap(IMV_SIQ_SS map) {
        ...
        quick6DLuoYaoguangSort.quick6DLuoYaoGuangSortWordFrequency(list, S_Pos.INT_ZERO
            , list.size() - S_Pos.INT_ONE);
        return list;
    }

    public IMV_SIQ_S_ getWordFrequencyByReturnSortMap(List<String> sets, App NE) {
    ...
    public IMV_SIQ_S_ sortWordFrequencyMapToSortMap(IMV_SIQ_SS map) {
        ...
        quick6DLuoYaoguangSort.quick6DLuoYaoGuangSortWordFrequency(listMap, S_Pos.INT_ZERO
            , listMap.size() - S_Pos.INT_ONE);
        return listMap;
    }

    public IMV_SIQ_SS parserMixStringByReturnFrequencyMap(String inputString, App NE) {
        ...
        for (int charPosition = S_Pos.INT_ZERO; charPosition < inputLength
            ...
            if (find == S_Pos.INT_ONE) {
                find = S_Pos.INT_ZERO;
                for (String temp : fHMMList.englishStringToWordsList(fixWords[S_Pos
                    .INT_ZERO].toString())) {
            ...
            stringBuilder.delete(S_Pos.INT_ZERO, stringBuilder.length());
            stringBuilder = nero_C.getBinaryForestRecurWords(stringBuilder.append(mixedString
                    .charAt(charPosition)), mixedString, charPosition, inputLength
