 ...
 * 再 Fusion_X_fusionPosition2DwithHeart 测试main函数demo的test版本，在导入了api之后进行系统
 * 集成，然后用下面的对应的函数中源码逻辑进行复制粘贴到工程中，直接运行，即可出结果，源码的逻辑按照输入
 * 准备计算的参数，然后执行，然后获取输出需要的结果，可以用断点来查看数据，也可以用println来显示输出，
 * 方便集成，对程序员友好。系统需要jdk1.8 以上的java环境，本人会把测试的输入输出都注释在这个文件里。
 * 及其傻瓜化的流程，方便商业化落地。
 * --罗瑶光
 * */
...
//
        ...
        Map<Double, List<AMV_MVS_VSQ_2D>> output = Fissile_X_fissilePosition2D._E(
            input, 13);
...
//2观测结果 用10 和11分别带入可测试。
        Map<Double, List<AMV_MVS_VSQ_2D>> fusion
            = Fusion_X_fusionPosition2DwithHeart._E(output, groupsHeart, 11);
    ...
    public static void _E(List<AMV_MVS_VSQ_2D> inputs) {
        ...
        Map<Double, List<AMV_MVS_VSQ_2D>> output
            = Fissile_X_fissilePosition2D._E(inputs, 13);
        Iterator<Double> iterator = output.keySet().iterator();
        while (iterator.hasNext()) {
            double temp = iterator.next();
            System.out.println();
...
//2观测结果 用10 和11分别带入可测试。
        Map<Double, List<AMV_MVS_VSQ_2D>> fusion
            = Fusion_X_fusionPosition2DwithHeart._E(output
            , groupsHeart, 11);
        Iterator<Double> iterator3 = fusion.keySet().iterator();
        while (iterator3.hasNext()) {
            double temp = iterator3.next();
