 ...
 * */
/*
 * 这是一个 处理fusion 先 fissile 采样 然后fusion 1 观测heart Euclid_X_findHeartPosition2D
 * 测试main函数demo的test版本，在导入了api之后进行系统集成，然后用下面的对应的函数中源码逻辑进行复制
 * 粘贴到工程中，直接运行，即可出结果，源码的逻辑按照输入准备计算的参数，然后执行，然后获取输出需要的结果
...
//
        ...
        Map<Double, List<AMV_MVS_VSQ_2D>> output
            = Fissile_X_fissilePosition2D._E(input, 13);
