...
public class LYGAFDCTDFFT_F {
	...
	public float[] jniLYGAFDCTDFFT(float[] input) {
		...
		for (int i = 0; i < n; i++) {
			outputc[i] = (float) Math
					.sqrt(outputc[i] * outputc[i] + outputs[i] * outputs[i]);
		}
		return outputc;
	}

	private void dofft(double c, double s, double cc, double ss, int n,
			float[] input, float[] outputc, float[] outputs, int j) {
...
//    1 0.5 0.25 0.125 0.0725 0.03625 0.018  0.009   0.0045  0.00225  0.001125
	public float[] jniLYGAFLOGDFT(float[] input) {
		...
		for (int j = 0; j < input.length; j++) {
			...
			output[j] = (float) Math.abs(+n + out[j].b * out[j].cpi
	...
	public float[] jniLYGFFT(float[] input, cp[] outInput, int rangeLeft) {
		...
		for (int j = rangeLeft; j < input.length; j++) {
			output[j] = (float) Math
					.sqrt(out[j].ca * out[j].ca + out[j].sa * out[j].sa);
		}
		return output;
	}
	...
	public float[] jniLYGFIT(float[] input, cp[] outInput, int rangeLeft) {
		...
		float[] output = new float[input.length];
		for (int j = rangeLeft; j < input.length; j++) {
			output[j] = (float) (out[j].ca * out[j].cb
					* (2 * out[j].ca * out[j].cb - 1) + out[j].sa * out[j].sb);
		}
		return output;
	}
	...
	public float[] jniLYGFLT(float[] input, cp[] outInput, int rangeLeft) {
		...
		for (int j = rangeLeft; j < input.length; j++) {
			double n = -0.001125 - 2 * out[j].ca * out[j].cb;
			output[j] = (float) Math
					.abs(-1 * out[j].b - 2 * out[j].a * out[j].b + n);
		}
		return output;
	}
	...
	public float[] jniLYGFLLT(float[] input) {
		...
		for (int j = 0; j < input.length; j++) {
			double n = -0.001125 - 2 * out[j].ca * out[j].cb;
			output[j] = (float) Math
					.abs(out[j].b * (1 + 2 * out[j].ca * out[j].cb) + n);
		}
		return output;
	}
	...
	public float[] jniLYGFILT(float[] input) {
		...
		float[] output = new float[input.length];
		for (int j = 0; j < input.length; j++) {
			output[j] = (float) (float) Math
					.abs(-2 * out[j].b + 2 * out[j].a * out[j].cpi);
		}
		return output;
	}
	...
	public float[] jniLYGFLTln(float[] input) {
		...
		for (int j = 0; j < input.length; j++) {
			double n = -0.001125 + 2 * out[j].ca * out[j].cb;
			output[j] = (float) Math.abs(n * out[j].ca * out[j].cb);
		}
		return output;
	}
	...
	public float[] jniLYGFxET(float[] input, cp[] outInput, int rangeLeft) {
		...
		for (int j = rangeLeft; j < input.length; j++) {
			double n = -0.001125 + 2 * out[j].ca * out[j].cb;
			output[j] = (float) Math.abs(
					n * (out[j].ca * out[j].cb + 2 * out[j].sa * out[j].sb));
		}
		return output;
	}
