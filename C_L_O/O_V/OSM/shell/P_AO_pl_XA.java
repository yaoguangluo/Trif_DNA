		...
		if (sets[1].equalsIgnoreCase("进行字符排序")) {
			...
			int returnInt = new LYG10D13D_X_CAS()
					.quick4DChineseStringArrayWithSmallInTwoChar3bihuaReturns(
							strings, 0, strings.length - 1, 80,
							SortStringDemo.pinYin, SortStringDemo.biHua, 7, 70);
			// 4 输出
			outputTemp.clear();
...
//							, SortStringDemo.biHua, 7, 70);
			...
			if (sets[2].equalsIgnoreCase("从小到大")) {
				...
				for (int i = 0; i < doubles.length; i++) {
					outputTemp.add(
							maps.get("" + doubles[doubles.length - i - 1]));
				}
			}
			output.clear();
			output.addAll(outputTemp);
		...
		if (sets[1].equalsIgnoreCase("进行分词")) {
			...
			while (iterator.hasNext()) {
				...
				if (sets[2].equalsIgnoreCase("词性显色")) {
					...
					// rowCellFromString= "<div style=\"background:white\"><font
					// color=\""+ sets[2] +"\">"
				...
				if (sets[2].equalsIgnoreCase("DNN")) {
					...
					String[][] ann = aNNTest.getANNMatrix(rowCellFromString,
							NE);
					String[][] dnn = dNNTest.getDNNMatrix(ann,
							rowCellFromString, NE);
					List<String> cigan = new LinkedList<>();
					Here: for (int i = 0; i < dnn.length; i++) {
						double dnn_lwa = 0.0;
						if (null == dnn[i][3]) {
							continue Here;
						}
