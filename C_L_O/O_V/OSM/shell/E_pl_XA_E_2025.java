 ...
 * */
public class E_pl_XA_E_2025 {
    public static TinMap E_pl_XA(String plSearch, boolean mod, TinMap output, App NE) {
        //working for here
        ...
        for (String command : commands) {
            ...
            if (temp[0].equals("获取表名")) {
                FlowerAction.doAction("P_TableName", temp, output, NE);
            }
            //acknowledge= command.split(":");
            if (temp[0].equals("setRoot")) {
                FlowerAction.doAction("P_SetRoot", temp, output, NE);
            }
            if (temp[0].equals("baseName")) {//库名
                FlowerAction.doAction("P_BaseName", temp, output, NE);
            }
            if (temp[0].equals("界面操作")) {
                FlowerAction.doAction("P_YLJOperation", temp, output, NE);
            }
            if (temp[0].equals("设")) {//新增计算变量存储。
                FlowerAction.doAction("P_StoreValues", temp, output, NE);
            }
            //确定文件夹:路径:进行搜索
            if (temp[0].equals("确定文件夹")) {//准备设计文件夹计算
                FlowerAction.doAction("P_fileOperations", temp, output, NE);
            }
            // 匹配条件:文件类型|包含|java;
            //  *     匹配条件:文件大小KB|小于|200;
            //  *     匹配条件:文件名|包含|哈哈;
            ...
            if (temp[0].equals(S_ShellETL.SHELL_ETL_CULUMNNAME)) {
                FlowerAction.doAction("P_ListNeedStart", temp, output, NE);
            ...
            if (temp[0].equals("join")) {
                FlowerAction.doAction("P_Join", temp, output, NE);
        ...
        Pl_XA_Command_E.P_Check(output.get("newCommand").toString(), output, mod, NE);
        return output;
    }

    //哲学 辩证学 辩论
    private static boolean hasVerb(String[] acknowledgeSwap, int position, App NE) {
    ...
    private static boolean hasData2DSubject(String[] acknowledgeSwap, int position, App NE) {
        String string = acknowledgeSwap[position];
        List<String> list = NE.app_S._A.parserMixedString(string);
        Iterator<String> iterator = list.iterator();
        while (iterator.hasNext()) {
            String stringIterator = iterator.next();
            if (NE.app_S.workVerbaMap.data2DSubjectMap.containsKey(stringIterator)) {
                return true;
            }
        }
        return false;
    ...
    private static void doAcknowledgeSwap(String[] acknowledgeSwap, String command, App NE) {
        //null 将 授权 选择 进行 执行 数据 智慧 逻辑 操作 :null数据 矩阵 对象 表格 表
        boolean hasData2DSubjectBoolean = hasData2DSubject(acknowledgeSwap, 0, NE);
        ...
        boolean hasData2DObjectBoolean = hasData2DSubject(acknowledgeSwap, 2, NE);
