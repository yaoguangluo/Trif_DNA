    ...
    public static TinMap E_pl_XA(String plSearch, boolean mod, TinMap output, App NE) {
        ...
        for (String command : commands) {
            ...
            if (null != commandSwap) {
                ...
                doAcknowledgeSwap(acknowledgeSwap, command, NE);//later duplications
                //return null;
            }
            String[] temp = NE._I_U.acknowledge.clone();
            //*/
            Iterator<String> iterators = NE.app_S.workVerbaMap.cartesianWorkActionsRights.keySet().iterator();
            while (iterators.hasNext()) {
                String string = iterators.next();
                String[] strings = string.split("_");
                if (null != strings[0]) {
                    /*loop s later*/
                    FlowerAction.FlowerSixDomainActions.couldDoThenDo(strings[0], temp, output, NE);//later in pdn
                }
            }
            ...
            /*loop s later*/
            FlowerAction.FlowerSixDomainActions.couldDoThenDo(temp[0], temp, output, NE);//later in pdn
            //*/
