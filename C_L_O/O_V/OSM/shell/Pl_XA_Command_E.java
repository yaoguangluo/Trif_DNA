...
public class Pl_XA_Command_E {
	...
	private static void P_E_Kernel(TinMap object, boolean mod, App NE)
		...
		if (object.getCompareV("type", "进行选择") && (object
				.getCompareV("countJoins", "n")
				|| (object.getCompareV("countJoins", "1")
						&& !object.getCompareV("newCommand", "join")))) {
			...
			if (object.containsKey("PLETL")) {
				// object.put("obj",
				// SearchShellQ_Rows_E.selectRowsByAttributesOfPLETL(object));
			}
			object.remove("recordRows");
		}
		...
		if (object.getCompareV("type", "update")
			...
			if (object.containsKey(S_ShellETL.SHELL_ETL_CONDITION)) {
				...
				// , U_Rows_E.updateRowsByAttributesOfCondition(object,
				// mod));
			}
			if (object.containsKey("aggregation")) {
				// object.put("updateObj"
				// , U_Rows_E.updateRowsByAttributesOfAggregation(object,
				// mod));
			}
		...
		if (object.getCompareV("type", "update") && (object
				.getCompareV("countJoins", "n")
				|| (object.getCompareV("countJoins", "1")
						&& !object.getCompareV("newCommand", "join")))) {
			if (object.containsKey(S_ShellETL.SHELL_ETL_CONDITION)) {
				...
				// ,
				// U_JoinRows_E.updateRowsByAttributesOfJoinCondition(object,
				// mod));
			}
			if (object.containsKey("relation")) {
				// object.put("updateObj"
				// ,
				// U_JoinRows_E.updateRowsByAttributesOfJoinRelation(object,
				// mod));
			}
			if (object.containsKey("aggregation")) {
				// object.put("updateObj"
				// ,
				// U_JoinRows_E.updateRowsByAttributesOfJoinAggregation(object,
				// mod));
			}
			if (object.containsKey("culumnValue")) {
				// U_Rows_E.updateRowsByRecordConditions(object, mod);
	...
	public static void P_Check(String acknowledge, TinMap object, boolean mod,
			App NE) throws InterruptedException, IOException {
		...
		int rowBeginIndex = object.containsKey("pageBegin")
				? Integer.valueOf(object.get("pageBegin").toString())
				: 0;
		...
		object.put("tablePath",
				(null == object.get("获取表名") ? "" : object.get("获取表名"))
						.toString());
		...
		//// Base base=
		// DetaDBBufferCache_M.db.getBase(object.get("baseName").toString());
		...
		// }
		XA_ShellTable table = XA_ShellTables.searchShellTables
				.get((null == object.get("获取表名") ? "" : object.get("获取表名"))
						.toString());
		if (null != table) {
			Object[] specs = table.getHuaRuiJiJtableCulumns();
	...
	public static void P_StoreValues(String[] acknowledge, IMV_SIQ output) {
		...
		if (acknowledge[0].contains("设") && acknowledge[2].contains("为")) {
			S_ShellETL.storeValue.put(acknowledge[1].toString(),
					acknowledge[3].toString());
		}
	}
	...
	public static void P_fileOperations(String[] acknowledge, IMV_SIQ output) {
		...
		if (acknowledge[0].equals("确定文件夹")) {
			String filePath = (S_ShellETL.storeValue.containsKey(acknowledge[1])
					...
					: acknowledge[1]);
			S_ShellETL.storeValue.put("确定文件夹",
			...
			if (acknowledge[2].equals("替换为")) {
				...
				try {
					...
					conditions.put("replaceLogPath",
							S_ShellETL.storeValue.get("确定文件夹")
									+ "\\replace_Log.lyg");// \\只适用于windows, 记得
														   // / 比较。
	...
	public static void P_YLJOperation(String[] acknowledge, IMV_SIQ output,
		...
		if (null != searchWays) {
			if (searchWays.contains("DNN") || searchWays.contains("dnn")
					|| searchWays.contains("Dnn")
					|| searchWays.contains("dNN")) {
				NE.app_S.appConfig.SectionJPanel.jlabel_peizhi_di2122
						.setSelected(true);
			} else {
				NE.app_S.appConfig.SectionJPanel.jlabel_peizhi_di2122
						.setSelected(false);
			...
			if (searchWays.contains("专家")) {
				NE.app_S.appConfig.SectionJPanel.jlabel_peizhi_di2233
						.setSelected(true);
			} else {
				NE.app_S.appConfig.SectionJPanel.jlabel_peizhi_di2233
						.setSelected(false);
		...
		if (null != relatiedJingLuoFilter) {
			NE.app_S.name_filter_not_have
					.setText(relatiedJingLuoFilter.toString());
		} else {
