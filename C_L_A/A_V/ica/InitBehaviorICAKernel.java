...
public class InitBehaviorICAKernel {
	...
	public double getTrustRate(String text, App NE) {
		...
		double positiveCountEnvironment = emotionInitEnvironment
			.getPositiveCount();
		double totalCountEnvironment = emotionInitEnvironment
			.getTotalCount();
	...
	public double[] getBehaviorICAKernel(String text, App NE) {
		...
		double positiveCount = NE.app_S.emotionInit
			.getPositiveCount();
		double negativeCount = NE.app_S.emotionInit
			.getNegativeCount();
		...
		double phychologicRatio = (positiveCount + negativeCount)
			/ totalCount;
		...
		while (Iterator.hasNext()) {
			...
			if (null != NE.app_S.emotionSample.getDistinction()) {
				environmentText += NE.app_S.emotionSample
					.getDistinction() + S_Pos.SPACE_STRING;
			}
		}
		forRestReturn.add(environmentText);
		kernel[S_Pos.INT_THREE] = getTrustRate(environmentText, NE);
		forRestReturn.add(S_Pos.EMPTY_STRING
			+ kernel[S_Pos.INT_THREE]);
		...
		while (Iterator.hasNext()) {
			...
			if (null != NE.app_S.emotionSample.getMotivation()) {
				if (NE.app_S.emotionSample.getMotivation().contains(
					"语料库")) {
					continue;
				}
				motivationText += NE.app_S.emotionSample
					.getMotivation() + S_Pos.SPACE_STRING;
			}
		}
		forRestReturn.add(motivationText);
		kernel[S_Pos.INT_FOUR] = getTrustRate(motivationText, NE);//distincguide with old version later
		forRestReturn.add(S_Pos.EMPTY_STRING
			+ kernel[S_Pos.INT_FOUR]);
		...
		while (Iterator.hasNext()) {
			...
			if (null != NE.app_S.emotionSample.getTrending()) {
				if (NE.app_S.emotionSample.getTrending().contains(
					"语料库")) {
					continue;
				}
		...
		while (Iterator.hasNext()) {
			...
			if (null != NE.app_S.emotionSample.getPrediction()) {
				if (NE.app_S.emotionSample.getPrediction().contains(
					"语料库")) {
					continue;
				}
				predictionText += NE.app_S.emotionSample
					.getPrediction() + S_Pos.SPACE_STRING;
			}
		}
