...
public class RatioMap_E implements RatioMap {
    @Override
    public IMV_SIQ_X_ getEmotionSampleMap(IMV_SIQ_S_ wordFrequencyMap, IMV_SIQ positive
        , IMV_SIQ negative, App NE) {
        IMV_SIQ_X_ output = new IMV_SIQ_X_();
        for (int i = wordFrequencyMap.size() - S_Pos.INT_ONE; i >= S_Pos.INT_ZERO; i--) {
            if (wordFrequencyMap.getW(i).get_word().length() > S_Pos.INT_ONE) {
                ...
                if (output.containsKey(wordFrequencyMap.getW(i).get_word())) {
                    emotionSample = output.get_S(wordFrequencyMap.getW(i).get_word());
                } else {
                    emotionSample = new EmotionSample();
                }
                if (positive.containsKey(wordFrequencyMap.getW(i).get_word())) {
                    emotionSample.I_PositiveCount(wordFrequencyMap.getW(i).get_frequency());
                } else if (negative.containsKey(wordFrequencyMap.getW(i).get_word())) {
                    emotionSample.I_NegativeCount(wordFrequencyMap.getW(i).get_frequency());
                } else {
                    emotionSample.I_MedCount(wordFrequencyMap.getW(i).get_frequency());
                }
    ...
    @Override
    public void IQ_MotivationRatio(IMV_SIQ_X_ emotionSampleMap, double sumOfEmotion) {
    ...
    @Override
    public void IQ_CorrelationRatio(IMV_SIQ_X_ emotionSampleMap, double sumOfEmotion) {
        Iterator<String> Iterator = emotionSampleMap.keySet().iterator();
        while (Iterator.hasNext()) {
            String word = Iterator.next();
            EmotionSample emotionSample = emotionSampleMap.get_S(word);
    ...
    public void IQ_ContinusRatio(IMV_SIQ_X_ emotionSampleMap
        ...
        while (Iterator.hasNext()) {
            ...
            emotionSample.I_ContinusRatio((emotionSample.getPositiveCount()
                + emotionSample.getNegativeCount() + emotionSample.getMedCount())
                * emotionRatio);
            emotionSampleMap.put(word, emotionSample);
        }
    }
    ...
    public void IQ_Prediction(IMV_SIQ_X_ emotionSampleMap, IMV_SIQ prediction) {
        ...
        while (Iterator.hasNext()) {
            ...
            if (prediction.containsKey(emotionSample.getTrending())) {
                emotionSample.I_Prediction(prediction.get(emotionSample.getTrending())
                    .toString());
            } else if (prediction.containsKey(emotionSample.getMotivation())) {
                emotionSample.I_Prediction(prediction.get(emotionSample.getMotivation())
                    .toString());
            } else {
    ...
    @Override
    public void IQ_Distinction(IMV_SIQ_X_ emotionSampleMap, IMV_SIQ distinction) {
        Iterator<String> Iterator = emotionSampleMap.keySet().iterator();
        while (Iterator.hasNext()) {
            String word = Iterator.next();
            EmotionSample emotionSample = emotionSampleMap.get_S(word);
