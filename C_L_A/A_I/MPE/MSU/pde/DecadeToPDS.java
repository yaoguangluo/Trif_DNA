...
public class DecadeToPDS {
	...
	private int doPDS(DecadeToPDS decadeToPDS, int decade,
			double pDE_KEY_rate) {
		...
		String pDSInitons = decadeToPDS.PDSToInitons(pDS, pDE_KEY_rate,
				decadeToPDS);
		String pDSSeventeen = decadeToPDS.initonsToSeventeen(pDSInitons,
				decadeToPDS);
		int pDSDecade = decadeToPDS.seventeenToDecade(pDSSeventeen,
				decadeToPDS);
		return pDSDecade;
	}
	...
	public String decadeToSeventeen(int decade, DecadeToPDS decadeToPDS) {
		...
		while (0 < decad / 17) {
			...
			seventeen = decadeToPDS.initonsSet.get("" + seventeenth)
					+ seventeen;
			decad = decad / 17;
		}
	...
	public int seventeenToDecade(String seventeen, DecadeToPDS decadeToPDS) {
		...
		// A11 10*17*17 + 1*17 + 1
		for (int i = 0; i < seventeen.length(); i++) {
			int value = decadeToPDS.numberSet.get("" + seventeen.charAt(i));
	...
	public int[][] doPDSMatrix(DecadeToPDS decadeToPDS, int[][] rp,
			double facx) {
