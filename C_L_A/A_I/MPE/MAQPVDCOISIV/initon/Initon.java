...
//			}
	...
	// 举例V的 SWAP initon.store= "UQ"; -> initon.store= "U"; 和
	// NEXT initon.store= "Q";
	// 用于RNA 肽展遗传和增元计算.
	public Initon storeRNAExtension(Initon initonPDE) {
		...
		while (initonPDE != null) {
			...
			if (initonPDE.store.length() == 2) {
				...
				// 赋值
				initonFirst.store = S_Common.STRING_EMPTY
						+ initonPDE.store.charAt(0);
				initonSecond.store = S_Common.STRING_EMPTY
						+ initonPDE.store.charAt(1);
				// 连接
