...
import A_I.MPE.MAQPVDCOISIV.pde.PDE_Decrement_Formular;
import A_I.MPE.MAQPVDCOISIV.pde.PDE_Increment_Formular;
...
public class TokenPDI {
	...
	public void doKeyPress(String initons, TokenPDI pDE_RNA_Formular,
			boolean bYS) {
	...
	public static void main(String[] argv) {
		...
		pDE_RNA_Formular.pdw = pDE_RNA_Formular
				.initonSect(pDE_RNA_Formular.text);
		...
		// 20230106-S_logger.Log.logger.info("" + "开始Session解析： " +
		// pDE_RNA_Formular.session_key);
		// 20230106-S_logger.Log.logger.info("" + "开始概率钥匙解析：" +
		// pDE_RNA_Formular.pdedeKey
		// + pDE_RNA_Formular.pdedsKey+ pDE_RNA_Formular.pdeieKey
		// + pDE_RNA_Formular.pdeisKey);
		...
		// 20230106-S_logger.Log.logger.info("" + "观测：" +
		// pDE_RNA_Formular.code);
		pDE_RNA_Formular.doKeyUnPress(pDE_RNA_Formular.code, pDE_RNA_Formular1,
				true);
		// 20230106-S_logger.Log.logger.info("" + "得到原降元元基DNA序列："+
		// pDE_RNA_Formular.pds);
		// 20230106-S_logger.Log.logger.info("" + "得到新降元元基DNA序列："+
		// pDE_RNA_Formular1.pds);
		// 20230106-S_logger.Log.logger.info("" + "得到原元基DNA序列："+
		// pDE_RNA_Formular.pde);
		// 20230106-S_logger.Log.logger.info("" + "得到新元基DNA序列："+
		// pDE_RNA_Formular1.pde);
		// 20230106-S_logger.Log.logger.info("" + "验证正确？");
		// 20230106-S_logger.Log.logger.info("" + pDE_RNA_Formular.pde.equals(pDE_RNA_Formular1.pde)?
		// "正确": "失败");
		...
		// 20230106-S_logger.Log.logger.info("" + "准备计算元基DNA序列："+
		// pDE_RNA_Formular1.pde);
		pDE_RNA_Formular2.doSessionKeyUnPress(pDE_RNA_Formular1.pde,
				pDE_RNA_Formular2, true);
		// 20230106-S_logger.Log.logger.info("" + "得到原续降元元基DNA序列："+
		// pDE_RNA_Formular1.pds);
		// 20230106-S_logger.Log.logger.info("" + "得到后续降元元基DNA序列："+
		// pDE_RNA_Formular2.pds);
		// 20230106-S_logger.Log.logger.info("" + "验证正确？");
		// 20230106-S_logger.Log.logger.info("" + pDE_RNA_Formular1.pds.equals(pDE_RNA_Formular2.pds)?
		// "正确": "失败");
	}

	public void doSessionKeyUnPress(String initons, TokenPDI pDE_RNA_Formular2,
			boolean bYS) {
		Initon[] initon = new Initon[initons.length()];
		for (int i = 0; i < initons.length(); i++) {
			if (initon[i] == null) {
	...
	private void do_PDE_RNA_Formular_Back(Initon initon,
	...
	// 175-
	// Initon InitonPDE1S= doIncrementS(InitonPDE1,
	// initonLinkDNA, pDE_RNA_Formular, bYS);
	// Initon InitonPDE1C= doIncrementC(InitonPDE1S,
	// initonLinkDNA, pDE_RNA_Formular);
	// Initon InitonPDE1E= doIncrementE(InitonPDE1C,
	// initonLinkDNA, pDE_RNA_Formular, bYS);
	// Initon InitonPDE1V= doIncrementV(InitonPDE1E,
	// initonLinkDNA, pDE_RNA_Formular);
	// Initon InitonPDE1M= doIncrementM(InitonPDE1V,
	// initonLinkDNA, pDE_RNA_Formular);
	// Initon InitonPDE1P= doIncrementP(InitonPDE1M,
	// initonLinkDNA, pDE_RNA_Formular);
	// Initon InitonPDE1O= doIncrementO(InitonPDE1P,
	// initonLinkDNA, pDE_RNA_Formular);
	// Initon InitonPDE2= doIncrementA(InitonPDE1O,
	// initonLinkDNA, pDE_RNA_Formular);
	...
	public void doKeyUnPress(String initons, TokenPDI pDE_RNA_Formular,
			boolean bYS) {
		Initon[] initon = new Initon[initons.length()];
		for (int i = 0; i < initons.length(); i++) {
			if (initon[i] == null) {
	...
	public void do_PDE_RNA_Formular(Initon initon, TokenPDI pDE_RNA_Formular,
			boolean bYS) {
		Initon InitonPDE = initon;
		InitonLinkDNA initonLinkDNA = new InitonLinkDNA();
	...
	private Initon doIncrementE(Initon InitonPDE, InitonLinkDNA initonLinkDNA,
		...
		InitonPDE = new PDE_Increment_Formular()
				.PDE_IncrementE_IU(initonLinkDNA, pDE_RNA_Formular, bYS);
		while (InitonPDE.hasPrev()) {
			InitonPDE = InitonPDE.prev;
		}
		while (InitonPDE.hasNext()) {
	...
	private Initon doDecrementE(Initon InitonPDE, InitonLinkDNA initonLinkDNA,
		...
		InitonPDE = new PDE_Decrement_Formular()
				.PDE_DecrementE_IU(initonLinkDNA, pDE_RNA_Formular, bYS);
		while (InitonPDE.hasPrev()) {
			InitonPDE = InitonPDE.prev;
		}
		while (InitonPDE.hasNext()) {
