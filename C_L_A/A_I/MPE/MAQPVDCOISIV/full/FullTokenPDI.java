...
public class FullTokenPDI {
	...
	public void doKeyPress(String initons, FullTokenPDI pDE_RNA_FullFormular,
			boolean bYS) {
...
//		//20230106-S_logger.Log.logger.info("" + end-begin);
	}

	private void doSessionKeyUnPress(String initons,
			FullTokenPDI pDE_RNA_FullFormular2, boolean bYS) {
		Initon[] initon = new Initon[initons.length()];
		for (int i = 0; i < initons.length(); i++) {
			if (initon[i] == null) {
				initon[i] = new Initon();
	...
	private void do_PDE_RNA_FullFormular_Back(Initon initon,
			FullTokenPDI pDE_RNA_FullFormular, boolean bYS) {
...
//	}
	...
	private void doKeyUnPress(String initons, FullTokenPDI pDE_RNA_FullFormular,
			boolean bYS) {
		Initon[] initon = new Initon[initons.length()];
		for (int i = 0; i < initons.length(); i++) {
			if (initon[i] == null) {
...
/////INITONS SWAP	
	...
	private Initon doIncrementA(Initon InitonPDE, InitonLinkDNA initonLinkDNA,
			FullTokenPDI pDE_RNA_FullFormular) {
