...
import A_I.MPE.MAQPVDCOISIV.full.FullTokenPDI;
...
public class PDE_Increment_FullFormular {
	...
	// A= V + S LIST jdk util对象, 下面函数是直接用JDK的虚拟机函数 编写的, 逻辑比较清晰,
	// 各有各的用处.
	...
	// || initonLink.getStore().equalsIgnoreCase("D")) {
	// if(initonLink.hasNext()) {
	// Initon initonNext= initonLink.forwardNext();
	...
	public Initon PDE_IncrementE_DU(InitonLinkDNA initonLinkDNA,
			FullTokenPDI pDE_RNA_Formular, boolean bYS) {
		Initon initonLink = initonLinkDNA.getInitonLink();
		while (null != initonLink) {
			String initonString = initonLink.getStore();
	...
	// S= Q
	// 肽展公式的概率问题, 先这样命名, 之后讨论 是I, Q还是I + Q
	// public Initon PDE_IncrementS_Q(InitonLinkDNA
	// initonLinkDNA) {
	// Initon initonLink= initonLinkDNA.getInitonLink();
	// while(null!= initonLink) {
