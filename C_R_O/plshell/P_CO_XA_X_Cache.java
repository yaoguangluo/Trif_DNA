...
public class P_CO_XA_X_Cache extends P_CO_XA_X_ {
	public static void P_Cache(String[] sets, List<IMV_SIQ> output,
		 ...
		 * 如果是和就是上层结果进行跟进，
		 * 奇怪了，怎么好几年都输出正确？特别是输出正确，去掉激进的思绪，
		 * 不利于思考。
		 * 
		 * 罗瑶光
		 * */
...
//        }
		...
		if (sets[1].equalsIgnoreCase("精度搜索")) {
			...
			String key = (S_ShellETL.storeValue.containsKey(sets[2])
					? S_ShellETL.storeValue.get(sets[2])
					: sets[2]);
			...
			for (int i = 0; i < table.huaRuiJiJtableRows.length; i++) {
				...
				Here: while (iteratorWordFrequency.hasNext()) {
					...
					if (temps.contains(mapSearchaAtII)) {
						...
						reg[copyCount] += 1;
						score_code[copyCount] += (temps.contains(mapSearchaAtII)
								? 2
								: 1)
				...
				if (key.replace(" ", "").length() > 1
					...
					score_code[copyCount] = (int) (tempa
							/ Math.pow(Integer.parseInt(sets[3]) + 1, 4)
							+ tempb * Math.pow(Integer.parseInt(sets[3]), 2));
				}
				copyCount++;
			}
			...
			for (int i = score.length - 1; i > 0; i--) {
				...
				recordRows.put(Integer.valueOf(score[i].replace("i", "")),
						true);
			}
		...
		if (sets[1].equalsIgnoreCase("DNN搜索")) {
			...
			for (int i = 0; i < table.huaRuiJiJtableRows.length; i++) {
				String temps = table.huaRuiJiJtableRows[i].getCell(sets[0])
						.getCellValue().toString();
				// later empty check*/
				String textContent = table.huaRuiJiJtableRows[i]
						.getCell(sets[2]).getCellValue().toString();
				...
				Here: while (iteratorWordFrequency.hasNext()) {
					...
					if (dnnSet.containsKey(mapSearchaAtII)) {
						...
						// 比值分数PCA前10~25
						if (BonusPerRatio > 0.1
								- (Integer.parseInt(sets[4]) * 0.003)) {
							BonusPerRatio = 2.0;
						}
						// 总分比值的叠加, 只加第一次
				...
				if (key.length() > 4) {
					...
					double valuea = tempa
							/ Math.pow(Integer.parseInt(sets[4]) + 1, 4);
					double valueb = tempb
							* Math.pow(Integer.parseInt(sets[4]) + 1, 2);
		...
		for (int i = 0; i < rows.length; i++) {
			...
			if (sets[1].equalsIgnoreCase(S_ShellETL.SHELL_ETL_SMALL) || sets[1]
				...
				double rowCellFromBigDecimal = new BigDecimal(
						...
						.doubleValue();
				if (rowCellFromBigDecimal < new BigDecimal(
						(S_ShellETL.storeValue.containsKey(sets[2])
								? S_ShellETL.storeValue.get(sets[2])
								: sets[2]))
			...
			if (sets[1].equalsIgnoreCase(S_ShellETL.SHELL_ETL_CONTANS)) {
				String rowCellFromString = row.getCell(sets[0]).getCellValue()
						.toString();
				if (rowCellFromString
						.contains(S_ShellETL.storeValue.containsKey(sets[2])
								? S_ShellETL.storeValue.get(sets[2])
								: sets[2])) {
					if (!((Map<Integer, Boolean>) (object.get("recordRows")))
							.containsKey(i)) {
			...
			if (sets[1].equalsIgnoreCase(
					S_ShellETL.SHELL_ETL_STRING_LENGTH_GREATER_THAN)) {
				String rowCellFromString = row.getCell(sets[0]).getCellValue()
						.toString().trim();
				if (rowCellFromString.length() > new BigDecimal(
						(S_ShellETL.storeValue.containsKey(sets[2])
								? S_ShellETL.storeValue.get(sets[2])
								: sets[2]))
						.doubleValue()) {
			...
			// 字符串长度小于
			if (sets[1].equalsIgnoreCase(
					S_ShellETL.SHELL_ETL_STRING_LENGTH_LESS_THAN)) {
				String rowCellFromString = row.getCell(sets[0]).getCellValue()
						.toString().trim();
				if (rowCellFromString.length() < new BigDecimal(
						(S_ShellETL.storeValue.containsKey(sets[2])
			...
			if (sets[1].equalsIgnoreCase("过滤") || sets[1]
					.equalsIgnoreCase(S_ShellETL.SHELL_ETL_DOES_NOT_CONTANS)) {
				String rowCellFromString = row.getCell(sets[0]).getCellValue()
						.toString();
				if (!rowCellFromString
						.contains((S_ShellETL.storeValue.containsKey(sets[2])
			...
			if (sets[1].equalsIgnoreCase(S_ShellETL.SHELL_ETL_EQUAL)) {
				String rowCellFromString = row.getCell(sets[0]).getCellValue()
						.toString();
				if (rowCellFromString.equalsIgnoreCase(
						(S_ShellETL.storeValue.containsKey(sets[2])
								? S_ShellETL.storeValue.get(sets[2])
								: sets[2]))) {
					if (!((Map<Integer, Boolean>) (object.get("recordRows")))
 ...
 * 条件为:和:性味|不包含|寒; 条件为:和:性味|不包含|凉; getCulumnName:中药名称:打分;
 * 操作:0|行至|30; caoZuo:中药名称|颜色标记为|红色;
 * 
 * 获取表名:XLS1697259961711:进行选择; 条件为:和:用药参考|DNN搜索|用药参考|风寒咳嗽|1;
 * 获取类表名:病症&药名:打分; caoZuo:0|行至|30; caoZuo:病症&药名|颜色标记为|黄色;
 */
