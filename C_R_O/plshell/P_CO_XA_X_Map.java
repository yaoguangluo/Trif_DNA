	...
	public static void P_Map(String[] sets, List<IMV_SIQ> output,
		...
		if (output.isEmpty() && object.get("firstTime").equals("true")) {
			...
			for (int i = 0; i < output.size(); i++) {
				huaRuiJiJtableRows[i] = P_CO_XA_X_Map
						.rowMapToRow(output.get(i));
			}
			...
			for (int i = 0; i < table.huaRuiJiJtableRows.length; i++) {
				...
				Here: while (iteratorWordFrequency.hasNext()) {
					...
					if (temps.contains(mapSearchaAtII)) {
						...
						// score[copyCount]= temps;//因为 不再有map key, 所以就通用为map
						// 内容。, 还是需要map
		...
		for (int i = 0; i < table.huaRuiJiJtableRows.length; i++) {
			...
			if (sets[1].equalsIgnoreCase(S_ShellETL.SHELL_ETL_SMALL) || sets[1]
				...
				String rowCellFromString = row.getCell(sets[0]).getCellValue()
						.toString();
				// 大家看见没, rowvalue是 db的 Row单例,
				// 这里竟然是output的iterator。2019年被动手脚了。
				if (new BigDecimal(rowCellFromString)
			...
			if (sets[1].equalsIgnoreCase(S_ShellETL.SHELL_ETL_NOT_EUQAL_TO)
				...
				String rowCellFromString = row.getCell(sets[0]).getCellValue()
						.toString();
				if (new BigDecimal(rowCellFromString)
						.doubleValue() != new BigDecimal(sets[2])
								.doubleValue()) {
					outputTemp.add(P_CO_XA_X_Map.rowToRowMap(row));
				}
			}
			...
			if (sets[1].equalsIgnoreCase(S_ShellETL.SHELL_ETL_FILTER_BY)
					|| sets[1].equalsIgnoreCase(
							S_ShellETL.SHELL_ETL_DOES_NOT_CONTANS)) {
				String rowCellFromString = row.getCell(sets[0]).getCellValue()
						.toString();
				if (!rowCellFromString
						.contains(S_ShellETL.storeValue.containsKey(sets[2])
