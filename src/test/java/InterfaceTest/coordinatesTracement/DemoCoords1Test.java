 ...
 * */
/*
 * 这是一个 //处理 side end SideEnd_X_getSideEnd2D 测试main函数demo的test版本，
 * 在导入了api之后进行系统集成，然后用下面的对应的函数中源码逻辑进行复制粘贴到工程中，
 * 直接运行，即可出结果，源码的逻辑按照输入准备计算的参数，然后执行，然后获取输出需要的
 * 结果，可以用断点来查看数据，也可以用println来显示输出，方便集成，对程序员友好。系统
 * 需要jdk1.8 以上的java环境，本人会把测试的输入输出都注释在这个文件里。及其傻瓜化的流
 * 程，方便商业化落地。
 * --罗瑶光
 * */
...
public class DemoCoords1Test {
	...
	public static void main(String[] argv) {
		...
		//处理 side end
		List<AMV_MVS_VSQ_2D> output = SideEnd_X_getSideEnd2D._E(input,
			10);
	...
	public static void _E(List<AMV_MVS_VSQ_2D> inputs) {
		//处理 side end
		List<AMV_MVS_VSQ_2D> output = SideEnd_X_getSideEnd2D._E(
			inputs, 3);
		//.getSideEnd2D(input, 10);
		Iterator<AMV_MVS_VSQ_2D> iterator = output.iterator();
		while (iterator.hasNext()) {
	...
	private static void addNewPostion(List<AMV_MVS_VSQ_2D> inputs,
		double x, double y) {
	...
	public static void _E_(List<AMV_MVS_VSQ_2D> inputs) {
		...
		//处理 side end
		List<AMV_MVS_VSQ_2D> output = SideEnd_X_getSideEnd2D._E(
			inputs, 3);//.getSideEnd2D(input, 10);
		Iterator<AMV_MVS_VSQ_2D> iterator = output.iterator();
		while (iterator.hasNext()) {
			AMV_MVS_VSQ_2D temp = iterator.next();
			S_logger.Log.logger.info("" + temp.getX() + ":" + temp.getY());
