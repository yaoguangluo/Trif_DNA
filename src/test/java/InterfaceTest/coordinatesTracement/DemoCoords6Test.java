 ...
 * */
/*
 * 这是一个 处理fusion 先 fissile 采样 然后fusion 1 观测heart 
 * Euclid_X_findHeartPosition2D 测试main函数demo的test版本，在导入了api之后进行系统
 * 集成，然后用下面的对应的函数中源码逻辑进行复制粘贴到工程中，直接运行，即可出结果，源码的
 * 逻辑按照输入准备计算的参数，然后执行，然后获取输出需要的结果，可以用断点来查看数据，也可以
 * 用println来显示输出，方便集成，对程序员友好。系统需要jdk1.8 以上的java环境，本人会把测
 * 试的输入输出都注释在这个文件里。及其傻瓜化的流程，方便商业化落地。
 * --罗瑶光
 * */
	...
	public static void main(String[] argv) {
		...
		while (iterator2.hasNext()) {
			...
			AMV_MVS_VSQ_2D heartTemp = Euclid_X_findHeartPosition2D
				._E(output.get(temp));
			S_logger.Log.logger.info("" + "hearts->" + heartTemp.getX() + ":"
				+ heartTemp.getY());
			groupsHeart.put(temp, heartTemp);
		}
		//		//2观测结果 用10 和11分别带入可测试。
		//		Map<Double, List<AMV_MVS_VSQ_2D>> fusion
		//		= Fusion.fusionAMV_MVS_VSQ_2DwithHeart(output, groupsHeart, 11);
		...
		//		= PositionClasification.addNewPositionWithoutHeart(inputMap
...
//160
//		//2观测结果 用10 和11分别带入可测试。
//		Map<Double, List<AMV_MVS_VSQ_2D>> fusion= Fusion.fusionAMV_MVS_VSQ_2DwithHeart(
//		output, groupsHeart, 11);
//		Iterator<Double> iterator3= fusion.keySet().iterator();
//		while(iterator3.hasNext()) {
//			double temp= iterator3.next();
//			S_logger.Log.logger.info("" + "");
...
//		inputMap.put(0.0, input);
//		Map<Double, List<AMV_MVS_VSQ_2D>>output
//		= PositionClasification.addNewPositionWithoutHeart(inputMap, position2D5, 29);
// 采样距离28.4
//
//		Iterator<Double> iterator= output.keySet().iterator();
//		while(iterator.hasNext()) {
