...
class FullDNATokenPDITestTest {
	public static void main(String[] argv) {
		FullDNATokenPDITestTest fullDNATokenPDITestTest = new FullDNATokenPDITestTest();
		fullDNATokenPDITestTest.test();
	}

	/*
	 * 这个函数用于寻找不同酸碱浓度下可能存在的PDE变换，寻找最短的元基编码字符串浓缩结果。 适用
	 * 于肽计算中 元基编码后的 密码缩进，词汇提示，长篇语言总结，深度文本分析。 PDS-PDI很多人
	 * 不知道肽展公式PDE怎么广泛运用，我就多举几把测试DEMO例子先。
	 **/
	@Test
	void test() {
	...
	void main(String inputInitons) {
		// String inputInitons =
		// "AOAOPPPPPPMVMEEECSIIDUQDUQTXHFTXHFFFFFF";
	...
	private String duplication1(String inputInitons) {
		...
		Here: for (int i = 0; i < chars.length; i++) {
