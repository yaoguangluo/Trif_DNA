...
class LoadVerbalOutputMap3 {
    void exec(Map<String, String>[] verbal, String[] ss, String[] ss1) {
		...
		ss1[62] = "新闻-数据-香-*-*-*-学-将来-合-*-*-中-进行-招生-宣传-今年-在-皖-招-8-人-"
				+ "万-家-热线-安徽-第一-门户-";
		ss1[63] = "微博-数据-补-了-battle-赛-峰暴班-的-两个-弟弟-妹妹-@-杨宝心-@-修儿-一个是"
				+ "-我-很-挺-的-好-弟-弟-一个是-我-推荐-进-好-*-音-的-妹子-虽然-都-在-battle-阶段-"
				+ "都-下来-了-但是-我-依然-像-之前-那样-觉得-你们-非常-棒-"
				+ "";
		ss1[64] = "汽车-论坛-舒适-性-胎噪-风噪-偏大-硬-过-坎-弹跳-明显-";
		ss1[65] = "餐饮-点-评-根-闺-蜜-在-西单-逛-街-想-吃-寿司-了-在-西单-没-搜-到-其他-的-"
				+ "日料-店-就-来-了-禾绿-了-我们-俩-都-觉-得-没-以前-好了-";
		/*
		开始思考这句，上将前是奥运会，名词前是可以接名词的逻辑。如果改成 军*类，那么就成了 在-伦敦军事类上将
		可能有一位沙**伯的女子，歧义语句语义就是正确的表达逻辑，说明分词逻辑-上将-是正确的，但语义上是错误的。
		错误的原因是主题环境和名词，所以解决这类问题有2种方案
		1 去掉上将中将少将等 介量度+名组合的语料词库 复合名词，将上将拆开输出，之后用单字名词组合map去遍历
		这些输出了单字来包装。
		举个例子，分词输出是 在-伦-敦-奥运会-上-将-可-能-有-一位-沙-特-阿拉伯-的-女子- 然后通过奥运会得到
		分词国际环境标识，得到当前国际国家列表map去匹配 输出的单字sets，可以得到伦敦成为一个词，沙* 成为一个词，
		最终输出为在-伦*-奥运会-上-将-可能-有-一位-沙*-阿*伯-的-女子-，
		同样的问题，奥运会改为军委后，通过军委关键字得到分词军事环境标识，通过当前军事词汇列表map去匹配输出的
		单字sets，可以的得到 上将成为一个词。最终输出为在-伦*-军*-上将-可能-有-一位-沙*-*伯-的-女子-，
		这种环境类名词定制map在商业计算属于 输出校正逻辑，而不是 分词中枢逻辑。
		
		2 加入语义环境思维逻辑，思考奥运会是体育名词，上将是军事名词，后者名词与前者名词没有所属，同行，因果，
		关联，混淆等强逻辑关系，应该拆开进行前序匹配 上将 之后的词汇进行分析是否应拆开这个名词。这种逻辑模拟了人
		的辩证思维，是一个复杂的笛卡尔关系分解，耗费大量时间。但是分词会非常准确。
		
		综上所述，当前最快速最有效的区分组合类名词的方式是分词后通过关键词得到环境标识，通过环境标识得到该环境类的
		所有定制组合名词map，然后将单字连续2个以上的进行遍历是否需要组合包装输出。
		
		所以面对这类问题，我的结论是，我的65000词汇表会将所有含有介词 度 量 组合类的名词进行全部删除，比如 会上
		会下，中将，上将 第48526行，这类全部剔除出词汇表。，之后设计定制map将分词后的词汇 进行包装再输出。
		中将，上将并入军事环境定制map， 伦敦 阿拉伯 沙特并入国家环境定制map，会上 会下 并入 会议环境 定制map。
		
		另外我也得到一个结论，当前的一些分词机构的采样分词测试排名的数据其实没有实际价值。只要这个定制map集
		不完整，排名的极限永远趋近于无效。这些群体有这个功夫花钱做数据排个不停，不如花点时间进行详细的将当前不断地出现的新兴
		各类名词词语分类提供开源的map下载，我罗瑶光正需要这类map，不说别人，至少我罗瑶光一定会说声谢谢你们。
		
		文中 * 省略 代表作者的作品中，不想扯到这类数据，只是业界的分词测评非要扯这类数据 没办法。
...
分词环境-总分词正确比率 含有当前流行的浏览器AI自动生成的随机长文摘录分词
分词环境-总分词正确比率 属于全部去标点符号的歧义短文分词比率
分词环境-总分词正确比率 不适用太多单字词的古文，古文建议用分句api，不应该用分词api。
