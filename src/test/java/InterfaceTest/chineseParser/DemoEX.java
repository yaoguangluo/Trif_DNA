...
public class DemoEX {
	...
	/*
	 * 思考 20251007 今天在反复地分析测试输出结果，发现因为2018年10月中 2018和10已经提取了数字，
	 * 那么年月因为词性相同和单字出现满足组成双字词汇的条件，导致组合成了-年月-词汇，我开始更近思考。
	 * 这里在切词后组词逻辑中不应该合并，同时在笛卡尔关系中合并后需要保留单字。间接论证了，昨天的保留
	 * 单字在noun map和verb map中的逻辑属于必要的逻辑。于是开始优化，
	 * 途径-1-提取了数字，用一个空格替换，避免了组词。--操作快，
	 * 途径-2-提取了数字，用一个替代符号替换，避免了组词。同时将数字提取用list代替，这样可以用
	 * 变量来创造条件还原到分词list中。--质量高，影响很多场景的分词速度
	 * 
	 * 我采用途径-2的逻辑 尝试下。先构造一个_IMV_SQI_SS_Q_ list 变量，用来平衡_IMV_SQI_SS_Q map
	 * 如果条件成熟再剔除 _IMV_SQI_SS_Q 逻辑即可。
	 * 
	 * 于是用 filterString 中 加特殊符号 来标识数字 ,ok 稍后就提取出来作为input变量来全局操作。
	 * 看了下结果算是OK，但是又遇到了新的问题，因为数字的map 在之后会进行map的combination，那么
	 * 这里会出现伪指令。于是检查下，将WorkVerbalMap函数中的setHumanTalkAfterNewBusinessTest
	 * 的末尾先注释掉。
	 * 
	 * 思考--出现 +1+直+0+纠纷+ 这里 -1直- 因为是笛卡尔关系加上RNN的距离位置权值精确分析模式，
	 * 不会影响指令句识别。
	 * 
