...
class EffectionDemoTest {
	/*
	 * 著作权算法的动态用法
	 * 探索自然语言写作质量痕迹归纳。例子是名词思维在写作中的轨迹变化的切裂方式，用于探索大脑
	 * 的思绪记忆变化频率。--分词后输出格式化按行长增维进行词性词团--先-3.4 分词归纳-再通过
	 * -1.3 scale 距离来进行坐标团切裂。--该测试DEMO用于公文申请书类优化，适用于高等院校
	 * 进修材料，留学申请，写作优化，--写作习惯，写作特征分析等办公学习场景，适用于教育和传媒
	 * 宣传领域。--适用刑侦-文本加密和特征索引。匿名作家追踪分析。在导入了api之后进行系统集成，
	 * 然后用下面的对应的函数中源码逻辑进行复制粘贴到工程中，直接运行，即可出结果，源码的逻辑
	 * 按照输入准备计算的参数，然后执行，然后获取输出需要的结果，可以用断点来查看数据，也可以用
	 * println来显示输出，方便集成，对程序员友好。系统需要jdk1.8 以上的java环境，本人会把
	 * 测试的输入输出都注释在这个文件里。及其傻瓜化的流程，方便商业化落地。作为一个世界级科学家，
	 * 就不能思维总定固一个区域的层面，眼光思考的是整个体系的脉络，挑战的是整个环境变化。
	 * --罗瑶光
	 */

	public static void main(String[] argv) {
		EffectionDemoTest effectionDemoTest = new EffectionDemoTest();
		effectionDemoTest.main();
	}
	...
	void main() {
		...
		String inputString = "这个API主要应用于数据智能工程用来处理各种形式的海量数据类工程"
			...
			+ "近资本家产业投资者2018年后我一直在湖南省浏阳市集里大塘路1段7年大家要找我投资辩论"
			+ "直接来即可来前记得打我短信电话我有齐全的个人著作权身份证和法人银行账号等各类权属"
			+ "各类合作可以直接租赁方式交易有时间我文档pdf化农工商具体商业逻辑中数据内容操作服"
			+ "务实例DNA十六元基编码索引计算的科普推广版本数据智能工程生物信息学计算机应用科学"
			+ "高年级专业基础教学中真实具体的功能实验版本推动社会商业产业真实落地通过创造新的生"
			+ "产工具来提高生产力和改变生产环境关系一开始我是基于医学数据搜索来设计的我一个著作"
			+ "权产品养疗经api，国内因为严谨性，医学产业被一些部门和证件卡的严实我寸步难行又被"
			+ "迫开源于是利用这严实我把养疗经api中公共接口和逻辑全部剥离出来去掉医学组件成为"
			...
			+ "函数和100+个功能DEMO在面对某具体行业已经体现了许多基础实用价值但面向整个社会各"
			+ "行各业我会写更多的测试DEMO补充小目标26000+函数和1000+个功能DEMO扩大鲁棒性"
