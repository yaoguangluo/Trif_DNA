...
class EffectionDemoTest {
	/*
	 * 著作权算法的动态用法
	 * 探索自然语言写作质量痕迹归纳。例子是名词思维在写作中的轨迹变化的切裂方式，用于探索大脑的思绪记忆
	 * 变化频率。--分词后输出格式化按行长增维进行词性词团--先-3.4 分词归纳-再通过-1.3 scale
	 * 距离来进行
	 * 坐标团切裂。--该测试DEMO用于公文申请书类优化，适用于高等院校进修材料，留学申请，写作优化，--写作
	 * 习惯，写作特征分析等办公学习场景，适用于教育和传媒宣传领域。--适用刑侦-文本加密和特征索引。匿名作家
	 * 追踪分析。在导入了api之后进行系统集成，然后用下面的对应的函数中源码逻辑进行复制粘贴到工程中，直接
	 * 运行，即可出结果，源码的逻辑按照输入准备计算的参数，然后执行，然后获取输出需要的结果，可以用断点来
	 * 查看数据，也可以用println来显示输出，方便集成，对程序员友好。系统需要jdk1.8 以上的java环境，本人
	 * 会把测试的输入输出都注释在这个文件里。及其傻瓜化的流程，方便商业化落地。作为一个世界级科学家，就不
	 * 能思维总定固一个区域的层面，眼光思考的是整个体系的脉络，挑战的是整个环境变化。--罗瑶光
	 */
	...
	void main() {
		...
		String inputString = "这个API主要应用于数据智能工程用来处理各种形式的海量数据类工程计算问题"
				...
				+ "养疗经api，国内因为严谨性，医学产业被一些部门和证件卡的严实我寸步难行又被迫开源于是利用这严"
				+ "实我把养疗经api中公共接口和逻辑全部剥离出来去掉医学组件成为华瑞集API发现用途巨大适用面更广"
				+ "很多产业逻辑底层是相同相通的于是将开源编码进行元基编码化一边测试一边开源一边论证一边优化于是"
				+ "形成了这个DNA时微分编码体系先把华瑞集Rapi的所有可test文件进行综合功能测试归纳统一打包SRC"
				+ "下面的test/java/InterfaceTest区通过一种在真实的环境下用极简源码详细描述直观逻辑具体对象"
				+ "并含有输入输出参数实例的测试DEMO进行丰富地论证API的商业前景和使用价值当前例子有2600+函数"
			...
			for (int j = 0; j < wordString[0].length; j++) {
				if (null != wordString[i][j]) {
					...
					if (wordObjectPOS.toString().contains("名")) {
						System.out.println("有效名词-" + wordString[i][j] + "-坐标-"
								+ i + ":" + j);
