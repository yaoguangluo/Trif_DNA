...
*  208号 阳光家园别墅小区 第十栋别墅
 * */
public class Classification_X_addNewPositionWithHeart {
    public static Map<Double, List<AMV_MVS_VSQ_2D>> _E(Map<Double
        , List<AMV_MVS_VSQ_2D>> groups, AMV_MVS_VSQ_2D position2D, Map<Double
        , AMV_MVS_VSQ_2D> hearts, double scaleDistance) {
        double groupKey = 0.0;
        for (double aDouble : groups.keySet()) {
            ...
            if (scaleDistance < distance) {
                ...
                //hearts熵增
                AMV_MVS_VSQ_2D CryptHeart = Euclid_X_findCryptionPosition2D._E(heart, position2D);
