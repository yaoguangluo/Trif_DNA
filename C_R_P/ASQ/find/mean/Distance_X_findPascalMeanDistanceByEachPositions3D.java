 ...
 * */
public class Distance_X_findPascalMeanDistanceByEachPositions3D {
	...
	// 实现：罗瑶光
	// 3D坐标团中 每一个坐标的斥力预测算法。
	public static double[] _E(List<AMV_MVS_VSQ_3D> position3Ds,
		...
		while (outPosition3DIterator.hasNext()) {
			...
			int i = 0;
			// 取坐标点所有距离集合先
			for (AMV_MVS_VSQ_3D inPosition3D : position3Ds) {
				distance[i++] = Distance_X_getDistance3D._E(outPosition3D,
						inPosition3D);
			}
			// 距离非对称缺陷小高峰过滤极速快排 从小到大
			// distance= new LYG4DWithDoubleQuickSort4D().sort(distance,
			// sortRangeScale);
			distance = new LYG9DWithDoubleTopSort4D().sort(distance,
					sortRangeScale, 4);// 算法应用更新
			// 仅仅取精度内坐标点距离求平均值, 0 为本身所以从 1 开始
			double inMean = 0.0;
			observerPCAScale = observerPCAScale >= position3Ds.size()
					? position3Ds.size() - 1
					: observerPCAScale;
