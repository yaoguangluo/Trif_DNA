        ...
        while (outLoop.hasNext()) {
            ...
            for (double in : groupsHeart.keySet()) {
                ...
                AMV_MVS_VSQ_2D outHeart = outputHeart.containsKey(out)
                    ? outputHeart.get_AMV_MVS_VSQ_2D(out) : groupsHeart.get(out);
                double distance = Distance_X_getDistance2D._E(inHeart, outHeart);
                //比较 是融合
                if (distance < scale) {
                    List<AMV_MVS_VSQ_2D> outList;
                    //比较有融媒
                    if (output.containsKey(out)) {
                        outList = output.get_List_AMV_MVS_VSQ_2D(out);
                    } else {//比较无融媒
                    ...
                    //更新heart
                    AMV_MVS_VSQ_2D newHeart = Euclid_X_findCryptionPosition2D._E(outHeart, inHeart);
