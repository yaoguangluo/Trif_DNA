...
public class FindHeart_X_getPosition2DGroupsHearts {
    //求坐标团的重心。
    public static IMV_SQI _E(Map<Double, ArrayList<AMV_MVS_VSQ_2D>> groups) {
        IMV_SQI output = new IMV_SQI();
        for (double value : (Iterable<Double>) output.keySet()) {
            output.put(value, Euclid_X_findHeartPosition2D._E(groups.get(value)));
        }
        return output;
    }
}
