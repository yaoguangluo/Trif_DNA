...
//		}
		...
		if (string.equalsIgnoreCase("/login")) {
			return VtoV.ObjectToJsonString(RestLoginPort_E.login(
					data.getString("uEmail"), data.getString("uPassword")));
		}
		if (string.equalsIgnoreCase("/find")) {
			return VtoV.ObjectToJsonString(
					RestLoginPort_E.find(data.getString("uEmail")));
		}
		if (string.equalsIgnoreCase("/logout")) {
			return VtoV.ObjectToJsonString(RestLoginPort_E.logout(
					data.getString("uEmail"), data.getString("uToken")));
		}
...
//		}
		if (string.equalsIgnoreCase("/register")) {
			return VtoV.ObjectToJsonString(RestLoginPort_E.register(
					data.getString("uEmail"), data.getString("uName"),
					data.getString("uPassword")));
		}
		if (string.equalsIgnoreCase("/change")) {
			return VtoV.ObjectToJsonString(RestLoginPort_E.change(
					data.getString("uEmail"), data.getString("uChange"),
					data.getString("uChangeEnsure"), data.getString("uToken"),
					data.getString("uPassword")));
		}
		// if(string.equalsIgnoreCase("/checkStatus")){
		// return
		// VtoV.ObjectToJsonString(RestLoginPortImpl.checkStatus(data.getString("token")));
		// }
		if (string.equalsIgnoreCase("/ask")) {
			return VtoV.ObjectToJsonString(RestAskPort_E.ask(
					data.getString("ip"), data.getString("token"),
					data.getString("message"), data.getString("pointIp")));
		}
		if (string.equalsIgnoreCase("/feedBack")) {
			return VtoV.ObjectToJsonString(RestAskPort_E.feedBack(
					data.getString("ip"), data.getString("token"),
