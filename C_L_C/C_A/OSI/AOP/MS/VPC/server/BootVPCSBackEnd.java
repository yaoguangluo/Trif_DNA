...
public class BootVPCSBackEnd extends Thread {
    ...
    public void IV_(App NE) {//代入逻辑必定都是E实体。不确定的具体引用加Q
        try {
            ...
            //20230106-System.out.println("----德塔VPCS后端服务器端口启动:" + port);
            //MrTin.born.start();;
    ...
    public void bootBackEnd(SocketThreadPool socketThreadPool, App NE) {
        ...
        //20230106-System.out.println("----DETA VPCS BackEnd--2.0");
        //20230106-System.out.println("----Author: 罗瑶光");
        //20230106-System.out.println("----浏阳德塔软件开发有限公司开源项目");
        //20230106-System.out.println("----德塔VPCS后端服务器----");
        //20230106-System.out.println("----德塔VPCS后端服务器启动一切正常-总耗时:"+ (now- before)+ "毫秒");
        while (true) {
            try {
                if (socketThreadPool.getThreadsCount() < 300) {
                    SocketThread clientSocket = new SocketThread(socketThreadPool
                        , server.accept(), System.currentTimeMillis()
                        + "" + new Random().nextLong(), NE);
    ...
    public static void main(String[] args, EnvironmentInit environmentInit
        ...
        , EnvironmentInit environmentInit_QE, SocketThreadPool socketThreadPool
        , NERO_C_OneTime_E nERO_C_OneTime_E, Nlp_CE_X_S nlp_CE_X_S, Pos_X_P pos_X_P
        , SensingTest sensingTest, FMHMMListOneTime_E fMHMMListOneTime_E
