...
public class RequestRecord_C {
    ...
    public static void requestIpRecoder(VPCSRequest vPCSRequest
        ...
        vPCSRequest.I_RequestIp(vPCSResponse.getSocket()
            .getInetAddress().getHostAddress());
        vPCSRequest.I_RequestName(vPCSResponse.getSocket()
    ...
    public static void requestLinkRecoder(VPCSRequest vPCSRequest
        ...
        try {
            br = new DetaBufferedReader(DetaInputStreamReader.E(
                vPCSResponse.getSocket().getInputStream(), S_Web.CHARSET_UTF8));
        } catch (IOException e) {
        ...
        String[] content = type[S_Pos.INT_ONE].split(S_Web.STRING_SLASH_QUESTION);
        ...
        if (vPCSRequest.getRequestIsRest()) {
            String[] column = content[S_Pos.INT_ONE].split(S_Web.STRING_JUNCTION);
            IMV_SIQ data = new IMV_SIQ();
            for (String cell : column) {
                String[] cells = cell.split(S_Web.MATH_EQUAL);
                try {
                    data.put(cells[S_Pos.INT_ZERO]
                        , URLDecoder.decode(cells[S_Pos.INT_ONE], S_Web.CHARSET_UTF_8));
        ...
        vPCSRequest.I_RequestLink(content[S_Pos.INT_ZERO]);
    }
}
