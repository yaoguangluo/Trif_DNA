...
public class DatabaseLog_H {
	...
	private static void IV_LogCategoryPath() {
		if (null != Cache_M.getCacheInfo("DBPath")) {
			logCategoryPath = Cache_M.getCacheInfo("DBPath").getValue()
					.toString();
		} else {
			try {
				throw new Exception();
	...
	public static void writeLogFile(long when, String who, String plsql) {
		Q_CurrentFileRange();
		// zip text string and write; im thinking about a new method
		// to make string write small.
		...
		try {
			...
			fw.write("\n\r\n");
			fw.write(new String(
					GzipUtil.compress(logString.getBytes(S_Web.CHARSET_UTF8)),
					S_Web.CHARSET_UTF8));
			// fw.write(logString);
			fw.close();
		} catch (IOException e) {
	...
	private static void Q_CurrentFileRange() {
		...
		// if currentfilesize > 100mb will make discussion later,
		// now just make one file per day.
		// makenew file;
	}
	...
	private static void coverageByTime(long time) {
		...
		reader = new DetaBufferedReader(
				DetaFileReader.E("C:/DetaLog/log/logger.det"));
		String tempString;
		while ((tempString = reader.readDetaLine()) != null) {
			...
			try {
				tempString = new String(
						GzipUtil.uncompress(
								tempString.getBytes(S_Web.CHARSET_UTF8)),
						S_Web.CHARSET_UTF8);
