...
public class AVItoLYGNodeInterface extends OSU_MCI_MCU_MSI{	
	public AVItoLYGNodeInterface() {
		thisIcon= FastReadProjectFile.getDetaImageIconReader("imageread.jpg", "DetaResources/imgETL/");
		_SQ_OSU_MSQ_OSU_AVQ_ASQ_SQ_VPC_PCS=new String("AVItoLYG");
	...
	public void MEI_MSU(JTextPane jTextPane, ThisCanvas canvas) {
		_SQ__OVQ_OSQ_VSQ=new AVItoLYGOSU_MSQ_AVQ_ASQ_OVQ_OSQ_VSQ();
		_SQ__OPE_OPC_ECI=new AVItoLYGOSU_MSQ_OPE_OPC_ECI();
		_SQ__AOU_AOQ_VES
		= new AVItoLYGOSU_MSQ_AOI_AOD_AOU_AOQ_VES((AVItoLYGOSU_MSQ_OPE_OPC_ECI) _SQ__OPE_OPC_ECI);
		_SQ__AOU_AOQ_VES.config();
		showed=false;
	}
	public void OPE_E(JTextPane jTextPane) {
//		((AVItoLYGOSU_MSQ_OPE_OPC_ECI) _SQ__OPE_OPC_ECI)
//		.run((AVItoLYGOSU_MSQ_AVQ_ASQ_OVQ_OSQ_VSQ) _SQ__OVQ_OSQ_VSQ);
	}
