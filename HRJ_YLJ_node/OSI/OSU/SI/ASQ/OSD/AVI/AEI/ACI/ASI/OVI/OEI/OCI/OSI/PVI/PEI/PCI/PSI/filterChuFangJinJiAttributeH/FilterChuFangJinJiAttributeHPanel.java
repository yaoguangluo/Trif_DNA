...
public class FilterChuFangJinJiAttributeHPanel extends OSU_AOU_AOQ_VES{
	
	private JTextPane textTemp;
	private Object[][] tableData_old;
	public FilterChuFangJinJiAttributeHPanel(final FilterChuFangJinJiAttributeHRun _SQ__OPE_OPC_ECI
		, JTextPane text
						...
						for(int i= 0; i < rc; i++){
							for(int j= 0; j < rc; j++){
								if(_SQ__OPE_OPC_ECI.toptablein.getValueAt(j, 3).toString()
										.contains(_SQ__OPE_OPC_ECI.toptablein.getValueAt(i, 0).toString())) {
									continue Here;
								}
							}
