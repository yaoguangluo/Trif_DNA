...
public class FilterChuFangXingWeiKeyWordsAttributeHPanel extends OSU_AOU_AOQ_VES{

	private JTextPane textTemp;
	private Object[][] tableData_old;
	public FilterChuFangXingWeiKeyWordsAttributeHPanel(final FilterChuFangXingWeiKeyWordsAttributeHRun _SQ__OPE_OPC_ECI, JTextPane text
					...
					Here:
						for(int i= 0; i < rc; i++){
							for(int j= 0; j < filterw.length();j++) {
								if(_SQ__OPE_OPC_ECI.toptablein.getValueAt(i, 4).toString().contains(""+filterw.charAt(j))) {
									continue Here;
								}
							}
