...
public class FFTFilterNodeInterface extends OSU_MCI_MCU_MSI{
	public FFTFilterNodeInterface() {
		thisIcon= FastReadProjectFile.getDetaImageIconReader("imageread.jpg", "DetaResources/imgETL/");
		_SQ_OSU_MSQ_OSU_AVQ_ASQ_SQ_VPC_PCS=new String("FFT Filter");
	...
	public void MEI_MSU(JTextPane jTextPane, ThisCanvas canvas) {
		_SQ__OVQ_OSQ_VSQ= new FFTFilterOSU_MSQ_AVQ_ASQ_OVQ_OSQ_VSQ();
		_SQ__OPE_OPC_ECI= new FFTFilterOSU_MSQ_OPE_OPC_ECI();
		_SQ__AOU_AOQ_VES
		=new FFTFilterOSU_MSQ_AOI_AOD_AOU_AOQ_VES((FFTFilterOSU_MSQ_OPE_OPC_ECI) _SQ__OPE_OPC_ECI);
		_SQ__AOU_AOQ_VES.config();
		showed=false;
	}
	public void OPE_E(JTextPane jTextPane)  {
		((FFTFilterOSU_MSQ_OPE_OPC_ECI) _SQ__OPE_OPC_ECI)
		.run((FFTFilterOSU_MSQ_AVQ_ASQ_OVQ_OSQ_VSQ) _SQ__OVQ_OSQ_VSQ);
	}
	...
	public OSU_MCI_MCU_MSI AOP_MVE_CSI_DUQ()  {  
	        _SQ_OSU_MSQ_OSU_MCI_MCU_MSI= new FFTFilterNodeInterface();   
