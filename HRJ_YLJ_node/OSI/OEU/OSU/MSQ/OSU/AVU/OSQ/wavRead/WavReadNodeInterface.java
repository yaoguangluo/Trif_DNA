...
public class WavReadNodeInterface extends OSU_MCI_MCU_MSI{
	public WavReadNodeInterface() {
		thisIcon= FastReadProjectFile.getDetaImageIconReader("imageread.jpg", "DetaResources/imgETL/");
		_SQ_OSU_MSQ_OSU_AVQ_ASQ_SQ_VPC_PCS=new String("WAVRead");
	...
	public void MEI_MSU(JTextPane jTextPane, ThisCanvas canvas) {
		_SQ__OVQ_OSQ_VSQ=new WavReadOSU_MSQ_AVQ_ASQ_OVQ_OSQ_VSQ();
		_SQ__OPE_OPC_ECI=new WavReadOSU_MSQ_OPE_OPC_ECI();
		_SQ__AOU_AOQ_VES
		=new WavReadOSU_MSQ_AOI_AOD_AOU_AOQ_VES((WavReadOSU_MSQ_OPE_OPC_ECI) _SQ__OPE_OPC_ECI);
		_SQ__AOU_AOQ_VES.config();
		showed=false;
	}
	public void OPE_E(JTextPane jTextPane)  {
		((WavReadOSU_MSQ_OPE_OPC_ECI) _SQ__OPE_OPC_ECI)
		.run((WavReadOSU_MSQ_AVQ_ASQ_OVQ_OSQ_VSQ) _SQ__OVQ_OSQ_VSQ);
	}
	...
	public OSU_MCI_MCU_MSI AOP_MVE_CSI_DUQ()  {  
	        _SQ_OSU_MSQ_OSU_MCI_MCU_MSI= new WavReadNodeInterface();   
