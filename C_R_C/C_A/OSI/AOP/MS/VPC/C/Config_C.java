...
import DSU.V.VtoV;
import E_A.OP.SM.AOP.MEC.SIQ.VPC.PP.port.E.RestDBConfig_E;
...
public class Config_C {
    public static String exec(String string, Map<String, String> data) {
        if (string.equalsIgnoreCase(S_Web.REST_PATH_SET_DB_PATH)) {
            return VtoV.ObjectToJsonString(RestDBConfig_E.setDBPath(data
                .get("baseName"), data.get("baseName"), data.get("baseName")));
        }
        if (string.equalsIgnoreCase(S_Web.REST_PATH_SET_DB_TABLE)) {
            return VtoV.ObjectToJsonString(RestDBConfig_E.setDBTable(data
                .get("tableName"), data.get("token"), data.get("auth")));
        }
        return "";
    }
}
