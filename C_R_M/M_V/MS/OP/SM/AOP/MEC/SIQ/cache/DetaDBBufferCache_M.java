...
public class DetaDBBufferCache_M {
	...
	private static void loopTables(Base base, String dBasePath,
			String tableName) {
		...
		if (fileSpecPath.isDirectory()) {
			...
			for (String s : specs) {
				...
				DetaBufferedReader reader = new DetaBufferedReader(
						DetaFileReader.E(specCulumnPath + "/" + "value.lyg"));
				String temp = "";
				String tempString;
				...
				if (!culumn.contains("is_delete")) {
					...
					try {
						reader = new DetaBufferedReader(
								DetaFileReader.E(rowCulumnValuePath));
					} catch (Exception e) {
						e.printStackTrace();
					}
					String temp = "";
					...
					while (true) {
						assert reader != null;
						if ((tempString = reader.readDetaLine()) == null)
							break;
						temp += tempString;
					}
					reader.closeDeta();
