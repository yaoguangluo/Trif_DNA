				...
				if (fileDBTableRowsPath.isDirectory()) {
					...
					while (i < 15) {
						String DBTableRowIndexPath = DBTableRowsPath + "/row"
								+ (direction.contains("next") ? rowEndIndex++
										: --rowBeginIndex);
						...
						File deleteTest = new File(
								DBTableRowIndexPath + "/is_delete_1");
						...
						for (String readDBTableRowCulumnIndexFile : readDBTableRowCulumnsIndexFile) {
							...
							if (readDBTableRowIndexCulumnFile.exists()) {
								String temp = "";
								FileInputStream fis = DetaFileInputStream
										.E(new File(DBTableRowIndexCulumnPath
												+ "/value.lyg"));
								...
								fis.close();
								// bis.close();
								culumnMap.put(S_ShellETL.SHELL_ETL_CULUMNNAME,
										readDBTableRowCulumnIndexFile);
