...
public class Q_JoinRows_E_X_selectRowsByAttributesOfJoinCondition {
    public static Object _E(IMV_SIQ object) {
        ...
        String DBPath = Objects.requireNonNull(Cache_M.getCacheInfo("DBPath"))
					.getValue().toString() + "/" + object.get(S_ShellETL.SHELL_ETL_JOIN_BASE_NAME).toString();
        //锁定表
                        ...
                        for (int i = 2; i < conditionValue.length; i++) {
                            ...
                            if (overMap && andMap) {
                                ...
                                X_Cache.P_Cache(sets, output
                                    ...
                                    , object.get(S_ShellETL.SHELL_ETL_JOIN_BASE_NAME)
																		.toString(), object);//1
                            } else {
                                X_Table.P_Table(sets, output
                                    , DBTablePath, object);//1
                            }
                        }
                    }
                }
