...
public class Q_Rows_E_X_selectRowsByAttribute {
		...
		if (fileDBPath.isDirectory()) {
			...
			if (fileDBTable.isDirectory()) {
				...
				if (fileDBTableCulumn.isDirectory()) {
					...
					File readDBTableSpecCulumnFile = new File(
							DBTableCulumnPath + "/" + fileList[0]);
					DetaBufferedReader reader = new DetaBufferedReader(
							DetaFileReader.E(readDBTableSpecCulumnFile));
					String tempString;
						...
						if (fileDBTableRowsPath.isDirectory()) {
							...
							for (String row : rowList) {
								...
								if (readDBTableRowIndexFile.isDirectory()) {
									...
									if (readDBTableRowIndexCulumnFile
											.isDirectory()) {
										...
										if (temp.equalsIgnoreCase(
												value.toString())) {
											String[] culumnList = readDBTableRowIndexFile
													.list();
											...
											for (String culumn : culumnList) {
												...
												String DBTableCulumnIndexPath = DBTableRowIndexPath
														+ "/" + culumn;
												File readDBTableCulumnIndexPathFile = new File(
														DBTableCulumnIndexPath);
												if (readDBTableRowIndexCulumnFile
														.isDirectory()) {
													reader = new DetaBufferedReader(
															DetaFileReader.E(
																	readDBTableCulumnIndexPathFile
																			+ "/"
																			+ "value.lyg"));
													temp = "";
													while ((tempString = reader
	...
	public static void main(String[] args) {
		Q_Rows_E_X_selectRowsByAttribute._E("backend", "login", "usr_name",
				"yaoguangluo");
	}
}
//118
