				...
				if (readDBTableRowIndexFile.isDirectory()) {
					...
					if (readDBTableRowIndexCulumnFile.isDirectory()) {
						DetaBufferedReader reader = new DetaBufferedReader(
								DetaFileReader.E(readDBTableRowIndexCulumnFile
										+ "/" + "value.lyg"));
						String temp = "";
						String tempString;
						...
						if (sets[1].equalsIgnoreCase(S_ShellETL.SHELL_ETL_SMALL)
								|| sets[1].equalsIgnoreCase(
										S_ShellETL.SHELL_ETL_LESS_THAN)) {
							if (new BigDecimal(temp)
									.doubleValue() < new BigDecimal(sets[2])
											.doubleValue()) {
								if (!((Map<Integer, Boolean>) (object
										.get("recordRows")))
										.containsKey(count)) {
									Row bufferRow = new Row();
									bufferRow
											.I_Cells(new ConcurrentHashMap<>());
									X_Kernel.P_kernel(readDBTableRowIndexFile,
						...
						if (sets[1].equalsIgnoreCase(
								S_ShellETL.SHELL_ETL_DOUBLE_EQUALS)
								|| sets[1].equalsIgnoreCase(
						...
						if (sets[1]
								.equalsIgnoreCase(S_ShellETL.SHELL_ETL_GREATER)
								|| sets[1].equalsIgnoreCase(
						...
						if (sets[1].equalsIgnoreCase(
								S_ShellETL.SHELL_ETL_NOT_EUQAL_TO)
								|| sets[1].equalsIgnoreCase(
						...
						if (sets[1]
								.equalsIgnoreCase(S_ShellETL.SHELL_ETL_EQUAL)) {
						...
						if (sets[1].equalsIgnoreCase(
								S_ShellETL.SHELL_ETL_DOES_NOT_EQUALS)) {
							if (!temp.equalsIgnoreCase(sets[2])) {
								if (!((Map<Integer, Boolean>) (object
										.get("recordRows")))
										.containsKey(count)) {
						...
						if (sets[1]
								.equalsIgnoreCase((S_ShellETL.SHELL_ETL_IN))) {
							String set = ", " + sets[2] + ", ";
