	...
	public static void P_Map(String[] sets, List<IMV_SIQ> output,
		...
		while (iterator.hasNext()) {
			...
			if (sets[1].equalsIgnoreCase(S_ShellETL.SHELL_ETL_SMALL) || sets[1]
				...
				String rowCellFromString = ((IMV_SIQ) (((IMV_SIQ) (row
						.get(S_ShellETL.SHELL_ETL_ROWVALUE))).get(sets[0])))
						.get("culumnValue").toString();
			...
			if (sets[1].equalsIgnoreCase(S_ShellETL.SHELL_ETL_LESS_AND_EQUAL_TO)
				...
				String rowCellFromString = ((Map<String, Object>) (((IMV_SIQ) (row
						.get(S_ShellETL.SHELL_ETL_ROWVALUE))).get(sets[0])))
						.get("culumnValue").toString();
				if (new BigDecimal(rowCellFromString)
						.doubleValue() <= new BigDecimal(sets[2])
			...
			if (sets[1].equalsIgnoreCase(S_ShellETL.SHELL_ETL_DOUBLE_EQUALS)
				...
				String rowCellFromString = ((IMV_SIQ) (((Map<String, Object>) (row
						.get(S_ShellETL.SHELL_ETL_ROWVALUE))).get(sets[0])))
						.get("culumnValue").toString();
				if (new BigDecimal(rowCellFromString)
						.doubleValue() == new BigDecimal(sets[2])
