	...
	public void Register(GUISample gUISample, App NE) {
		gUISample.load.addActionListener(new java.awt.event.ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					gUISample.first.first = File_X_Load._E(
							gUISample.first.first, gUISample.nodeView, file,
							gUISample.first, NE);
		...
		gUISample.save.addActionListener(new java.awt.event.ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				SaveAnd_U_File.U_(gUISample.fileCurrentpath,
		...
		gUISample.osgis.addActionListener(new java.awt.event.ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				Here: for (int i = 0; i < files.length; i++) {
					...
					String FirstCharOfLastName = ("" + lastName.charAt(0))
							.toUpperCase();
					...
					gUISample.nodeReflection.put(
							_OSU_MCI_MCU_MSI._SQ_OSU_MSQ_OSU_AVQ_ASQ_SQ_VPC_PCS,
							null);
					_OSU_MCI_MCU_MSI.register(gUISample.tableData_old,
							gUISample.text, NE);
					...
					gUISample.nodeView.first = gUISample.nodeView.link.addNode(
							gUISample.nodeView.first, _OSU_MCI_MCU_MSI);
				}
		...
		gUISample.boot.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(ActionEvent e) {
				BootNeroCell.bootCell(gUISample.first.first,
						gUISample.rightBotJTextPane, gUISample.canvas, NE);
				...
				.addActionListener(new java.awt.event.ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						try {
							...
							LinkNode needDeleteNode = gUISample.first.first;
							while (needDeleteNode != null) {
								gUISample.first.first = gUISample.first
										.deletNode(gUISample.first.first,
												needDeleteNode.name,
												needDeleteNode.ID,
							...
							gUISample.canvas.repaint();
							gUISample.first.first = File_X_Load._E(
									gUISample.first.first, gUISample.nodeView,
									file, gUISample.first, NE);
						} catch (Exception loadE) {
							loadE.printStackTrace();
						}
						gUISample.canvas.repaint();
		...
		gUISample.leftSplitPane.addPropertyChangeListener(
				new java.beans.PropertyChangeListener() {
					public void propertyChange(
						...
						if (evt.getPropertyName()
							...
							// action code
							gUISample.nodeProject.width = gUISample.leftSplitPane
									.getWidth();
							gUISample.nodeProject.height = gUISample.leftSplitPane
									.getDividerLocation();
							gUISample.nodeProject.setBounds(0, 0,
									gUISample.leftSplitPane.getWidth(),
									gUISample.leftSplitPane
											.getDividerLocation());
							gUISample.nodeProject.validate();
						}
					}
				});
		gUISample.mainSplitPane.addPropertyChangeListener(
				new java.beans.PropertyChangeListener() {
					public void propertyChange(
							java.beans.PropertyChangeEvent evt) {
						if (evt.getPropertyName()
		...
		gUISample.nodeView.tree
				.addTreeSelectionListener(new TreeSelectionListener() {
					public void valueChanged(TreeSelectionEvent evt) {
						DefaultMutableTreeNode note = (DefaultMutableTreeNode) gUISample.nodeView.tree
						...
						if (tr != null) {
							...
							gUISample.rightBotJTextPane.setText(
									"jieDianMing：" + gUISample.treeNodeName);
							gUISample.rightBotJTextPane.validate();
						}
					}
				});
				...
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(ActionEvent e) {
						if (gUISample.treeNodeName != null) {
							gUISample.first.first = gUISample.first.addNode(
									gUISample.first.first,
									gUISample.treeNodeName, 100, 50,
									gUISample.nodeView.first, NE);
							gUISample.righttopScrollPane.validate();
							gUISample.rightBotJTextPane.setText(
									"jieDianMing：" + "gUISample.treeNodeName");
							gUISample.rightBotJTextPane.validate();
		...
		gUISample.configre
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						gUISample.first.first = Sort
								.sort(gUISample.first.first);
						node = gUISample.first.first;
						while (node != null) {
							if (node.name
									.equals(gUISample.canvas.currentNodeName)
									&& node.ID == gUISample.canvas.currentNodeID
									&& node.primaryKey.equals(
											gUISample.canvas.currentNodePrimaryKey)) {
								node.thisFace.MEI_MSU(
										gUISample.rightBotJTextPane,
										gUISample.canvas);
								node.thisFace._SQ__AOU_AOQ_VES
										.setLocation(node.x, node.y);
								node.thisFace._SQ__AOU_AOQ_VES.setSize(500,
										500);
								node.thisFace._SQ__AOU_AOQ_VES
										.setResizable(true);
								node.thisFace._SQ__AOU_AOQ_VES.scrollPane
										.setBounds(0, 0,
												node.thisFace._SQ__AOU_AOQ_VES
														.getWidth() - 10,
												node.thisFace._SQ__AOU_AOQ_VES
														.getHeight() - 45);
								node.thisFace._SQ__AOU_AOQ_VES.panel
										.setPreferredSize(
								...
								// 加primaryKEY标识
								node.thisFace.primaryInterfaceKeyID = node.primaryKey
										.toString();
							}
							node = node.next;
						}
						gUISample.rightBotJTextPane
								.setText(S_Annotation.NODE_UPDATE_SUCCESS);
						gUISample.rightBotJTextPane.validate();
					}
				});
			...
			public void actionPerformed(ActionEvent e) {
				...
				while (node != null) {
					if (node.name.equals(gUISample.canvas.currentNodeName)
						...
						node.thisFace.primaryInterfaceKeyID = node.primaryKey
								.toString();
						node.thisFace.OPE_E(gUISample.rightBotJTextPane, NE);// 稍后hook
																			 // 进行
																			 // 异常修整
																			 // 20230218
					}
		...
		gUISample.show.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				while (node != null) {
					if (node.name.equals(gUISample.canvas.currentNodeName)
						...
						if (!node.thisFace.showed) {
							try {
								node.thisFace.AVQ_ASQ_OVQ_OSQ_VSQ(
										gUISample.rightBotJTextPane, NE);
								node.thisFace._SQ__OVQ_OSQ_VSQ
										.setLocation(node.x, node.y);
								node.thisFace._SQ__OVQ_OSQ_VSQ.setSize(500,
										500);
								node.thisFace._SQ__OVQ_OSQ_VSQ
										.setResizable(true);
								node.thisFace._SQ__OVQ_OSQ_VSQ.scrollPane
										.setBounds(0, 0,
												node.thisFace._SQ__AOU_AOQ_VES
														.getWidth() - 10,
												node.thisFace._SQ__AOU_AOQ_VES
														.getHeight() - 45);
								node.thisFace._SQ__OVQ_OSQ_VSQ.panel
										.setPreferredSize(
												new Dimension(800, 600));
								...
								// e1.printStackTrace();
								gUISample.rightBotJTextPane.setText(
										S_Annotation.NODE_INSPECT_ERROR);
								gUISample.rightBotJTextPane.validate();
							}
						} else {
							node.thisFace._SQ__OVQ_OSQ_VSQ.setVisible(true);
				...
				gUISample.rightBotJTextPane
						.setText(S_Annotation.NODE_INDICATE_SUCCESS);
				gUISample.rightBotJTextPane.validate();
			}
		});
