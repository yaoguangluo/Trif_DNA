	...
	public static void _E(GUISample gUISample, App NE) {
		...
		Here: for (int i = 0; i < files.length; i++) {
			...
			String filename = files[i].getName().replace(
					S_Annotation.FILE_FORMAT_JAR, S_Annotation.STRING_EMPTY);
			if (!filename.contains("OSI.OSU.")) {// 新接口注意下包名20230206
				continue Here;
			}
			/*
			 * 源头: 见JDK 官方文档 的classloader 描述 //如下注释2行代码 refer
			 * https://www.cnblogs.com/chinaxin/p/3678442.html
			 * 这小伙子以后有前途。哈哈 //Class<?> myclass=
			 * loader.loadClass("hand.java.loadjar .TestClass"); //Gene
			 * new object //Object myobject= myclass.newInstance();
			 */
			...
			if (gUISample.nodeReflection.containsKey(
				...
				gUISample.rightBotJTextPane
						.setText("请勿重复添加, 节点已存在" + System.currentTimeMillis());
				continue Here;
				// return;
			}
			gUISample.nodeReflection.put(
					_OSU_MCI_MCU_MSI._SQ_OSU_MSQ_OSU_AVQ_ASQ_SQ_VPC_PCS, null);
			_OSU_MCI_MCU_MSI.register(gUISample.tableData_old, gUISample.text,
					NE);
			...
			gUISample.rightBotJTextPane.setText(
					"节点:" + _OSU_MCI_MCU_MSI._SQ_OSU_MSQ_OSU_AVQ_ASQ_SQ_VPC_PCS
