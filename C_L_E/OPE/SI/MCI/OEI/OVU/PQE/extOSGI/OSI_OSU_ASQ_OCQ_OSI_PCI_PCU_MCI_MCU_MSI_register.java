...
public class OSI_OSU_ASQ_OCQ_OSI_PCI_PCU_MCI_MCU_MSI_register {
    ...
    public OSI_OSU_ASQ_OCQ_OSI_PCI_PCU_MCI_MCU_MSI_register(Object[][] tableData_old
    ...
    public NodeOSGI Rigester(NodeOSGI first, LinkOSGI link, App NE) {
        //注册
        OSU_MCI_MCU_MSI XlsReadernode = new XlsReaderNodeInterface();
        first = link.addNode(first, XlsReadernode);

        OSU_MCI_MCU_MSI imageReadNode = new ImageReadNodeInterface();
        first = link.addNode(first, imageReadNode);

        OSU_MCI_MCU_MSI imageStrechNode = new ImageStrechNodeInterface();
        first = link.addNode(first, imageStrechNode);

        OSU_MCI_MCU_MSI MedianImageNode = new MedianImageNodeInterface();
        first = link.addNode(first, MedianImageNode);

        OSU_MCI_MCU_MSI GrayFilterNode = new GrayFilterNodeInterface();
        first = link.addNode(first, GrayFilterNode);

        OSU_MCI_MCU_MSI GuassianFilterNode = new GuassianFilterInterface();
        first = link.addNode(first, GuassianFilterNode);

        OSU_MCI_MCU_MSI FindColorRNode = new FindColorRInterface();
        first = link.addNode(first, FindColorRNode);
        OSU_MCI_MCU_MSI FindColorGNode = new FindColorGInterface();
        first = link.addNode(first, FindColorGNode);
        OSU_MCI_MCU_MSI FindColorBNode = new FindColorBInterface();
        first = link.addNode(first, FindColorBNode);

        OSU_MCI_MCU_MSI SobelFilterNode
                = new SobelFilterNodeInterface();
...
//		first= link.addNode(first, dNA3DShow);
        ...
        OSU_MCI_MCU_MSI _OSU_MCI_MCU_MSI = (OSU_MCI_MCU_MSI) searchPG;
        u.gUISample.nodeReflection.put(_OSU_MCI_MCU_MSI._SQ_OSU_MSQ_OSU_AVQ_ASQ_SQ_VPC_PCS, "");
        _OSU_MCI_MCU_MSI.register(u.gUISample.tableData_old, u.gUISample.text, NE);
