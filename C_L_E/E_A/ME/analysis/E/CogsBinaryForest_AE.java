    ...
    public IMV_SIQ_SS parserMixStringByReturnFrequencyMap(String inputString, App NE) {
        ...
        for (int charPosition = S_Pos.INT_ZERO; charPosition < inputLength
            ...
            if (find == S_Pos.INT_ONE) {
                ...
                while (it.hasNext()) {
                    ...
                    if (outputList.containsKey(temp)) {
                        ...
                        WordFrequency wordFrequency = new WordFrequency(1.0, temp);
                        wordFrequency.positions.add(NE._I_U.parserCharPosition);
                        outputList.put(temp, wordFrequency);
                    }
                }
            ...
            if (compare == S_Pos.INT_THREE) {
                ...
                countLength = nlp_C.ofThreeForMap(countLength, outputList, stringBuilder
                    , fixWords, NE);
                continue Here;
            }
            if (compare == S_Pos.INT_ONE) {
                if (outputList.containsKey(countWordNode)) {
                    ...
                    wordFrequency.positions.add(NE._I_U.parserCharPosition);//xian dan hou duo..later..
                    outputList.put(countWordNode, wordFrequency);
                }
                fixWords[S_Pos.INT_ZERO].delete(S_Pos.INT_ZERO
                    , fixWords[S_Pos.INT_ZERO].length());
