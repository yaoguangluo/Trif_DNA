 ...
 * */
public class Quick9DLYGWithIntObject_ESU extends Quick_7D_luoyaoguang_Sort {
    public int[] sort(int[] a, Object[] sa) {
        processInt(a, 0, a.length - 1, sa, 0);
        return a;
    }

    //增加了deep
    private void processInt(int[] array, int leftPoint, int rightPoint, Object[] sa
        , int deep) {
        int c = rightPoint - leftPoint + 1;
        if (!(c < 7 || deep > 70)) {
            int pos = partition(array, leftPoint, rightPoint, sa);
            if (leftPoint < pos - 1) {
                processInt(array, leftPoint, pos - 1, sa, deep + 1);
            }
            if (pos + 1 < rightPoint) {
                processInt(array, pos + 1, rightPoint, sa, deep + 1);
            }
            return;
        }
        int i = leftPoint;
    ...
    private int partition(int[] array, int leftPoint, int rightPoint, Object[] sa) {
