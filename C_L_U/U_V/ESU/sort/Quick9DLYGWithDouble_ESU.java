 ...
 * */
public class Quick9DLYGWithDouble_ESU extends Quick_7D_luoyaoguang_Sort {
    public double[] sort(double[] a, double[] sa) {
        processDouble(a, 0, a.length - 1, sa, 0);
        return a;
    }

    //增加了deep
    private void processDouble(double[] array, int leftPoint, int rightPoint, double[] sa
        , int deep) {
    ...
    private int partition(double[] array, int leftPoint, int rightPoint, double[] sa) {
        double x = array[leftPoint] <= array[rightPoint] ? array[leftPoint] : array[rightPoint];
        double xString = array[leftPoint] <= array[rightPoint] ? sa[leftPoint] : sa[rightPoint];
