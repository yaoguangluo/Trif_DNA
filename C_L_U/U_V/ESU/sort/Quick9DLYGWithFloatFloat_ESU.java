 ...
 * */
public class Quick9DLYGWithFloatFloat_ESU extends Quick_7D_luoyaoguang_Sort {
    public float[] sort(float[] a, float[] sa) {
        processFloat(a, 0, a.length - 1, sa, 0);
        return a;
    }

    //增加了deep
    private void processFloat(float[] array, int leftPoint, int rightPoint, float[] sa
        , int deep) {
    ...
    private int partition(float[] array, int leftPoint, int rightPoint, float[] sa) {
        float x = array[leftPoint] <= array[rightPoint] ? array[leftPoint] : array[rightPoint];
        float xString = array[leftPoint] <= array[rightPoint] ? sa[leftPoint] : sa[rightPoint];
