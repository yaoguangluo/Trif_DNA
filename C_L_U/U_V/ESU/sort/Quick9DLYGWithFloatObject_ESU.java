 ...
 * */
public class Quick9DLYGWithFloatObject_ESU extends Quick_7D_luoyaoguang_Sort {
    public float[] sort(float[] a, Object[] sa) {
        processFloat(a, 0, a.length - 1, sa, 0);
        return a;
    }

    //增加了deep
    private void processFloat(float[] array, int leftPoint, int rightPoint, Object[] sa
        , int deep) {
        int c = rightPoint - leftPoint + 1;
        if (!(c < 7 || deep > 70)) {
            int pos = partition(array, leftPoint, rightPoint, sa);
            if (leftPoint < pos - 1) {
                processFloat(array, leftPoint, pos - 1, sa, deep + 1);
            }
            if (pos + 1 < rightPoint) {
                processFloat(array, pos + 1, rightPoint, sa, deep + 1);
            }
            return;
        }
        int i = leftPoint;
    ...
    private int partition(float[] array, int leftPoint, int rightPoint, Object[] sa) {
        float x = array[leftPoint] <= array[rightPoint] ? array[leftPoint] : array[rightPoint];
        Object xString = array[leftPoint] <= array[rightPoint] ? sa[leftPoint] : sa[rightPoint];
        ...
        while (leftPointReflection < rightPoint) {
            ...
            if (leftPointReflection < ++rightPoint) {
                ...
                Object stemp = sa[rightPoint];
                sa[rightPoint] = sa[leftPointReflection];
                sa[leftPointReflection] = stemp;
            }
        }
        array[leftPoint] = array[rightPoint];
        sa[leftPoint] = sa[rightPoint];
        array[rightPoint] = x;
        sa[rightPoint] = xString;
        return rightPoint;
    }
}
