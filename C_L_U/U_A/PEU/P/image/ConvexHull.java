 ...
 * */
//奇怪，之前我refer Google 搜索的边界文字那段，今天优化新陈代谢，发现不见了，那就再注释下。
/*
 * 作者 罗瑶光, 浏阳
 * yaoguangluo@outlook.com, 313699483@qq.com, 2080315360@qq.com, -
 ** 15116110525-
...
public class ConvexHull {
    ...
    public int compareSlope(int dy2, int dx2, int dy1, int dx1) {
        if (dx2 != 0 && dx1 != 0) {
            double test = Double.valueOf(dy2 * dx1 - dy1 * dx2);
    ...
    public void PNGWrite(int[][] g, int pix, String output) {
    ...
    public void CSVWrite(int[][] g, int pix, String outputcsv) {
        int[][] g1 = new int[g.length][g[0].length];
        int n = 0;
        for (int p = 0; p < g.length; p++) {
            for (int q = 0; q < g[0].length; q++) {
