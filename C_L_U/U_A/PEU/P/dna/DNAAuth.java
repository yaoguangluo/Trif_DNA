...
public class DNAAuth {
	...
	public static boolean DNAAuthStatusCheckEmailAndPds(App app,
		String string, Map<String, String> data) {
		//同理稍后做pds
		String uEmail = "id";
		String uPassword = "password";
		//检查是否有email id
		if (!data.containsKey(uEmail) || !data.containsKey(
			uPassword)) {
			uEmail = "uEmail";
			uPassword = "uPassword";
			if (!data.containsKey(uEmail) || !data.containsKey(
		...
		//数据库取密
		UsrToken usrToken = LoginServiceImpl.findUsrTokenByUId(Integer
			.valueOf(usr.getuId()));
		if (null == usrToken.getuPassword()) {
			return false;
		}
		//取密进行加密
		String[] MD5dice_DNA = usrToken.getuKey().split(
		...
		TokenCerts tokenCerts = X_sessionTokenCertsInitWithHumanWordsByDNA
			._E(data.get(uPassword), true, MD5dice_DNA[0]);
		//tvmpassword->pde, dps
		...
		bootTokenPDI.doKeyUnPress(tokenCerts.getPdnPassword(),
			tokenPDI, true);
	...
	//写个新的DNA status 跟之前的区分开来
	public static boolean DNAAuthStatusCheckEmailAndPde(App app,
		String string, Map<String, String> data) {
		String uEmail = "id";
		String uPassword = "password";
		//检查是否有email id
	...
	public static boolean DNAAuthStatusCheckEmailAndPassword(
		App_S app, String string, Map<String, String> data) {
		String uEmail = "id";
		String uPassword = "password";
		//检查是否有email id
		if (!data.containsKey(uEmail) || !data.containsKey(
	...
	public static boolean DNAAuthCheck(App app, String string,
		...
		String lock = data.containsKey("lock") ? data.get("lock")
			: "";// data.get("lock");
		//也可已放数据表中.不在rest里面发送.
		...
		//if(null== password) {
		id = "313699483@qq.com";
		String idString = String_ESU_X_charsetSwap._E(id, "UTF8",
			"UTF8");
		String idEncoder = String_ESU_X_stringToURIencode._E(idString,
			"UTF8");
		...
		String passwordString = String_ESU_X_charsetSwap._E(token
		...
		tokenCerts = X_sessionTokenCertsInitWithHumanWordsByDNA._E(
			DB_password, true, lock);
