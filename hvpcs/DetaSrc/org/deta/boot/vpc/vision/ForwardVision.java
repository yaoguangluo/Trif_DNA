 ...
 * */public class ForwardVision {
		 ...
		 try {
			 if(vPCSRequest.getRequestIsRest()){
				 ...
				 if(filePath.contains(".ttf")||filePath.contains(".eot")
						 ||filePath.contains(".svg")||filePath.contains(".woff")
						 ||filePath.contains(".woff2")||filePath.contains(".otf")){
			 ...
			 System.out.flush(); System.out.println("-fi-"+ fileString);
				 ...
				 if(StableSets.contentTypes.containsKey(stings[1])) {
					 if(stings[1].contains("js")|| stings[1].contains("map")) {
						 if(code.equalsIgnoreCase("UTF-8")) {
							 vPCSResponse.setResponseContentType(StableSets
									 .contentTypes.get(stings[1])); 
							 return;
						 }
					 }
					 vPCSResponse.setResponseContentType(StableSets
 ...
 //	 vPCSResponse.setResponseContentType("content-type: image/ico \n\n");
 //	 return;
 // }
 ...
 //	 return;
 // }
 // if(filePath.contains(".map") && code.equalsIgnoreCase("UTF-8")){//	
 //	 vPCSResponse.setResponseContentType(
 ...
 //	 vPCSResponse.setResponseContentType("content-type: application/jar \n\n");
 //	 return;
 // }
 // if(filePath.contains(".pptx")){
 //	 vPCSResponse.setResponseContentType("content-type: application/pptx \n\n");
 //	 return;
 // }

 //	 if(!StableBlob.addBlobLimit(filePath, System.currentTimeMillis())) {//fix later
 //		 System.out.flush();System.out.println(vPCSResponse.getHashCode()+"-dreaming asyc");
