...
public class RequestRecordController {
	public static void requestIpRecoder(VPCSRequest vPCSRequest,
		...
		vPCSRequest.setRequestIp(
				vPCSResponse.socket.getInetAddress().getHostAddress());
		vPCSRequest.setRequestName(
				vPCSResponse.socket.getInetAddress().getHostName());
	}
	...
	public static void requestLinkRecoder(VPCSRequest vPCSRequest,
		...
		if (content.length == 2) {
			...
			if (content[0].equalsIgnoreCase("/") || content[0].contains(".ash")
					|| content[0].contains(".txt") || content[0].contains(".js")
