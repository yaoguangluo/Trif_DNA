	...
	public CrabNodePanel(final CrabNodeRun _SQ__OPE_OPC_ECI, JTextPane text,
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				S_logger.Log.logger.info(e.getSource().toString());
				close = true;
		...
		readfile.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				int c = 0;
				Here: for (int i = 0; i < rc; i++) {
					for (int j = 0; j < rc; j++) {
						if (_SQ__OPE_OPC_ECI.toptablein.getValueAt(j, 3)
								.toString().contains(_SQ__OPE_OPC_ECI.toptablein
