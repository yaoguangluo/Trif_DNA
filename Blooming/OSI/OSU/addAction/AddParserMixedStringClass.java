 ...
 * */
//稍后封装成一个统一的傻瓜接口。
public class AddParserMixedStringClass implements CrabInterface {
	String callFunctionKey;
	String className = "AddParserMixedStringClass";
	...
	public void chromosomes(App NE) {
		...
		NE.app_S.staticRootMap.chromosomeNode.put(callFunctionKey,
			new AddParserMixedStringClass());//20241001准备把这行移出去。
	...
	public boolean logic(IMV_SIQ inputValues, String[] 传参因子, int 因子,
		...
		inputValues = StaticFunctionMap.preValues(NE.app_S.outputMap,
			传参因子);
		Object map = null;
		if (inputValues.getBoolean("find")) {
			map = StaticFunctionMapA_VECS_C.parserMixedString(NE);
		}
		StaticFunctionMap.postValues(NE.app_S.outputMap, inputValues
