 ...
 * */
public class ZhongYaoSearch {//缩小倍差 later。。
    ...
    public static void zhongYaoSearch(App_S app, String zhongyao, String forE, String key, App NE) {
        ...
        app._A.initPCAWordPOS(mapSearchWithoutSort, NE);
        int copyCount = 0;
        ...
        while (iteratorForCopy.hasNext()) {
            ...
            while (iteratorWordFrequency.hasNext()) {
                ...
                if (temps.contains(mapSearchaAtII)) {
                    ...
                    if (tempsIndex.contains(zhongyaoReplace.toUpperCase())) {
                        reg[copyCount] += 120;// later..
                    }
                    int wgf = (int)wordFrequencySearch.get_frequency();
                    wgf = wgf > 5 ? 5 : wgf;
                    score[copyCount] = iteratorForCopyString;
                    if (!app.fMHMMListOneTime_E_X_S.posCnToCn.containsKey(mapSearchaAtII)) {
                        reg[copyCount] += 1;
                        score_code[copyCount] += 1 << mapSearchaAtII.length() << wgf;
                    ...
                    score_code[copyCount] += (!posString.contains("名")
                        ? posString.contains("动") ? 15 : 1 : 20) << (temp ? 2 : 1)
                        << mapSearchaAtII.length() * wgf;
                    if (score[copyCount].contains(mapSearchaAtII)) {
                        if (score[copyCount].length() > 1) {
                            reg[copyCount] += 10;
                        }
            ...
            score_code[copyCount] *= reg[copyCount];
            score_code[copyCount] >>= 6;
            //
            int code = 200;
            int tempb = 0;
            ...
            if (zhongyaoReplace.length() > 1 && zhongyaoReplace.length() < 5) {
                if (temps.contains(zhongyaoReplace)) {
                    tempb += code << 4;
                }
                if (temps.contains(key.replace(" ", ""))) {
                    tempb += code << 2;
        ...
        for (int i = app.copy.size() - 1; i > -1; i--) {
            ...
            if (!app.nameFeelFilter.getText().isEmpty()) {//性味
                String wei = !app.dic_xw.containsKey(score[i]) ? ""
                    : app.dic_xw.get(score[i]).toString()
                    .replaceAll("\\s*", "");
                CatalyticDNA_xingwei = app.nameFeelFilter.getText()
                    .replace(" ", "");
            ...
            if (null != app.name_filter_not_have.getText()) {
                if (!app.name_filter_not_have.getText().replace(" "
                    , "").isEmpty()) {
                    String wei = !app.dic_jm.containsKey(score[i]) ? ""
                        : app.dic_jm.get(score[i]).toString()
                        .replaceAll("\\s*", "");
                    CatalyticDNA_gongxiao = app.name_filter_not_have.getText()
                        .replace(" ", "");
                    for (int j = 0; j < CatalyticDNA_gongxiao.length(); j++) {//.知道了就不要说出来
            ...
            tableData[new_count] = new Object[]{
                ...
                (app.dic_zf.get(score[i]) == null ? ""
                    ...
                    .replaceAll("\\s*", ""),
                "" + (new_count + 1)};
            /*later do a main tab with all in 1 and do a normalization and duplication*/
            ...
            for (int j = 0; j < strings.length; j++) {
                ...
                if (filters[0].equalsIgnoreCase("药食同源")) {//later all in 1*?\
                    for (int k = 0; k < tableData[new_count].length; k++) {
                        if (NE.app_S.columnTitle[k].toString().equalsIgnoreCase(filters[1])) {
                            if (null == tableData[new_count][k]) {
                                tableData[new_count][k] = "";
                            }
                            if (filters[2].contains("剔除")) {
                                if (tableData[new_count][k].toString().contains(filters[3])) {
                                    needDelete = true;
                                }
