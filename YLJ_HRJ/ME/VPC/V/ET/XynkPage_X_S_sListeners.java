...
import ME.VPC.S.pillow.XynkPage_X_S_s;
...
public class XynkPage_X_S_sListeners extends XynkPage_X_S_s
    ...
    public javax.swing.JTable jTable(App app_NE) {
        ...
        for (int i = 0; i < copy.size(); i++) {
            tableData_old[i] = new Object[]{"" + (i + 1), "" + 0, copy.get(i).trim()
                , dic_map.getTrimString(copy.get(i))
    ...
    public void keyReleased(KeyEvent arg0) {
        ...
        int count = AppSearch_X_DetaSearch.detaSearch(score_code, score, newTableModel
            , tableData_old, copy, key, dic_map, true, NE);
        ...
        for (int i = copy.size() - 1; i > 0; i--) {
            ...
            tableData[new_count] = new Object[]{new_count + 1, score_code[i - 1], score[i],
                dic_map.getString(score[i]),
                dic_zl.getString(score[i]),
                dic_lc.getString(score[i]),
                dic_sy.getString(score[i]),
                dic_zd.getString(score[i]),
                dic_by.getString(score[i]),
                dic_gn.getString(score[i]),
                dic_lx.getString(score[i]),
                dic_wx.getString(score[i]),
                dic_bl.getString(score[i]),
                dic_bf.getString(score[i]),
                dic_jy.getString(score[i]),
                dic_yh.getString(score[i]),
                dic_yf.getString(score[i]),
                dic_yx.getString(score[i])
            };
...
//171 //				if(score[i].contains("检验")) {
////				try {
////					FileWriter fw= new FileWriter("D:/Users/Lenovo/eclipse-workspace
////					/DETA_YLJ1.0.0/medReport/org/tinos/reportMap/report.txt", true);
////					fw.write(score[i]+"->"+dic_lc.get(score[i]).toString()+ "->"
