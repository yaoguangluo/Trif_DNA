...
public class XynkPage_X_S_sListeners extends XynkPage_X_S_s
	...
	public void keyReleased(KeyEvent arg0) {
		...
		Here: for (int i = copy.size() - 1; i > 0; i--) {
			if (score_code[i] < 1) {
				continue Here;
			}
			...
			tableData[new_count] = new Object[] { new_count + 1,
					score_code[i - 1], score[i], dic_map.getString(score[i]),
					dic_zl.getString(score[i]), dic_lc.getString(score[i]),
					dic_sy.getString(score[i]), dic_zd.getString(score[i]),
					dic_by.getString(score[i]), dic_gn.getString(score[i]),
					dic_lx.getString(score[i]), dic_wx.getString(score[i]),
					dic_bl.getString(score[i]), dic_bf.getString(score[i]),
					dic_jy.getString(score[i]), dic_yh.getString(score[i]),
					dic_yf.getString(score[i]), dic_yx.getString(score[i]) };
