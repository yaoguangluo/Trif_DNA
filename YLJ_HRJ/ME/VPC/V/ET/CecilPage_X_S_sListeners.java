...
public class CecilPage_X_S_sListeners extends CecilPage_X_S_s
    ...
    public void mouseClicked(MouseEvent arg0) {
            ...
            if (!setOfi.equals("")) {
                ...
                if (pecsi.contains("名")) {
                    page.append("<span style=\"background:" + Color_P
                        .P(255, 245, 255) + "\"><font color=\"black\" size=\"5\">"
                        + setOfi + "</font></span>");
                    continue;
                }
                if (pecsi.contains("动")) {
                    page.append("<span style=\"background:" + Color_P
                        .P(245, 255, 245) + "\"><font color=\"black\" size=\"5\">"
                        + setOfi + "</font></span>");
                    continue;
                }
                if (pecsi.contains("形")) {
                    page.append("<span style=\"background:" + Color_P
                        .P(255, 255, 245) + "\"><font color=\"black\" size=\"5\">"
                        + setOfi + "</font></span>");
                    continue;
                }
                if (pecsi.contains("副")) {
        ...
        for (int i = fwa.size() - 1; i >= 0; i--) {
            if (fwa.get(i) != null) {
                String pecsi = "";
                String pfullecsi = posFullec.getString(fwa.getW(i).get_word().toLowerCase());
                if (pfullecsi == null) {
                    page.append("<div style=\"background:black\">" + "<font color=\"white\">"
                        + fwa.getW(i).get_word() + "" + fwa.getW(i).get_frequency()
                        + "</font></div>");
                    continue;
                }
                if (pos.containsKey(pfullecsi)) {
    ...
    public void keyReleased(KeyEvent arg0) {
        ...
        for (int i = 0; i < stringReg.length; i++) {
            stringReg[i] = key.substring(i * 3, (i * 3 + 3)
                < key.length() ? (i * 3 + 3) : key.length() - 1);
        }
        while (iteratorForCopy.hasNext()) {
            ...
            while (iteratorWordFrequency.hasNext()) {
                ...
                if (temps.contains(mapSearchaAtII) && mapSearchaAtII.length() > 2) {
                    ...
                    score_code[copyCount] += (iteratorForCopyString.contains(mapSearchaAtII)
                        ? 2 : 1) * (!pecsi.contains("名")
                        ? pecsi.contains("动") ? 10 : 1 : 50);
                }
            }
            ...
            if (key.length() > 24) {
                ...
                score_code[copyCount]
                    = (int) (tempa / Math.pow(this.app_S.lookrot + 1, 4)
