            ...
            while (iteratorWordFrequency.hasNext()) {
                ...
                if (temps.contains(mapSearchaAtII)) {
                    ...
                    if (!pos.containsKey(mapSearchaAtII)) {
                        ...
                        wfs = wfs > 5 ? 5 : wfs;
                        score_code[copyCount] += 1 << mapSearchaAtII.length() << (int)wfs;
                        continue Here;
                    }
                    if (pos.getString(mapSearchaAtII).contains("名") || pos.getString(mapSearchaAtII).contains("动")
                    ...
                    score_code[copyCount] += (iteratorForCopyString.contains(mapSearchaAtII) ? 2 : 1)
                        * (!pos.getString(mapSearchaAtII).contains("名")
                        ? pos.getString(mapSearchaAtII).contains("动") ? 45 : 1 : 50)
