	...
	public javax.swing.JTable jTable(App app_NE) {
		...
		for (int i = 0; i < copy.size(); i++) {
			tableData_old[i] = new Object[] { "" + (i + 1), "" + 0,
					copy.get(i).trim(),
					dic_chengfen_danwei.get(copy.get(i)).toString()
							.replaceAll("\\s*", ""),
					dic_zhu_yi.get(copy.get(i)).toString().replaceAll("\\s*",
							""),
					dic_yong_liang.get(copy.get(i)).toString()
							.replaceAll("\\s*", ""),
					...
					dic_yong_fa.get(copy.get(i)).toString().replaceAll("\\s*",
							""),
					dic_bu_liang_fan_ying.get(copy.get(i)).toString()
							.replaceAll("\\s*", ""),
					dic_yao_wu_xiang_hu_zuo_yong.get(copy.get(i)).toString()
							.replaceAll("\\s*", ""),
	...
	public void keyReleased(KeyEvent arg0) {
		...
		while (iteratorForCopy.hasNext()) {
			...
			String temps = iteratorForCopyString.toString()
					+ NullObject
							.checkNULL(dic_jie_shao.get(iteratorForCopyString))
							.replaceAll("\\s*", "")
					+ NullObject
							.checkNULL(dic_yao_li.get(iteratorForCopyString))
							.replaceAll("\\s*", "")
					+ NullObject
							.checkNULL(dic_chengfen_danwei
									.get(iteratorForCopyString))
							.replaceAll("\\s*", "")
					+ NullObject
							.checkNULL(dic_yong_fa.get(iteratorForCopyString))
							.replaceAll("\\s*", "")
					+ NullObject
							.checkNULL(dic_zhu_yi.get(iteratorForCopyString))
							.replaceAll("\\s*", "")
					+ NullObject
							.checkNULL(dic_shi_ying.get(iteratorForCopyString))
							.replaceAll("\\s*", "")
					+ NullObject
							.checkNULL(dic_bu_liang_fan_ying
									.get(iteratorForCopyString))
							.replaceAll("\\s*", "")
					+ NullObject
							.checkNULL(
									dic_yong_liang.get(iteratorForCopyString))
							.replaceAll("\\s*", "")
					+ NullObject
							.checkNULL(dic_yao_wu_xiang_hu_zuo_yong
									.get(iteratorForCopyString))
							.replaceAll("\\s*", "")
		...
		Here: for (int i = copy.size() - 1; i > -1; i--) {
			...
			tableData[new_count] = new Object[] { new_count + 1, score_code[i],
					score[i],
					dic_chengfen_danwei.get(score[i]).toString()
							.replaceAll("\\s*", ""),
					...
					dic_bu_liang_fan_ying.get(score[i]).toString()
							.replaceAll("\\s*", ""),
					dic_yao_wu_xiang_hu_zuo_yong.get(score[i]).toString()
							.replaceAll("\\s*", ""),
