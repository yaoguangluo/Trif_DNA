...
public class XyscDictionary extends Dictionary_S {
    public void txtToMap(IMV_SIQ dic_yao_ming
        , IMV_SIQ dic_chengfen_danwei, IMV_SIQ dic_yong_fa
        , IMV_SIQ dic_yao_li, IMV_SIQ dic_zhu_yi
        ...
        InputStream in = new BookIndex().getClass().getResourceAsStream(S_File._127xiYiChangYongYaoPinGongNengZhuZhi_txt);
        ...
        in = new BookIndex().getClass().getResourceAsStream(S_File.xiYiChangYongYao_txt);
        ...
        while ((ctempString = cReader.readDetaLine()) != null) {
            if (!ctempString.replace(" ", "").equals("")) {
                if (ctempString.contains("西药类")) {
                    ...
                    if (temps.length > 1) {
                        if (!dic_jie_shao.containsKey(yaoming)) {
                            dic_jie_shao.put(yaoming, temps[1].replaceAll("\\s*"
                                    , "").replace("〔", "")
                                .replace("〕", ":"));
                        } else {
                            dic_jie_shao.put(yaoming, dic_jie_shao.get(yaoming)
                ...
                if (ctempString.contains("用法")) {//later。。
                    if (!dic_yong_fa.containsKey(yaoming.replaceAll("\\s*", "")
                        .replace("〔", "").replace("〕", ":"))) {
                        dic_yong_fa.put(yaoming.replaceAll("\\s*", "")
                                .replace("〔", "")
                                .replace("〕", ":")
                ...
                if (ctempString.contains("注意事项")) {
                    if (!dic_zhu_yi.containsKey(yaoming.replaceAll("\\s*", "")
                        .replace("〔", "").replace("〕", ":"))) {
                        dic_zhu_yi.put(yaoming.replaceAll("\\s*", "")
                                .replace("〔", "")
                                .replace("〕", ":")
                            , ctempString.replaceAll("\\s*", "")
                                .replace("〔", "")
                ...
                if (ctempString.contains("此外")) {
                    if (!dic_qi_ta.containsKey(yaoming.replaceAll("\\s*", "")
                        .replace("〔", "").replace("〕", ":"))) {
                        dic_qi_ta.put(yaoming.replaceAll("\\s*", "")
                                .replace("〔", "")
                                .replace("〕", ":")
                            , ctempString.replaceAll("\\s*", "")
                                .replace("〔", "")
        ...
        cReader.closeDeta();
        in = new BookIndex().getClass().getResourceAsStream(S_File.yaoLiXue_DiQiBan_txt);
        ...
        while ((ctempString = cReader.readDetaLine()) != null) {
            if (!ctempString.replace(" ", "").equals("")) {
                if (ctempString.contains("西药类")) {
                    ...
                    temps += ctempString.replaceAll("\\s*", "")
                        .replace("〔", "").replace("〕", ":");
                }
            }
        }
        ...
        in = new BookIndex().getClass().getResourceAsStream(S_File.linChuangYaoWuShouCe_txt);
        cReader = new DetaBufferedReader(DetaInputStreamReader.E(in, "UTF8"));
        ctempString = null;
        while ((ctempString = cReader.readDetaLine()) != null) {
            if (!ctempString.replace(" ", "").equals("")) {
        ...
        for (int i = 0; i < temp_list.size(); i++) {
            if (temp_list.get(i).contains("西药类")) {
                ...
                med_name = med_name.replaceAll("\\s*", "")
                    .replace("〔", "").replace("〕", ":");
                if (!dic_map.containsKey(med_name)) {
                    dic_map.put(med_name, (med_text + "〔〔").replace("\\s+", " "));
                } else {
                    dic_map.put(med_name, dic_map.get(med_name) + (med_text + "〔〔")
                        .replace("\\s+", " "));
                }
                ...
                if (!dic_yao_li.containsKey(med_name)) {
                    ...
                    dic_yao_li.put(med_name, dic_yao_li.get(med_name)
                        + temps.replaceAll("\\s*", "")
                        .replace("〔", "").replace("〕", ":"));
                }
            }
        ...
        for (int i = 0; i < copy.size(); i++) {
            ...
            if (m.find()) {
                temp = m.group(0);
                if (!dic_chengfen_danwei.containsKey(med_name)) {
                    dic_chengfen_danwei.put(med_name, temp.replaceAll("\\s*"
                            , "").replace("〔", "")
                        .replace("〕", ":"));
                } else {
                    dic_chengfen_danwei.put(med_name, dic_chengfen_danwei.get(med_name)
                        + temp.replaceAll("\\s*", "")
                        .replace("〔", "").replace("〕", ":"));
                }
            }
            if (m1.find()) {
                ...
                if (!dic_shi_ying.containsKey(med_name)) {
                    ...
                    dic_shi_ying.put(med_name, dic_shi_ying.get(med_name)
                        + temp.replaceAll("\\s*", "")
                        .replace("〔", "").replace("〕", ":"));
                }
            }
            if (m2.find()) {
                ...
                if (!dic_zhu_yi.containsKey(yaoming)) {
                    ...
                    dic_zhu_yi.put(med_name, dic_zhu_yi.get(med_name)
                        + temp.replaceAll("\\s*", "")
                        .replace("〔", "").replace("〕", ":"));
                }
            }
            if (m3.find()) {
                ...
                if (!dic_yong_liang.containsKey(med_name)) {
                    ...
                    dic_yong_liang.put(med_name, dic_yong_liang.get(med_name)
                        + temp.replaceAll("\\s*", "")
                        .replace("〔", "").replace("〕", ":"));
                }
            }
            if (m4.find()) {
                ...
                if (!dic_yao_wu_xiang_hu_zuo_yong.containsKey(med_name)) {
                    ...
                    dic_yao_wu_xiang_hu_zuo_yong.put(med_name, dic_yao_wu_xiang_hu_zuo_yong.get(med_name)
                        + temp.replaceAll("\\s*", "")
                        .replace("〔", "").replace("〕", ":"));
                }
            }
            if (m5.find()) {
                temp = m5.group(0);
                if (!dic_bu_liang_fan_ying.containsKey(med_name)) {
                    dic_bu_liang_fan_ying.put(med_name, temp.replaceAll("\\s*", "")
                        .replace("〔", "").replace("〕", ":"));
                } else {
                    dic_bu_liang_fan_ying.put(med_name, dic_bu_liang_fan_ying.get(med_name)
                        + temp.replaceAll("\\s*", "")
                        .replace("〔", "").replace("〕", ":"));
                }
            }
