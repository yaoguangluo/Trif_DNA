...
public class ZynkxDictionary extends Dictionary_S {
    public List<String> txtToList() {
        ...
        InputStream in = new BookIndex().getClass().getResourceAsStream(S_File.n_txt);
        ...
        in = new BookIndex().getClass().getResourceAsStream(S_File.zhongYiFangJiXue_txt);
        cReader = new DetaBufferedReader(DetaInputStreamReader.E(in, "UTF8"));
        ctempString = null;
        while ((ctempString = cReader.readDetaLine()) != null) {
            if (!ctempString.replace(" ", "").equals(""))
        ...
        for (int i = 0; i < dic_list.size(); i++) {
            ...
            if (dic_list.get(i).contains("..")) {
                ...
                if (!med_text.contains("中医大辞典")) {
                    dic_map.put(med_name.replaceAll("\\s*", "")
                            .replace("〔", "").replace("〕", ":")
                        , med_text.replaceAll("\\s*", "").replace("〔", "").replace("〕", ":"));
                }
            }
    ...
    public IMV_SIQ MapToChuFang(List<String> dic_list
        , IMV_SIQ dic_map) {
        ...
        for (int i = 0; i < dic_list.size(); i++) {
            if (dic_list.get(i).contains("^")) {
                ...
                if (dic_map.containsKey(med_name)) {
                    ...
                    dic_chu_fang.put(med_name.replaceAll("\\s*", "")
                            .replace("〔", "").replace("〕", ":")
                        , med_text.replaceAll("\\s*", "").replace("〔", "").replace("〕", ":"));
                }
            }
