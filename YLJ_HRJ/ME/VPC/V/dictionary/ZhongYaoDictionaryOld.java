	...
	public IMV_SIQ_ listToMap(List<String> dic_list) {
		...
		for (int i = 0; i < dic_list.size(); i++) {
			if (dic_list.get(i).contains("摘要")) {
				...
				dic_map.put(med_name.replaceAll(" ", "").replace("〔", "")
						.replace("〕", ":"), med_text + "〕〕");
			}
			...
			if (dic_list.get(i).contains("^^")) {
				...
				if (!dic_map.containsKey(med_name.replaceAll(" ", "")
						.replace("〔", "").replace("〕", ":"))) {
					dic_map.put(med_name.replaceAll(" ", "").replace("〔", "")
							.replace("〕", ":"), med_text + "〕〕");
				}
...
//later..
			if (!dic_li.containsKey(med_name.replaceAll("\\s*", "")
					.replace("〔", "").replace("〕", ":"))) {
				dic_li.put(
	...
	public IMV_SIQ mapToMap_hai(IMV_SIQ dic_map) {
		...
		for (int i = 0; i < copy.size(); i++) {
			...
			if (!dic_hai.containsKey(med_name.replaceAll("\\s*", "")
					.replace("〔", "").replace("〕", ":"))) {
				dic_hai.put(
						med_name.replaceAll("\\s*", "").replace("〔", "")
								.replace("〕", ":"),
	...
	public IMV_SIQ mapToMap_xz(IMV_SIQ dic_map) {
		...
		for (int i = 0; i < copy.size(); i++) {
			...
			if (!dic_xz.containsKey(med_name.replaceAll("\\s*", "")
					.replace("〔", "").replace("〕", ":"))) {
				dic_xz.put(
						med_name.replaceAll("\\s*", "").replace("〔", "")
								.replace("〕", ":"),
