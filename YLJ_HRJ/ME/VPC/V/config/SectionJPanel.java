...
public class SectionJPanel extends SectionJPanel_X_S {
	...
	public SectionJPanel(App app) {
				...
				for (int i = 0; i < components.length; i++) {
					...
					if (pageNameString.contains("临时")) {
						if (newPages.getSelectedItem() != null) {
							String target = newPages.getSelectedItem()
									.toString();
							if (app.app_S.tempPages.containsKey(target)) {
								Object componentTemp = app.app_S.tempPages
										.get(target);
								// if is xls
								CommonXls commonXls = (CommonXls) componentTemp;
								// app.app_S.jTabbedpane.remove(commonXls);
								// app.app_S.jTabbedpane.addTab(commonXls.getName(),
								// new ImageIcon()
								// , commonXls, commonXls.getName());//
								// app.app_S.jTabbedpane.setSelectedComponent(commonXls);
								// pagehookJCheckBoxMap.put(commonXls.getName(),
								// true);//
								if (pagehookJCheckBoxMap
										.containsKey(commonXls.getName())) {
									pagehookJCheckBox
											.setSelected(pagehookJCheckBoxMap
													.get(commonXls.getName()));
								} else {
									pagehookJCheckBoxMap
											.put(commonXls.getName(), false);
		...
		xlsWithSpecLabel.setBounds(10 + 130 * 5 - 50, 20 + 25 * 2 + 8 + 120,
				300, 50);
		...
		M_V.MVQ.button.DetaButton addxls_button = new M_V.MVQ.button.DetaButton(
				"添加XLS:");
		...
		addxls_button.addActionListener(e -> {
			try {
				...
				MakeContainerXLS makeContainerXLS = new MakeContainerXLS(
						...
						app.app_S.tabNames,
						app.app_S.fMHMMListOneTime_E_X_S.posCnToCn,
						...
						app.app_S.emotionSample, app.app_S.ratioMap_E,
						app.app_S.sensingTest, NE);
		...
		M_V.MVQ.button.DetaButton deletexlsx_button = new M_V.MVQ.button.DetaButton(
				"剔除XLS:");
		deletexlsx_button.setBounds(10 + 130 * 3 - 20, 20 + 25 * 3 + 120, 100,
				30);
		deletexlsx_button.addActionListener(e -> {
			...
			Component[] jTabbedpaneComponents = NE.app_S.jTabbedpane
					.getComponents();
		...
		M_V.MVQ.button.DetaButton deletexls_button = new M_V.MVQ.button.DetaButton(
				"导出XLS:");
		deletexls_button.setBounds(10 + 130 * 4 - 40, 20 + 25 * 3 + 120, 100,
				30);
		deletexls_button.addActionListener(e -> {
			FileDialog filedialog = new FileDialog(new Frame(), "Export Path:",
					FileDialog.LOAD);
		...
		pagehook.setBounds(10 + 130 * 5 - 50 - 80, 20 + 25 * 0 + 8 + 120, 300,
				50);
		pagehookJCheckBox = new JCheckBox();
		pagehookJCheckBox.setBounds(10 + 130 * 5 + 30 - 90, 20 + 25 * 1 + 120,
				20, 20);
		...
		pagehookJCheckBox.addActionListener(arg0 -> {
			...
			if (pagehookJCheckBox.isSelected()) {
				...
				Iterator<String> iterators = app.app_S.tempPages.keySet()
						.iterator();
				while (iterators.hasNext()) {
					...
					if (tempPage.contains("临时")) {
						if (targetPage.equalsIgnoreCase(tempPage)) {// 准备加载
							if (newPages.getSelectedItem() != null) {
								if (pagehookJCheckBoxMap
										.containsKey(tempPage)) {
									if (pagehookJCheckBoxMap.get(tempPage)) {// true
										return;
									}
								} else {
									pagehookJCheckBoxMap.put(tempPage, true);// nofind
																			 // init
								...
								// component下层函数不提供
								// 导致我还要把这个函数做成专门加xls的。
								// app.app_S.jTabbedpane.addTab(tempPage, new
								// ImageIcon()
								// , component, tempPage);
								// 因为java awt的 component到object 层是隐藏，所以在object和
								// 新类之间变换会class invalid，我又还改不了
								// 改不了又看不到函数结构，用已有函数
								// 就像彩票中奖一样，于是从最重的高级类开始。vpcs价值就出来
								// 依赖删除仅仅，资源还在，sleeper替换了而已。
								// Container container= new Container();
								MakeContainerXLS makeContainerTemp = new MakeContainerXLS(
										component, app.app_S._A, app.app_S,
										app.app_S.jTabbedpane,
										app.app_S.tabNames,
										app.app_S.fMHMMListOneTime_E_X_S.posCnToCn,
										app.app_S.fMHMMListOneTime_E_X_S.posEnToEn,
										app.app_S.fMHMMListOneTime_E_X_S.enToCn,
					...
					if (pageNameString.contains("临时")) {
						if (newPages.getSelectedItem() != null) {
							if (targetPage
								...
								if (app.app_S.tempPages
										.containsKey(component.getName())) {
									if (!pagehookJCheckBoxMap
											.containsKey(component.getName())) {
										pagehookJCheckBoxMap.put(
									...
									// if(Object is commonXLS)
									// CommonXls commonXls = (CommonXls)
									// component;
									// app.app_S.tempPages.put(commonXls.getName(),
									// commonXls);
									app.app_S.jTabbedpane.remove(component);// remove
																			// 后之前操作
																			// 就没了。所以先put下。
		...
		qian_duan_jie_kouJTextField.setBounds(10 + 0 * 130 + 120,
				20 + 25 * 1 + 120, 50, 25);
		qian_duan_jie_kouJTextField.addActionListener(
				arg0 -> appInThisClass.app_S.frontendTxt = qian_duan_jie_kouJTextField
						.getText().trim());
		qian_duan_jie_kouJTextField.addKeyListener(new KeyListener() {
			...
			public void keyReleased(KeyEvent e) {
				appInThisClass.app_S.frontendTxt = qian_duan_jie_kouJTextField
						.getText().trim();
			}
		});
		...
		hou_duan_jie_kouJTextField.setBounds(10 + 0 * 130 + 120,
				20 + 25 * 2 + 120, 50, 25);
		hou_duan_jie_kouJTextField.addActionListener(
				arg0 -> appInThisClass.app_S.backendTxt = hou_duan_jie_kouJTextField
						.getText().trim());
		hou_duan_jie_kouJTextField.addKeyListener(new KeyListener() {
			...
			@Override
			public void keyReleased(KeyEvent e) {
				appInThisClass.app_S.backendTxt = hou_duan_jie_kouJTextField
						.getText().trim();
			}
		});
		...
		shu_ju_ku_jie_kouJTextField.setBounds(10 + 0 * 130 + 120,
				20 + 25 * 3 + 120, 50, 25);
		shu_ju_ku_jie_kouJTextField.addActionListener(
				arg0 -> appInThisClass.app_S.databasePortTxt = shu_ju_ku_jie_kouJTextField
						.getText().trim());
		shu_ju_ku_jie_kouJTextField.addKeyListener(new KeyListener() {
			...
			@Override
			public void keyReleased(KeyEvent e) {
				appInThisClass.app_S.databasePortTxt = shu_ju_ku_jie_kouJTextField
						.getText().trim();
			}
		});
		...
		huan_cun_jie_kouJTextField.setBounds(10 + 0 * 130 + 120,
				20 + 25 * 4 + 120, 50, 25);
		huan_cun_jie_kouJTextField.addActionListener(
				arg0 -> appInThisClass.app_S.huan_cun_jie_kouTxt = huan_cun_jie_kouJTextField
						.getText().trim());
		huan_cun_jie_kouJTextField.addKeyListener(new KeyListener() {
			...
			@Override
			public void keyReleased(KeyEvent e) {
				appInThisClass.app_S.huan_cun_jie_kouTxt = huan_cun_jie_kouJTextField
						.getText().trim();
			}
		});
		...
		lan_ya_jie_kouJTextField.setBounds(10 + 0 * 130 + 120,
				20 + 25 * 5 + 120, 50, 25);
		lan_ya_jie_kouJTextField.addActionListener(
				arg0 -> appInThisClass.app_S.lan_ya_jie_kouTxt = lan_ya_jie_kouJTextField
						.getText().trim());
		lan_ya_jie_kouJTextField.addKeyListener(new KeyListener() {
			...
			@Override
			public void keyReleased(KeyEvent e) {
				appInThisClass.app_S.lan_ya_jie_kouTxt = lan_ya_jie_kouJTextField
						.getText().trim();
			}
		});
		...
		Tel_zhong_duan_jie_kou.setBounds(10 + 0 * 130, 20 + 25 * 6 + 120, 150,
				25);
		JTextField Tel_zhong_duan_jie_kouJTextField = new JTextField();
		Tel_zhong_duan_jie_kouJTextField.setBounds(10 + 0 * 130 + 120,
				20 + 25 * 6 + 120, 50, 25);
		Tel_zhong_duan_jie_kouJTextField.addActionListener(
				arg0 -> appInThisClass.app_S.Tel_zhong_duan_Txt = Tel_zhong_duan_jie_kouJTextField
						.getText().trim());
		Tel_zhong_duan_jie_kouJTextField.addKeyListener(new KeyListener() {
			...
			@Override
			public void keyReleased(KeyEvent e) {
				appInThisClass.app_S.Tel_zhong_duan_Txt = Tel_zhong_duan_jie_kouJTextField
						.getText().trim();
			}
		});
		...
		jlabel_peizhi_di21.setBounds(10 + 85 + 0 * 150, 33 + 25 * 11 - 300, 20,
				20);
		...
		jlabel_peizhi_di2122.setBounds(10 + 85 + 1 * 150, 33 + 25 * 11 - 300,
				20, 20);
		...
		jlabel_peizhi_di2123.setBounds(10 + 85 + 2 * 150, 33 + 25 * 11 - 300,
				20, 20);
		...
		jlabel_peizhi_di21231.setBounds(10 + 85 + 3 * 150, 33 + 25 * 11 - 300,
				20, 20);
		...
		jlabelpeizhi2123111.setBounds(10 + 3 * 150, 20 + 25 * 12 - 300, 300,
				50);
		...
		cui_hua_bi_zhi.addChangeListener(event -> {
			JSlider source = (JSlider) event.getSource();
			appInThisClass.app_S.catalyticRot = source.getValue();
			appInThisClass.keyReleased(null);
		});
		cui_hua_bi_zhi.setBounds(10 + 85 + 3 * 150, 33 + 25 * 12 - 300, 170,
				20);
		...
		jlabel_peizhi_di212311.setBounds(10 + 85 + 4 * 150, 33 + 25 * 11 - 300,
				20, 20);
		...
		jlabel_peizhi_di22.setBounds(10 + 85 + 0 * 150, 33 + 25 * 12 - 300, 20,
				20);
		...
		jlabel_peizhi_di2233.setBounds(10 + 85 + 1 * 150, 33 + 25 * 12 - 300,
				20, 20);
		...
		jlabel_peizhi_di22331.setBounds(10 + 85 + 2 * 150, 33 + 25 * 12 - 300,
				20, 20);
		...
		jlabel_peizhi_di23.setBounds(10 + 85 + 0 * 150, 33 + 25 * 13 - 300, 20,
				20);
		...
		jlabel_peizhi_di23D.setBounds(10 + 85 + 1 * 150, 33 + 25 * 13 - 300, 20,
				20);
		...
		jlabel_peizhi_di23D1.setBounds(10 + 85 + 2 * 150, 33 + 25 * 13 - 300,
				20, 20);
		...
		jlabel_peizhi_di23D12.setBounds(10 + 85 + 3 * 150, 33 + 25 * 13 - 300,
				20, 20);
		...
		jlabelpeizhi23D1277.setBounds(10 + 3 * 150, 20 + 25 * 14 - 300, 300,
				50);
		JCheckBox jlabel_peizhi_di23D1277 = new JCheckBox();
		jlabel_peizhi_di23D1277.setBounds(10 + 85 + 3 * 150, 33 + 25 * 14 - 300,
				20, 20);
		...
		jlabel_peizhi_di23D123.setBounds(10 + 85 + 4 * 150, 33 + 25 * 13 - 300,
				20, 20);
		...
		jlabelpeizhi23D12377.setBounds(10 + 4 * 150, 20 + 25 * 14 - 300, 300,
				50);
		JCheckBox jlabel_peizhi_di23D12377 = new JCheckBox();
		jlabel_peizhi_di23D12377.setBounds(10 + 85 + 4 * 150,
				33 + 25 * 14 - 300, 20, 20);
		...
		jlabel_peizhi_di2355.setBounds(10 + 85 + 0 * 150, 33 + 25 * 14 - 300,
				20, 20);
		...
		jlabel_peizhi_di2356.setBounds(10 + 85 + 1 * 150, 33 + 25 * 14 - 300,
				20, 20);
		...
		jlabel_peizhi_di2357.setBounds(10 + 85 + 2 * 150, 33 + 25 * 14 - 300,
				20, 20);
		...
		jlabel_peizhi_di2511.setBounds(10 + 85 + 0 * 150, 33 + 25 * 15 - 300,
				20, 20);
		...
		jlabel_peizhi_di24.setBounds(10 + 85 + 1 * 150, 33 + 25 * 15 - 300, 20,
				20);
		...
		jlabel_peizhi_di2512.setBounds(10 + 85 + 2 * 150, 33 + 25 * 15 - 300,
				20, 20);
		...
		jlabel_peizhi_di2513.setBounds(10 + 85 + 3 * 150, 33 + 25 * 15 - 300,
				20, 20);
		...
		jlabel_peizhi_di2515 = new JCheckBox();
		jlabel_peizhi_di2515.setBounds(10 + 85 + 4 * 150, 33 + 25 * 15 - 300,
				20, 20);
		...
		boot_button.addActionListener(e -> {
			if (appInThisClass.app_S.appConfig.SectionJPanel.jlabel_peizhi_di2122
					.isSelected()) {
				jTextPane.setText("正在载入 华瑞集 DNN 索引 5.0.0.0. 系统 . . .");
				if (null == appInThisClass.app_S.DNNmap
						|| appInThisClass.app_S.DNNmap.isEmpty()) {
					appInThisClass.app_S.DNNmap = GetDNNMap
							.doGetDNNMap(appInThisClass.app_S, NE);
				}
			}
			tabNamesHook[0] = true;
			isConfig = false;
		...
		M_V.MVQ.button.DetaButton jlabel_flush_button = new M_V.MVQ.button.DetaButton(
				"异常刷新:");
		jlabel_flush_button.setBounds(10 + 100 + 30 + 120 + 140,
				460 - 300 + 120, 100, 30);
		...
		jsp_jTextPane.setBounds(10 + 0 * 150, 20 + 25 * 15 + 120 - 180, 750,
				350);
