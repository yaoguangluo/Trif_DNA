...
public class AppButtonUtil_X_DNN_Page extends AppButtonUtil_X_DNN {
    ...
    public static void pageDNN(StringBuilder page, List<String> sets, int currentPage
        , App_S u, JTable table, int row, String key
        , DetaButton buttonSum, DetaButton buttonCrt, JTextPane data
        , JTextPane statistic, String zhong_xi_yao, App NE) {//shaohou zhenghe danli
        ...
        if (!u.CacheString.containsKey(keyCache)) {
            String string = String_ESU_X_charsetSwap._E(
                setOfi, "UTF8", "UTF8");
        ...
        if (zhong_xi_yao.contains("中药")) {
            ...
            u.coAuthorForWord.bootDetaDnnFlowerForest(table.getValueAt(row
                , 2).toString(), dnn, true);
        } else if (zhong_xi_yao.contains("西药")) {
            u.coAuthorForWord.bootDetaBingMingDnnFlowerForest(table.getValueAt(row
                , 2).toString(), dnn, true);
        } else {
        }
