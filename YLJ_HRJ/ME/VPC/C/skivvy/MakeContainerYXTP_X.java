...
public class MakeContainerYXTP_X extends Thread implements Runnable {
    ...
    public MakeContainerYXTP_X(Container jpanelFourth, List<String> tabNames2, App NE) {
        super();
    ...
    public void AOP_VEC_SQ_PDE(List<String> pathRankList) {
        ...
        while (iteratorPathRankList.hasNext()) {
            ...
            if (strings.length > 1) {
                map.put(StringPath.split(S_ShellETL.SHELL_ETL_GREATER)[1], true);
            }
        }
    ...
    public LinkedList<String> sort(List<String> copy, String key, App NE) {
        ...
        while (iteratorForCopy.hasNext()) {
                ...
                if (temps.contains(mapSearchaAtII)) {
                    ...
                    score_code[copyCount] += (iteratorForCopyString.contains(mapSearchaAtII) ? 2 : 1)
                        * (!S_Maps.mingCi.containsKey(mapSearchaAtII)
                        ? S_Maps.dongCi.containsKey(mapSearchaAtII) ? 50 : 1 : 50)
            ...
            if (key.length() > 4) {
                ...
                score_code[copyCount] = (int) (tempa / Math.pow(this.app.lookrot + 1
                    , 4) + tempb * Math.pow(this.app.lookrot, 2));
            }
