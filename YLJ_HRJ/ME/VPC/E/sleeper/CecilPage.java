...
public class CecilPage extends CecilPage_X_S_sListeners {
	...
	public CecilPage(JTextPane text, App NE)
	...
	public JTextPane data(App NE) throws InterruptedException,
		IOException {
		data = new JTextPane();
		data.setBounds(850, 150, WindowsUI.basicWidth - 600, 800
			- 70);
		...
		NE._I_U.string._S_ = "共有 " + (sets == null ? 0
			: (1 + sets.size() / 2001)) + " 页";
		// 因为异步，所以支持。
		...
		buttonNext = Page_X_S_sButtonsrollafter.data_X_rollafter(this,
			2000, NE);
		buttonCTE = Page_X_S_sButtonschineseAnnotation
			.data_X_chineseAnnotation(this, NE);
		buttonFRS = Page_X_S_sButtonsreturnEnglish
			.data_X_returnEnglish(this, NE);
		// buttonNext =
		// Page_X_S_sButtonsrollafter.data_X_rollafter(this
		//                , 410, 0, 100, 30, 2000);
		// buttonCTE =
		// Page_X_S_sButtonschineseAnnotation.data_X_chineseAnnotation(this
		//                , 410, 0, 100, 30, 2000);
		//        buttonFRS = Page_X_S_sButtonsreturnEnglish.data_X_returnEnglish(this
		//            , 410, 0, 100, 30, 2000);
		buttonETC = Page_X_S_sButtonsreading.data_X_reading(this,
			app_S, NE);
		...
		buttonADD.addActionListener(e -> {
			...
			List<String> setsForGet = sets.subList(currentPage * 2000,
				(currentPage + 1) * 2000 < sets.size() ? (currentPage
					+ 1) * 2000 : sets.size());
		...
		buttonBox.setBounds(5 + 800 - 650, 290 + 100 - 80 + 200 - 260
			+ 120, 950, 20);
		this.add(buttonBox);
		return data;
	}
	...
	public JTextPane statistic() {
		...
		statistic.setBounds(850, 150, WindowsUI.basicWidth - 840,
			800);
		return statistic;
	}
}