...
*  208号 阳光家园别墅小区 第十栋别墅
 * */
public class Page_X_S_sButtonssymnorine {
    public static DetaButton data_X_symnorine(Page_X_S_sVSQ page_X_S_s
        , int x, int y, int weight, int height, int maxInPage, App NE) {
        DetaButton buttonETC = new DetaButton("同义");
        buttonETC.setBounds(x, y, weight, height);
        buttonETC.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                List<String> setsForGet = page_X_S_s.sets.subList(page_X_S_s.currentPage
                    * maxInPage, (page_X_S_s.currentPage + 1) * maxInPage
                    < page_X_S_s.sets.size() ? (page_X_S_s.currentPage + 1)
                ...
                while (iterator.hasNext()) {
                    ...
                    if (page_X_S_s.pos.get(setOfi) == null) {
                        page.append("<span style=\"background:#ffffff\">"
                            ...
                            + (page_X_S_s.cte.containsKey(setOfi) ? (page_X_S_s.etc
                            .containsKey(page_X_S_s.cte.get(setOfi)) ? ":(" + page_X_S_s
                            .etc.get(page_X_S_s.cte.get(setOfi)) + ")" : "") : "")
                            + "</font></span>");
                        continue Here;
                    }
                    if (!setOfi.equals("")) {
                        if (page_X_S_s.key.contains(setOfi)
                            ...
                            page.append("<span style=\"background:red\">"
                                ...
                                ? (page_X_S_s.etc.containsKey(page_X_S_s.cte.get(setOfi))
                                ? ":(" + page_X_S_s.etc.get(page_X_S_s.cte.get(setOfi))
                                + ")" : "") : "") + "</font></span>");
                            continue Here;
                        }
                        if (S_Maps.mingCi.containsKey(setOfi)) {
                            page.append("<span style=\"background:" + Color_P.P(255, 215 - NE.app_S.raterot << 3
                                , 255) + "\"><font color=" + Color_P.P(0, 0, 0) + " size=\"" + NE
                                .app_S.size_font_rot + "\">" + setOfi + (page_X_S_s.cte
                                .containsKey(setOfi) ? (page_X_S_s.etc.containsKey(
                                page_X_S_s.cte.get(setOfi)) ? ":(" + page_X_S_s.etc
                                .get(page_X_S_s.cte.get(setOfi)) + ")" : "") : "")
                                + "</font></span>");
                            continue Here;
                        }
                        if (page_X_S_s.pos.getString(setOfi).contains("动")) {
                            page.append("<span style=\"background:" + Color_P.P(255 - NE.app_S.raterot << 3, 255
                                , 255 - 40 - NE.app_S.raterot << 3)
                                + "\"><font color=" + Color_P.P(0
                                , 0, 0) + " size=\"" + NE.app_S
                                .size_font_rot + "\">" + setOfi + (page_X_S_s.cte
                                .containsKey(setOfi) ? (page_X_S_s.etc.containsKey(
                                page_X_S_s.cte.get(setOfi)) ? ":(" + page_X_S_s.etc
                                .get(page_X_S_s.cte.get(setOfi)) + ")" : "") : "")
                                + "</font></span>");
                        ...
                        if (page_X_S_s.pos.getString(setOfi).contains("形")) {
                            page.append("<span style=\"background:" + Color_P.P(255, 255, 215 - NE.app_S
