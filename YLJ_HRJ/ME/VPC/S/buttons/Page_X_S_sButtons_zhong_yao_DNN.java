...
public class Page_X_S_sButtons_zhong_yao_DNN {
    public static DetaButton data_X_zhong_yao_DNN(Page_X_S_sVSQ page_X_S_s, int x, int y
        , int weight, int height, int maxInPage, int max, App NE) {
...
//						response= RestCall_X_backEndRequest._E(encode);
                    ...
                    if (page_X_S_s.app_S.CacheStringPiple.size() < 301) {//buman
                        ...
                        page_X_S_s.app_S.CacheString.put(keyCache, response);
                        page_X_S_s.app_S.CacheStringPiple.add(0, keyCache);
                        page_X_S_s.app_S.CacheString.remove(page_X_S_s.app_S
                            .CacheStringPiple.get(300));
                ...
                String[] dnn = response.replace("\r\n"
                    , "<br/>").split("<br/>");
                page_X_S_s.app_S.coAuthorForWord.bootDetaDnnFlowerForest(page_X_S_s
                        .table.getValueAt(page_X_S_s.row, 2).toString()
                    , dnn, true);
                ...
                page_X_S_s.buttonSum.setText("共有 "
                    + (page_X_S_s.sets == null
                    ? 0 : (1 + page_X_S_s.sets.size() / 2001)) + " 页");
                page_X_S_s.buttonCrt.setText("当前页面："
                    + (page_X_S_s.sets == null ? 0 : 1));
                ...
                for (int i = fwa.length - 1; i > 0; i--) {
                    if (fwa[i] != null) {
                        if (page_X_S_s.pos.getString(fwa[i].split(":")[0]) == null) {
                        ...
                        if (S_Maps.xingRongCi.containsKey(fwa[i].split(":")[0])) {
