...
*  208号 阳光家园别墅小区 第十栋别墅
 * */
public class Page_X_S_sButtonschinese {
	public static DetaButton data_X_chinese(Page_X_S_sVSQ page_X_S_s, int x,
			int y, int weight, int height, int maxInPage, App NE) {
		DetaButton buttonFRS = new DetaButton("chinese");
					...
					if (!setOfi.equals("")) {
						...
						if (S_Maps.dongCi.containsKey(setOfi)) {
							page.append("<span style=\"background:" + Color_P.P(
									245 - page_X_S_s.app_S.raterot << 3, 255,
						...
						if (S_Maps.fuCi.containsKey(setOfi)) {
							page.append("<span style=\"background:"
									+ Color_P.P(
											245 - page_X_S_s.app_S.raterot << 3,
											255, 255)
									+ "\">" + "<font color="
									+ Color_P.P(0, 0, 0) + " size=\""
									+ NE.app_S.size_font_rot + "\">" + setOfi
