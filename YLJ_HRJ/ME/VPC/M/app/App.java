...
public class App extends JApplet implements KeyListener, ActionListener {
    ...
    public void keyReleased(KeyEvent arg0) {
        ...
        if (this.app_S.appConfig.SectionJPanel.jlabel_peizhi_di2233.isSelected()
            || this.app_S.appConfig.SectionJPanel.jlabel_peizhi_di2122.isSelected()) {
            //识别回车 DNN 和 专家caoZuo
            if (null == arg0) {
                return;
            }
        ...
        this.app_S.appConfig.SectionJPanel.jTextPane.setText("" + app_S.lookrot);
        ...
        app_S.key = app_S.key.length() == 0 ? S_Tag.STRING_EMPTY
            : app_S.translator.MixedStringToChineseString(app_S._A, app_S.key);
        ...
        if (this.app_S.appConfig.SectionJPanel.jlabel_peizhi_di2233.isSelected()) {
            //literial测试
            keyZhenDuan = literial_X_P_BaGangBianZheng.P_BaGangBianZheng(app_S.key);
        }
        ...
        if (string.length() > 0) {
            ...
            for (int i = 0; i < this.app_S.searchList.size(); i++) {
                app_S.searchList.get(i).AOP_VEC_SQ(null
                    , zhongyao.toString(), this);
            }
            for (int i = 0; i < this.app_S.makeContainerXLSList.size(); i++) {
                try {
                    MakeContainerXLS makeContainerXLS = app_S.makeContainerXLSList.get(i);
                    ...
                    e.printStackTrace();
                    this.app_S.appConfig.SectionJPanel.jTextPane.setText("error");
                }
            }
            ...
            if (this.app_S.appConfig.SectionJPanel.jlabel_peizhi_di2233.isSelected()) {
                ...
                //String 文学key = literial_X_words_P.words_P(this, app_S.key);
                String wen_xuekey = literial_X_literial_liner_P.literial_liner_P(this.app_S, app_S.key);
                //稍后七情六淫八纲都入文学库。
                String wen_xue_zhongyao = zhongyao
                    + literial_X_literial_P.literial_P(this.app_S, zhongyao);
                ZhongYaoSearch.zhongYaoSearch(this.app_S, wen_xue_zhongyao, forE, wen_xuekey, this);
                //搜索静态化, 方便DNA元基索引编码
            } else {
                ZhongYaoSearch.zhongYaoSearch(this.app_S, zhongyao, forE, app_S.key, this);
            }
