...
public class AppInit_XCDX {
	...
	public void IV_(App app, App NE) throws IOException, InterruptedException {// later duplicate
		...
		app.app_S.frameConfig.getContentPane().add(app.app_S.appConfig);
		...
		app.app_S.jTabbedpane = new DetabbedPane(1000 - 60, 20, Color.black,
				NE);
...
//		serverInitController.initServer(app);
		...
		int leftRange = 10;
		JTextField jTextField = app.app_S.app_XCDX.name(NE);
		JScrollPane jScrollPane = new JScrollPane(jTextField);
		app.app_S.jsp_name = jScrollPane;
		...
		app.app_S.rate.addMouseListener(new MouseListener() {
			...
			public void mouseReleased(MouseEvent arg0) {
				...
				app.app_S.makeContainer_ZHONG_YAO.zhongYaoTableSimple
						.mouseClicked(arg0);
		...
		app.app_S.size_font_slider.setBounds(553 - leftRange * 3, 36, 50, 20);
		app.app_S.density_bloosm = new JLabel("词花密度:");
		app.app_S.density_bloosm.setForeground(Color.yellow);
		app.app_S.density_bloosm.setBounds(493 - 3 * leftRange, -7, 100, 50);
		...
		app.app_S.color_bg_slider.addChangeListener(event -> {
		});
		app.app_S.color_bg_slider.addMouseListener(new MouseListener() {// later
																		// in
																		// new
																		// panel.
			@Override
		...
		app.app_S.color_bg_slider.setBounds(674 - (leftRange << 2), 36, 50, 20);
		// 稍后缩进
		app.app_S.read_speed = new JLabel("语速快慢:");
		app.app_S.read_speed.setForeground(Color.yellow);
		app.app_S.read_speed.setBounds(614 - (leftRange << 2), -7, 100, 50);
		...
		app.app_S.read_speed_slider.addChangeListener(event -> {
			JSlider source = (JSlider) event.getSource();
		...
		app.app_S.read_speed_slider.setBounds(674 - (leftRange << 2), 8, 50,
				20);
		...
		app.app_S.jScrollPane.setBounds(0, 0, 1335, 780 + app.app_S.rangeHigh);
		app.app_S.appConfig.SectionJPanel.jTextPane
				.setText(app.app_S.appConfig.SectionJPanel.jTextPane.getText()
		...
		app.app_S.makeContainer_ZHONG_YAO = new MakeContainer_ZHONG_YAO(
				jpanelFirst, NE);
		// p1
		...
		// app.app_S.jTabbedpane.setBackgroundAt(0, new Color(253,
		//// 233, 254));
		...
		SkinPathDetectTrip_E
				.initDeciphring(app.app_S.fourInitonJianTxt.isEmpty()
						? app.app_S.fourInitonJianTxt
						: "F_DB5.txt", 5);
		SkinPathDetectTrip_E
				.initDeciphring(app.app_S.fourInitonSuanTxt.isEmpty()
						? app.app_S.fourInitonSuanTxt
						: "F_DB6.txt", 6);
		SkinPathDetectTrip_E
				.initDeciphring(app.app_S.sixteenInitonJianTxt.isEmpty()
						? app.app_S.sixteenInitonJianTxt
						: "F_DB6_jian_17.txt", 17);
		SkinPathDetectTrip_E
				.initDeciphring(app.app_S.sixteenInitonJianTxt.isEmpty()
						? app.app_S.sixteenInitonSuanTxt
