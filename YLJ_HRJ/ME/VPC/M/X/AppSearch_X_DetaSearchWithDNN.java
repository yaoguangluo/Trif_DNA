...
public class AppSearch_X_DetaSearchWithDNN extends ScrollPane {
	...
	public int detaSimpleSearch(int[] score_code, String[] score,
			DefaultTableModel newTableModel, String key, boolean keyIsPCA,
			int[] identifyColumns, String fileNamePrefix, String[] scoreOutput,
			List<Object> tableData_oldListObjects, String identifyParser,
			App NE) {
		int count = 0;
		App_S u = NE.app_S;
		try {
			boolean dnnIsSelected = u.appConfig.SectionJPanel.jlabel_peizhi_di2122
					.isSelected();
			...
			for (int i = 0; i < stringReg.length; i++) {
				...
				int i3 = i / 3;
				stringReg[i].append(key.substring(i3,
						(i3 + 3) < keyLength ? (i3 + 3) : keyLength - 1));
			}
			...
			while (iterators.hasNext()) {
				...
				Here: while (iteratorWordFrequency.hasNext()) {
					...
					if (temp) {
						...
						if (posString.contains("名") || posString.contains("动")
								|| posString.contains("形")
								|| posString.contains("谓")) {
							reg[copyCount] += 2;
						}
						reg[copyCount] += 1;
						int wfs = (int) wordFrequencySearch.get_frequency();
				...
				for (int i = 1; i <= stringReg.length; i++) {// 断句
					if (iteratorForCopyString
							.contains(stringReg[i - 1].toString())) {
						last = last == i - 1 ? has++ : 0;
				...
				if (0 != jing_du_sou_suo_suan_zi) {
					score_code[copyCount] = (zhi_hui_sou_suo_suan_zi >> (tempLookrot
							- 2)
