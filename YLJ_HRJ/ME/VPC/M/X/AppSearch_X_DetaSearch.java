...
public class AppSearch_X_DetaSearch extends ScrollPane {
    ...
    public static int detaSearch(int[] score_code, String[] score
        ...
        while (iteratorForCopy.hasNext()) {
            ...
            String dai_sou_suo_ci_dian = dic_map.get(iteratorForCopyString).toString();
            ...
            if (u.appConfig.SectionJPanel.jlabel_peizhi_di2122.isSelected()) {
                ...
                if (null == dnnSet || dnnSet.isEmpty()) {
                    String[][] ann = aNNTest.getANNMatrix(dai_sou_suo_ci_dian, NE);
                    String[][] dnn = dNNTest.getDNNMatrix(ann, dai_sou_suo_ci_dian, NE);
            ...
            while (iteratorWordFrequency.hasNext()) {
                ...
                if (mapSearchaAtII.length() > 1) {
                    for (int j = 0; j < mapSearchaAtII.length(); j++) {
                        ...
                        if (dai_sou_suo_ci_dian.contains(String.valueOf(temp))) {
                            if (reg[copyCount] == 0) {
                                count += 1;
                            }
                            score[copyCount] = iteratorForCopyString;
            ...
            if (keyLength > 4) {
                ...
                for (int i = 0; i < string.length; i++) {
                    if (dai_sou_suo_ci_dian.contains(string[i])) {//因为用FrequencyMap代替, 所以要乘以频率。
...
//                        + jing_du_sou_suo_suan_zi * Math.pow(u.lookrot + 1, 2));
        ...
        //WriteDNNMap.doWriteDNNMap("C:\\Users\\yaogu\\Desktop\\project\\DNN19228.lyg", u) ;
        return count;
    }
}
//35
