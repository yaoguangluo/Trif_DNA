...
public class App_XCDX {
    ...
    public JTextField nameFeelFilter(App NE) {
        NE._I_U.string = new AES_QMS_XSD_TIH("");
        IMV_SIQ map = InitonTalk.listenFlower("执行 " + "M_VECS" + " 下 "
                + "CfxTextField" + " 接口, 参数是 传参因子", "CfxTextField"
            , "M_VECS", NE);
...
//		= new ImageIcon(new YaoCaiIndex().getClass().getResource(S_File.yc_2_1_png));
        app.app_S.pngy_2_1 = FastReadProjectFile.getDetaImageIconReader(S_File.yc_2_1_png
    ...
    public JTextPane data(App NE) {
        ...
        IMV_SIQ map = InitonTalk.listenFlower("执行 " + "M_VECS" + " 下 "
                + "CfxTextPane" + " 接口, 参数是 传参因子", "CfxTextPane"
            , "M_VECS", NE);
    ...
    public JTextField name(App NE) {
        ...
        app.app_S.name = (CfxTextField) map.get("interfaceReturn");
        app.app_S.name.setBounds(180 - 50, 50, 380, 80);
        app.app_S.name.setBackground(new Color(210, 210, 210));
        app.app_S.name.setText(S_Tag.STRING_SPACE);
        app.app_S.name.addKeyListener(app);
        return app.app_S.name;
    }

    public JTextField nameHospital(App NE) {
        NE._I_U.string = new AES_QMS_XSD_TIH("");
        IMV_SIQ map = InitonTalk.listenFlower("执行 " + "M_VECS" + " 下 "
                + "CfxTextField" + " 接口, 参数是 传参因子", "CfxTextField"
            , "M_VECS", NE);
    ...
    public JTextField name_filter(App NE) {
        NE._I_U.string = new AES_QMS_XSD_TIH("");
        IMV_SIQ map = InitonTalk.listenFlower("执行 " + "M_VECS" + " 下 "
                + "CfxTextField" + " 接口, 参数是 传参因子", "CfxTextField"
            , "M_VECS", NE);
    ...
    public JTextField name_filter_not_have(App NE) {//某种稳定的函数执行与固定搭配。
        NE._I_U.string = new AES_QMS_XSD_TIH("");
        IMV_SIQ map = InitonTalk.listenFlower("执行 " + "M_VECS" + " 下 "
                + "CfxTextField" + " 接口, 参数是 传参因子", "CfxTextField"
            , "M_VECS", NE);
        app.app_S.name_filter_not_have = (CfxTextField) map.get("interfaceReturn");
