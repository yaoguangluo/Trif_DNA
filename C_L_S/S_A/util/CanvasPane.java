 ...
 * 于是 数据帧long 对应list的double片段，图片帧仅仅做片段的前后10+内片段集进行展示，
 * 超过点击前滚或者后滚模式翻页。计算频率仅仅计算片段前后10+页内的当前屏幕显示的1024
 * 滑动显示焦点数据进行图片化分析即可。也就是说软件不需要修改，只要翻页录入即可，这样
 * 不美观，所以还是。
 * */
/*
 * 2 预计功能，首先我要一个1024的展示当前页面的功能，同时要记录完整波形，这里遇到一个
 * 问题，每秒钟至少有几十个1024，一分钟几千个1024，1小时电脑就会内存拓机。所以不可能
 * 全部画图，需要设置最大值封顶，所以这里出现一个option就是保留现在所有的当前画图功能，
 * 同时内部还有一个记录max封顶的循环队列隐藏记忆。，点击这个记忆，便可以list一样横幅
 * 观测完整波形。
 * */
/*
 * 3 预先设计 首先我需要设计一个面板替换原来的1024的展示。然后这个面板方便我在点击
 * 停止录制的时候会切换到横幅记忆这个记忆队列我需要设计成 前置丢失限长的linklist，
 * 长度限制在1024 00 00，100 00个1024即可, 不需要分页鼠标滑动画布横幅，频率计算
 * 当前屏幕的1024图片即可。
 * */
/*
