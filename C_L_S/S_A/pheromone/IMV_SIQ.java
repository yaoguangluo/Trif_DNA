...
public class IMV_SIQ extends ConcurrentHashMap {
    ...
    public ArrayList<AMV_MVS_VSQ_3D> getArrayList_AMV_MVS_VSQ_3D(double shortestDoubleScale) {
    ...
    public ArrayList<AMV_MVS_VSQ_2D> get_List_AMV_MVS_VSQ_2D(double doubleScale) {
        Double _doubleScale = Double.valueOf(doubleScale);
        if (this.containsKey(_doubleScale)) {
    ...
    public void couldSQLThenSQL(String key, String[] strings, TinMap output, App NE) {//7代花再缩减 flex
        for (String actionKey : (Iterable<String>) FlowerAction.FlowerP_E_KernelActions.keySet()) {
            if (key.contains(actionKey)) {//later separate.
                String temp = FlowerAction.FlowerP_E_KernelActions.getString(actionKey);
    ...
    //later for char
    public void couldDoThenDo(String key, String[] strings, TinMap output, App NE) {//7代花再缩减 flex
        Iterator<String> iterators = FlowerAction.FlowerSixDomainActions.keySet().iterator();
        while (iterators.hasNext()) {
            String actionKey = iterators.next();
            if (key.contains(actionKey)) {//later separate.
                String temp = FlowerAction.FlowerSixDomainActions.getString(actionKey);
                FlowerAction.doAction(temp, strings, output, NE);
            }
        }
    }
