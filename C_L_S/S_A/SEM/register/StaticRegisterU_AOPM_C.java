...
public class StaticRegisterU_AOPM_C {
	public static void load(StaticFunctionMapU_AOPM_E staticFunctionMapU_AOPM_E) {
		...
		StaticFunctionMapU_AOPM_C.annotationMap.put("DNAAuthStatusCheckEmailAndPds"
				, "app:string:data");
		StaticFunctionMapU_AOPM_C.annotationMap.put("DNAAuthStatusCheckEmailAndPde"
				, "app:string:data");
		StaticFunctionMapU_AOPM_C.annotationMap.put("DNAAuthStatusCheckEmailAndPassword"
				, "app:string:data");
		...
		StaticFunctionMapU_AOPM_C.annotationMap.put("getNewTokenFromUsrAndUsrTokenByDNA"
				, "usr:usrToken");
		...
		StaticFunctionMapU_AOPM_C.annotationMap.put("liangHuaDengChaMines", "input:scale");
		//稍后去掉拼音接口, 只有中文和英文
		...
		Iterator<String> iterators= StaticFunctionMapU_AOPM_C.annotationMap.keySet().iterator();
		while(iterators.hasNext()) {
			String temp= iterators.next().toString();
			StaticRootMap.staticBloomingTimes.put(temp, (long) 0);
			StaticRootMap.staticClass_XE_Map.put(temp, "U_AOPM");
		}
	}
}
