...
import S_A.SEM.bloom.StaticFunctionMapQ_VECS_C;
...
public class StaticRegisterQ_VECS_C {
	public static void load(StaticFunctionMapQ_VECS_E staticFunctionMapQ_VECS_E) {
		...
		StaticFunctionMapQ_VECS_C.annotationMap.put("searchFromTablewithScale"
				, "key:标题:内容:lookrot");
		
		Iterator<String> iterators= StaticFunctionMapQ_VECS_C.annotationMap.keySet().iterator();
		while(iterators.hasNext()) {
			String temp= iterators.next().toString();
			StaticRootMap.staticBloomingTimes.put(temp, (long) 0);
			StaticRootMap.staticClass_XE_Map.put(temp, "Q_VECS");
		}
	}
}
