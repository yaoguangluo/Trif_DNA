...
public class StaticFunctionMapQ_VECS_E extends StaticFunctionMapCASE
	implements StaticFunctionMapQ_VECS_C {
	...
	public void searchFromTable(App NE, int rangeScale) {
		...
		for (int i = 0; i < stringReg.length; i++) {
			stringReg[i] = key._S_.substring(i * 3, (i * 3
				+ 3) < key._S_.length() ? (i * 3 + 3)
					: key._S_.length() - 1);
		}
		while (iteratorForCopy.hasNext()) {
			String iteratorForCopyString = iteratorForCopy.next();
			score[copyCount] = iteratorForCopyString;
			String temps = dic_xj.get(iteratorForCopyString)
				.toString();
			...
			Here: while (iteratorWordFrequency.hasNext()) {
				...
				if (temps.contains(mapSearchaAtII)) {
					...
					int wfs = (int) wordFrequencySearch
						.get_frequency();
					wfs = wfs > 5 ? 5 : wfs;
					...
					if (S_Maps.mingCi.containsKey(mapSearchaAtII)
						...
						|| S_Maps.xingRongCi.containsKey(
							mapSearchaAtII) || S_Maps.weiCi
								.containsKey(mapSearchaAtII)) {
						reg[copyCount] += 2;
					}
					reg[copyCount] += 1;
			...
			score_code[copyCount] = score_code[copyCount]
				* reg[copyCount];
			//词距
			int code = 100;
			int tempb = 0;
			...
			if (key._S_.length() > 4) {
				...
				for (int i = 0; i < string.length; i++) {
					if (temps.contains(string[i])) {//因为用FrequencyMap代替, 所以要乘以频率。
						tempb += code * mapSearchWithoutSort.getW(
							string[i]).get_frequency();
					}
				}
				//断句
				for (int i = 0; i < stringReg.length; i++) {
				...
				score_code[copyCount] = (int) (tempa / Math.pow(
					app.app_S.lookrot + 1, 4) + tempb * Math.pow(
						app.app_S.lookrot, 2));
			}
			if (key._S_.replace(" ", "").length() > 1 && key._S_
				.replace(" ", "").length() < 5) {
		...
		int new_count = 0;
		newTableModel.getDataVector().clear();
		Here: for (int i = copy_xj.size() - 1; i > -1; i--) {
			...
			tableData[new_count] = new Object[] { new_count + 1,
				score_code[i], score[i], dic_xj.get(score[i])
					.toString() };
	...
	public List<Object[]> searchFromTablewithScale(App NE,
		...
		for (int i = 0; i < stringReg.length; i++) {
			stringReg[i] = searchkey._S_.substring(i * 3, (i * 3
				+ 3) < searchkey._S_.length() ? (i * 3 + 3)
					: searchkey._S_.length() - 1);
		}
		while (iteratorForCopy.hasNext()) {
			String iteratorForCopyString = iteratorForCopy.next();
			score[copyCount] = iteratorForCopyString;
			String temps = textContent.get(iteratorForCopyString)
				.toString();
			if (isKey) {
			...
			Here: while (iteratorWordFrequency.hasNext()) {
				...
				if (temps.contains(mapSearchaAtII)) {
					...
					score[copyCount] = iteratorForCopyString;
					if (score[copyCount].contains(searchkey._S_
						.replace(" ", ""))) {
						reg[copyCount] += 500;
					}
					if (searchkey._S_.contains(score[copyCount]
						.replace(" ", ""))) {
						reg[copyCount] += 500;
					}
					if (!pos.containsKey(mapSearchaAtII)) {
			...
			if (searchkey._S_.length() > 4) {
				...
				score_code[copyCount] = (int) (tempa / Math.pow(
					lookrot + 1, 4) + tempb * Math.pow(lookrot, 2));
			}
		...
		Object[][] tableData = new Object[count][18];
		int new_count = 0;
		Here: for (int i = nameContent.size() - 1; i > -1; i--) {
			if (score_code[i] < rangeScale) {
				continue Here;
			}
			String value = "";
	...
	public static void main() {
		...
		textContent.put("罗**", "罗**luoy**");
		textContent.put("罗**", "罗**luoy**");
		textContent.put("罗**", "罗**luoz**");
		textContent.put("罗**", "罗**luoz**");
