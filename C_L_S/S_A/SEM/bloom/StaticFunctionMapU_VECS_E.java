...
//, 只是static 函数而已,
public class StaticFunctionMapU_VECS_E extends StaticFunctionMapCASE
	implements StaticFunctionMapU_VECS_C {
	...
	//稍后加前缀
	@Override
	public Map<Double, Map<String, Boolean>> getMapFromDoubleStringArray(
		App NE) {
		double[] score = NE._I_U.scoreA;
		String[] nameScore = NE._I_U.nameScore;
		return Double_ESU.getMapFromDoubleStringArray(score,
			nameScore);
	}

	@Override
	public Map<Integer, Map<String, Boolean>> getMapFromIntStringArray(
		App NE) {
	...
	@Override
	public Map<Integer, Map<String, Boolean>> getMapFromIntStringArray_ZhongYao(
		App NE) {
		int[] score = NE._I_U.score;
		String[] nameScore = NE._I_U.nameScore;
		return Double_ESU.getMapFromIntStringArray_ZhongYao(score,
			nameScore);
	}
	...
	public Object[][][] arrayToMatrix3D(App NE) {
		...
		return Array_ESU_X_arrayToMatrix3D._E(objects, widthRange,
			heightRange);
	}
	...
	public Object[][] xlsOrCsvFileToObjectMartix(App NE) {
		...
		return CSV_ESU.xlsOrCsvFileToObjectMartix(filePath._S_,
			pageSheetIndex);
	}
	...
	public List<Object[]> xlsOrCsvFileToListObject(App NE) {
		AES_QMS_XSD_TIH filePath = NE._I_U.FilePath;
		int pageSheetIndex = NE._I_U.pageSheetIndex;
		return CSV_ESU.xlsOrCsvFileToListObject(filePath._S_,
			pageSheetIndex);
	}
	...
	public List<Object[]> xlsOrCsvFileToRangedListObject(App NE) {
		...
		return CSV_ESU.xlsOrCsvFileToRangedListObject(filePath._S_,
			pageSheetIndex, rows, culumns);
	}
	...
	public Object[][] xlsOrCsvFileToRangedObjectMartix(App NE) {
		...
		Map<Integer, Boolean> rows = NE._I_U.rowsMap;
		Map<Integer, Boolean> culumns = NE._I_U.culumns;
		return CSV_ESU.xlsOrCsvFileToRangedObjectMartix(filePath._S_,
			pageSheetIndex, rows, culumns);
	}
	...
	public String timeStampToMiliSecondsWithSize(App NE) {
		...
		return Date_ESU.timeStampToMiliSecondsWithSize(timestamp,
			size);
	}
	...
	public String getJson(App NE) {
		...
		return new HttpUnicode().getJson(urlString._S_,
			jsonString._S_);
	}
	...
	public String postXML(App NE) {
		...
		return new HttpUnicode().postXML(urlString._S_,
			XMLString._S_);
	...
	public String postJsonWithSercurity(App NE) {
		AES_QMS_XSD_TIH urlString = NE._I_U.urlString;
		AES_QMS_XSD_TIH jsonString = NE._I_U.jsonString;
		return new HttpUnicode().postJsonWithSercurity(urlString._S_,
			jsonString._S_);
	}
	...
	public String postXMLWithSercurity(App NE) {
		AES_QMS_XSD_TIH urlString = NE._I_U.urlString;
		AES_QMS_XSD_TIH XMLString = NE._I_U.xmlString;
		return new HttpUnicode().postXMLWithSercurity(urlString._S_,
			XMLString._S_);
	}
	...
	public void pixTableToFile(App NE) {
		...
		Image_ESU_X_pixTableToFile._E(pngOutputPath._S_, pix,
			fileType._S_);
	}
	...
	public void pixRGBTableToFile(App NE) {
		...
		AES_QMS_XSD_TIH fileType = NE._I_U.fileType;
		Image_ESU_X_pixRGBTableToFile._E(pngOutputPath._S_, pixRed,
			pixGreen, pixBlue, fileType._S_);
	}
	...
	public void bufferedImageToFile(App NE) {
		...
		AES_QMS_XSD_TIH fileType = NE._I_U.fileType;
		Image_ESU_X_bufferedImageToFile._E(pngOutputPath._S_,
			bufferedImage, fileType._S_);
	...
	public void imageToFile(App NE) {
		...
		AES_QMS_XSD_TIH fileType = NE._I_U.fileType;
		Image_ESU_X_imageToFile._E(pngOutputPath._S_, image,
			fileType._S_);
	...
	public void bufferedImageToScaleImageFile(App NE) {
		...
		Image_ESU_X_bufferedImageToScaleImageFile._E(
			pngOutputPath._S_, bufferedImage, fileType._S_, scale);
	}

	@Override
	public List<Integer> waveImageFileToArrayWithBlack(App NE) {
		AES_QMS_XSD_TIH FilePath = NE._I_U.FilePath;
		return ImageToList_X_waveImageFileToArrayWithBlack._E(
			FilePath._S_);
	}
	...
	public List<Integer> waveImageToArrayWithBlack(App NE) {
		BufferedImage bufferedImage = NE._I_U.bufferedImage;
		return ImageToList_X_waveImageToArrayWithBlack._E(
			bufferedImage);
	}
	...
	public List<Integer> waveImageToArrayWithColor(App NE) {
		...
		return ImageToList_X_waveImageToArrayWithColor._E(
			bufferedImage, colorRange);
	}
	...
	public List<Integer> waveImageFileToArrayWithColor(App NE) {
		AES_QMS_XSD_TIH filePath = NE._I_U.filePath;
		int colorRange = NE._I_U.colorRange;
		return ImageToList_X_waveImageFileToArrayWithColor._E(
			filePath._S_, colorRange);
	}

	@Override
	public Map<Integer, List<Integer>> waveImageFileToArrayMapWithBlack(
		App NE) {
		AES_QMS_XSD_TIH filePath = NE._I_U.filePath;
		return ImageToList_X_waveImageFileToArrayMapWithBlack._E(
			filePath._S_);
	}

	@Override
	public Map<Integer, List<Integer>> waveImageToArrayMapWithBlack(
		App NE) {
		//Map<Integer, List<Integer>>
		BufferedImage bufferedImage = NE._I_U.bufferedImage;
		return ImageToList_X_waveImageToArrayMapWithBlack._E(
			bufferedImage);
	}
	...
	public BufferedImage toolkitImageToBufferImage(App NE) {
		...
		return U_X_toolkitImageToBufferImage._E(image, positionX,
			positionY, width, height, imageObserver);
	}

	@Override
	public BufferedImage toolkitImageToBufferImageAboutIconConfig(
		App NE) {
		Image image = NE._I_U.image;
		int positionX = NE._I_U.positionX;
		int positionY = NE._I_U.positionY;
		return U_X_toolkitImageToBufferImageAboutIconConfig._E(image,
			positionX, positionY);
	}
	...
	public Object stringIteratorToJsonString(App NE) {
		...
		return Iterator_ESU_X_stringIteratorToJsonString._E(gson,
			iterator);
	}
	...
	public double[] sort(App NE) {
		...
		return new LYG9DWithDoubleTopSort5D().sort(array, range,
			deeps);
	}
	...
	public Object[][] txtFileToObjectMatrix(App NE) {
		...
		return TXT_ESU_X_txtFileToObjectMatrix._E(filePath._S_,
			stopBy._S_);
	}
	...
	public Object[][] txtFileToObjectMatrixWithRange(App NE) {
		...
		return TXT_ESU_X_txtFileToObjectMatrixWithRange._E(
			filePath._S_, stopBy._S_, rangeBegin, rangeEnd);
	}
	...
	public List<String[]> txtFileToListStringArray(App NE) {
		AES_QMS_XSD_TIH filePath = NE._I_U.filePath;
		AES_QMS_XSD_TIH stopBy = NE._I_U.stopBy;
		return TXT_ESU_X_txtFileToListStringArray._E(filePath._S_,
			stopBy._S_);
	}
	...
	public String[] stringBuilderToSplitArray(App NE) {
		AES_QMS_XSD_TIH stopBy = NE._I_U.stopBy;
		StringBuilder stringBuilder = NE._I_U.stringBuilder;
		return S_ESU_X_stringBuilderToSplitArray._E(stopBy._S_,
			stringBuilder);
	}
	...
	public String charsetSwap(App NE) {
		...
		return String_ESU_X_charsetSwap._E(string._S_,
			inputCharset._S_, outputCharset._S_);
	}
