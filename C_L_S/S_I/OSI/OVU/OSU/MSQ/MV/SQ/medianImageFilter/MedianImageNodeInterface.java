...
*  208号 阳光家园别墅小区 第十栋
 * */
public class MedianImageNodeInterface extends OSU_MCI_MCU_MSI{	
	public MedianImageNodeInterface() {
		...
		_SQ_OSU_MSQ_OSU_AVQ_ASQ_SQ_VPC_PCS=new String("MedianImage");
	...
	public void MEI_MSU(JTextPane jTextPane, ThisCanvas canvas) {
		_SQ__OVQ_OSQ_VSQ
		=new MedianImageOSU_MSQ_AVQ_ASQ_OVQ_OSQ_VSQ();
		_SQ__OPE_OPC_ECI=new MedianImageOSU_MSQ_OPE_OPC_ECI();
		_SQ__AOU_AOQ_VES
		=new MedianImageOSU_MSQ_AOI_AOD_AOU_AOQ_VES((MedianImageOSU_MSQ_OPE_OPC_ECI) 
				_SQ__OPE_OPC_ECI);
		_SQ__AOU_AOQ_VES.config();
		showed=false;
	}
	public void OPE_E(JTextPane jTextPane)  {
		((MedianImageOSU_MSQ_OPE_OPC_ECI) _SQ__OPE_OPC_ECI)
		.run((MedianImageOSU_MSQ_AVQ_ASQ_OVQ_OSQ_VSQ) 
				_SQ__OVQ_OSQ_VSQ);
	}
