...
*  208号 阳光家园别墅小区 第十栋
 * */
public class MorphologyFilterInterface extends OSU_MCI_MCU_MSI{	
	public MorphologyFilterInterface() {
		thisIcon= FastReadProjectFile.getDetaImageIconReader("imageread.jpg", "DetaResources/imgETL/");
		_SQ_OSU_MSQ_OSU_AVQ_ASQ_SQ_VPC_PCS=new String("Morphology");
	...
	public void MEI_MSU(JTextPane jTextPane, ThisCanvas canvas) {
		_SQ__OVQ_OSQ_VSQ=new MorphologyFilterView();
		_SQ__OPE_OPC_ECI=new MorphologyFilterRun();
		_SQ__AOU_AOQ_VES
		= new MorphologyFilterPanel((MorphologyFilterRun) _SQ__OPE_OPC_ECI);
		_SQ__AOU_AOQ_VES.config();
		showed=false;
	}
	public void OPE_E(JTextPane jTextPane)  {
		((MorphologyFilterRun) _SQ__OPE_OPC_ECI)
		.run((MorphologyFilterView) _SQ__OVQ_OSQ_VSQ);
	}
