...
*  208号 阳光家园别墅小区 第十栋
 * */
public class EmbossFilterInterface extends OSU_MCI_MCU_MSI{	
	public EmbossFilterInterface() {
		thisIcon= FastReadProjectFile.getDetaImageIconReader("imageread.jpg", "DetaResources/imgETL/");
		_SQ_OSU_MSQ_OSU_AVQ_ASQ_SQ_VPC_PCS= new String("EmbossFilter");
	...
	public void MEI_MSU(JTextPane jTextPane, ThisCanvas canvas) {
		_SQ__OVQ_OSQ_VSQ= new EmbossFilterView();
		_SQ__OPE_OPC_ECI= new EmbossFilterRun();
		_SQ__AOU_AOQ_VES 
		= new EmbossFilterPanel((EmbossFilterRun) _SQ__OPE_OPC_ECI);
		_SQ__AOU_AOQ_VES.config();
		showed= false;
	}
	public void OPE_E(JTextPane jTextPane)  {
		((EmbossFilterRun) _SQ__OPE_OPC_ECI)
		.run((EmbossFilterView) _SQ__OVQ_OSQ_VSQ);
	}
	...
	public OSU_MCI_MCU_MSI AOP_MVE_CSI_DUQ()  {
		_SQ_OSU_MSQ_OSU_MCI_MCU_MSI= new EmbossFilterInterface();   
