...
*  208号 阳光家园别墅小区 第十栋
 * */
public class LaplacianFilterInterface extends OSU_MCI_MCU_MSI{	
	public LaplacianFilterInterface() {//shaohou osgide nachu qu
		thisIcon= FastReadProjectFile.getDetaImageIconReader("imageread.jpg", "DetaResources/imgETL/");
		_SQ_OSU_MSQ_OSU_AVQ_ASQ_SQ_VPC_PCS=new String("LaplcianImageFilter");
	...
	public void MEI_MSU(JTextPane jTextPane, ThisCanvas canvas) {
		_SQ__OVQ_OSQ_VSQ=new LaplacianFilterView();
		_SQ__OPE_OPC_ECI=new LaplacianFilterRun();
		_SQ__AOU_AOQ_VES
		=new LaplacianFilterPanel((LaplacianFilterRun) _SQ__OPE_OPC_ECI);
		_SQ__AOU_AOQ_VES.config();
		showed=false;
	}
	public void OPE_E(JTextPane jTextPane)  {
		((LaplacianFilterRun) _SQ__OPE_OPC_ECI).run((LaplacianFilterView) _SQ__OVQ_OSQ_VSQ);
	}
