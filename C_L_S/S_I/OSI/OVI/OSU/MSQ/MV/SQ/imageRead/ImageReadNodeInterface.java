...
*  208号 阳光家园别墅小区 第十栋
 * */
public class ImageReadNodeInterface extends OSU_MCI_MCU_MSI{	
	public ImageReadNodeInterface() {
		...
		_SQ_OSU_MSQ_OSU_AVQ_ASQ_SQ_VPC_PCS=new String("ImageRead");
	...
	public void MEI_MSU(JTextPane jTextPane, ThisCanvas canvas) {
		_SQ__OVQ_OSQ_VSQ=new ImageReadOSU_MSQ_AVQ_ASQ_OVQ_OSQ_VSQ();
		_SQ__OPE_OPC_ECI=new ImageReadOSU_MSQ_OPE_OPC_ECI();
		_SQ__AOU_AOQ_VES=new ImageReadOSU_MSQ_AOI_AOD_AOU_AOQ_VES((ImageReadOSU_MSQ_OPE_OPC_ECI) _SQ__OPE_OPC_ECI);
		_SQ__AOU_AOQ_VES.config();
		showed=false;
	}
	public void OPE_E(JTextPane jTextPane)  {
		((ImageReadOSU_MSQ_OPE_OPC_ECI) _SQ__OPE_OPC_ECI).run((ImageReadOSU_MSQ_AVQ_ASQ_OVQ_OSQ_VSQ) _SQ__OVQ_OSQ_VSQ);
	}
	...
	public OSU_MCI_MCU_MSI AOP_MVE_CSI_DUQ()  {  
		_SQ_OSU_MSQ_OSU_MCI_MCU_MSI= new ImageReadNodeInterface();   
