...
*  208号 阳光家园别墅小区 第十栋
 * */
public class FindColorBInterface extends OSU_MCI_MCU_MSI{	
	public FindColorBInterface() {
		thisIcon= FastReadProjectFile.getDetaImageIconReader("imageread.jpg", "DetaResources/imgETL/");
		_SQ_OSU_MSQ_OSU_AVQ_ASQ_SQ_VPC_PCS= new String("FindColorB");
	...
	public void MEI_MSU(JTextPane jTextPane, ThisCanvas canvas) {
		_SQ__OVQ_OSQ_VSQ=new FindColorBView();
		_SQ__OPE_OPC_ECI=new FindColorBRun();
		_SQ__AOU_AOQ_VES=new FindColorBPanel((FindColorBRun) _SQ__OPE_OPC_ECI);
		_SQ__AOU_AOQ_VES.config();
		showed=false;
	}
	public void OPE_E(JTextPane jTextPane)  {
		((FindColorBRun) _SQ__OPE_OPC_ECI).run((FindColorBView) _SQ__OVQ_OSQ_VSQ);
	}
	...
	public OSU_MCI_MCU_MSI AOP_MVE_CSI_DUQ()  {  
		_SQ_OSU_MSQ_OSU_MCI_MCU_MSI= new FindColorBInterface();   
