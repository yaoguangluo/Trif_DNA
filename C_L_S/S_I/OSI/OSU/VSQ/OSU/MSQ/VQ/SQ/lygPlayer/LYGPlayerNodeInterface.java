...
public class LYGPlayerNodeInterface extends OSU_MCI_MCU_MSI{	
	public LYGPlayerNodeInterface() {
		thisIcon= FastReadProjectFile.getDetaImageIconReader("imageread.jpg", "DetaResources/imgETL/");
		_SQ_OSU_MSQ_OSU_AVQ_ASQ_SQ_VPC_PCS=new String("LYGPlayer");
	...
	public void MEI_MSU(JTextPane jTextPane, ThisCanvas canvas) {
		_SQ__OVQ_OSQ_VSQ=new LYGPlayerOSU_MSQ_AVQ_ASQ_OVQ_OSQ_VSQ();
		_SQ__OPE_OPC_ECI=new LYGPlayerOSU_MSQ_OPE_OPC_ECI();
		_SQ__AOU_AOQ_VES
		=new LYGPlayerOSU_MSQ_AOI_AOD_AOU_AOQ_VES((LYGPlayerOSU_MSQ_OPE_OPC_ECI) _SQ__OPE_OPC_ECI);
		_SQ__AOU_AOQ_VES.config();
		showed=false;
	}
	public void OPE_E(JTextPane jTextPane)  {
		((LYGPlayerOSU_MSQ_OPE_OPC_ECI) _SQ__OPE_OPC_ECI)
		.run((LYGPlayerOSU_MSQ_AVQ_ASQ_OVQ_OSQ_VSQ) _SQ__OVQ_OSQ_VSQ);
	}
